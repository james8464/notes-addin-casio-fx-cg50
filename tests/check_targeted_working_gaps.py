...
CASES = [
    ...
    ("range(-9)", ["Range:", "constant", "Answer: y = -9"]),
    ("range(2*x+3)", ["Range:", "linear", "all real"]),
    ("range(x^2+4*x+7)", ["vertex x = -2", "minimum y = 3", "Answer: y >= 3"]),
    ("range(-x^2+4*x+1)", ["vertex x = 2", "maximum y = 5", "Answer: y <= 5"]),
    ("range(x^2-4*x+5,x,0,5)", ["Interval: 0 <= x <= 5", "f(0) = 5", "f(5) = 10", "vertex x = 2 is inside", "Answer: 1 <= y <= 10"]),
