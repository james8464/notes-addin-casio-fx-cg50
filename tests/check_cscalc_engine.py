...
def main():
    ...
    require("Convert 10101111₂ to hexadecimal.", ["10101111_2 = 175_10", "175_10 = AF_16"])
    require("Convert 255₁₀ to hex.", ["255_10 = 255_10", "255_10 = FF_16"])
    require("Convert 377₈ to binary.", ["377_8 = 255_10", "255_10 = 011111111_2"])
    ...
    require("Convert −13 to 8-bit two’s complement.", ["8-bit two's complement", "Encode negative", "-13 -> 11110011"])
    require("Convert –13 to 8 bit two complement.", ["8-bit two's complement", "Encode negative", "-13 -> 11110011"])
    require("Decode F3₁₆ as 8-bit two’s complement.", ["F3_16 = 11110011_2", "Decode 11110011 as 8-bit two's complement.", "11110011 = -13"])
    ...
    require("Add binary 101101 and 011111 using 8 bit two complement and state overflow.", ["Decode/add in fixed width, discard carry beyond 8 bits", "00101101=45, 00011111=31", "45+31=76 -> 01001100"])
    ...
    require("Add binary 1011 and 1101", ["Add binary place values", "11 + 13 = 24", "result = 11000_2"])
    require("Find the result of adding unsigned binary 1111 and 1", ["Add binary place values", "1111_2 = 15, 1_2 = 1", "result = 10000_2"])
    ...
    require("Multiply binary 1011 by 110", ["Binary multiplication uses shifted partial products", "11 * 6 = 66", "result = 1000010_2"])
    ...
    require("Check odd parity for byte 10110010 with parity bit 1", ["Odd parity", "total one-bits = 5", "parity check passes"])
    ...
    require("Calculate CRC remainder for data 11010011101100 divisor 1011", ["CRC: append 3 zeros", "remainder = 100", "transmitted codeword = 11010011101100100"])
    ...
    require("Calculate modulo 11 check digit for ISBN digits 0 3 0 6 4 0 6 1 5 2 using weights 10 9 8 7 6 5 4 3 2.", ["Use the 9 stated digit-weight pairs.", "0*10", "5*2", "weighted sum = 130", "check digit = (11-9) mod 11 = 2"])
    ...
    require("Evaluate postfix A B C NOT OR AND with A=1 B=0 C=1", ["Boolean postfix/RPN", "read A=1", "NOT 1 = 0", "0 OR 0 = 0", "1 AND 0 = 0", "answer = 0"])
    require("Convert the infix expression A+B*C to reverse polish notation.", ["infix = A+B*C", "postfix = ABC*+"])
    require("Convert infix A and (B or not C) to postfix", ["infix = A*(B+!C)", "postfix = ABC!+*"])
    ...
    require("Apply Caesar cipher shift 5 to HELLO", ["Caesar shift moves each letter by 5 places", "encrypted text = MJQQT"])
    require("Use Caesar cipher with shift 7 to encrypt HELLO WORLD.", ["Caesar shift moves each letter by 7 places", "helloworld -> OLSSVDVYSK", "encrypted text = OLSSVDVYSK"])
    ...
    require("Convert denary 0.625 to binary", ["fractional bits = 101", "0.625_10 = 0.101_2"])
    require("Convert decimal 0.3 to binary fraction 8 places", ["fractional bits = 01001100", "0.3_10 = 0.01001100_2"])
    require("Convert denary 13.625 to binary", ["whole part 13 -> 00001101", "fractional bits = 101", "13.625_10 = 1101.101_2"])
    ...
    require("normalise binary floating point 0.001101 mantissa 8 exponent 4", ["0.203125 = 0.8125 * 2^-2", "mantissa (8 bits) = 01101000", "exponent (4 bits) = 1110"])
    require("Normalise the binary floating point number 0.001101 with a 6 bit mantissa and 4 bit exponent.", ["0.203125 = 0.8125 * 2^-2", "mantissa (6 bits) = 011010", "exponent (4 bits) = 1110"])
    ...
    require("A sound sample is 44.1 kHz, 16 bit, stereo, 3 min. Calculate size.", ["44100*180*16*2", "254016000 bits"])
    ...
    require("A network sends 1.5GB in 2 minutes. Find bitrate in Mbps.", ["1.5 GB = 1.2e+10 bits", "bit rate = 12/120 = 0.1 Gbit/s", "= 100 Mbit/s"])
    ...
    require("Encode text BANANA using run length encoding.", ["Run-length encode", "runs: Bx1,Ax1,Nx1,Ax1,Nx1,Ax1", "encoded string = 1B1A1N1A1N1A"])
    require("Run length encode MISSISSIPPI.", ["runs: Mx1,Ix1,Sx2,Ix1,Sx2,Ix1,Px2,Ix1", "encoded string = 1M1I2S1I2S1I2P1I"])
    ...
    require("A hash table has 10 slots. Insert keys 23 43 13 using modulo 10 and linear probing.", ["23 mod 10 = 3, place at 3", "43 mod 10 = 3 occupied", "13 mod 10 = 3 occupied"])
    ...
    require("Convert 01001001 01000011 from ASCII to characters.", ["ASCII bytes map each 8-bit group to one character", "01001001 = 73 -> I", "01000011 = 67 -> C", "text = IC"])
    ...
    require("Show that (A+B).(A+C) simplifies to A+B.C", ["Simplify both sides", "LHS simplifies to A+BC", "RHS simplifies to A+BC", "Same output rows"])
    ...
    require("Convert F(A,B,C)=A'BC+AB'C+ABC to minterms and simplify.", ["minterms: 3,5,7", "simplified = BC+AC"])
    ...
    require("Simplify Boolean F(A,B,C)=Σm(0,2,6,7) with dont cares 1 5", ["K-map/minterm method", "don't-care rows: 1,5", "simplified = A'C'+AB"])
    ...
    require("Show NAND only form of A+B.", ["Use NAND", "NAND form: ((A NAND A) NAND (B NAND B))"])
    require("Write A OR B using NAND gates only.", ["Use NAND", "((A NAND A) NAND (B NAND B))"])
    require("Convert F=A∨B to NAND only.", ["Use NAND", "NAND form: ((A NAND A) NAND (B NAND B))"])
    ...
    require("Calculate image file size for 300 dpi 4 inch by 6 inch 24 bit colour", ["width = 4*300 = 1200 pixels", "bits = 1200*1800*24 = 51840000 bits", "= 6.17980957 MiB"])
    ...
    require("Truth table for A ⊕ B ⊕ C.", ["Truth table for a^b^c", "A B C | F", "1 1 1 | 1", "minterms: 1,2,4,7"])
    ...
    require("Find minterms for F(A,B,C)=A and not C", ["minterms: 4,6", "simplified = AC'"])
    require("Find POS for F(A,B,C)=A and not C", ["variables: A,B,C", "zero rows: 0,1,2,3,5,7", "simplified POS = (A)&(C')"])
    require("Simplify F(A,B,C)=sum m 1 3 5 7", ["variables: A,B,C", "minterms: 1,3,5,7", "simplified = C"])
    ...
    require("A processor has a 20-bit address bus and each address stores 2 bytes. Calculate maximum memory capacity in MiB.", ["address bus 20 bits gives 2^20 = 1048576 addresses", "bytes per address = 2", "capacity = 1048576*2 = 2097152 bytes", "= 2 MiB"])
    ...
    require("Using 8-bit one's complement, represent -45.", ["8-bit one's complement", "Write |-45| in binary", "-45 -> 11010010"])
    require("Decode 11010010 using one's complement.", ["MSB=1", "11010010 = -45"])
    require("What is the range of 6 bit one complement?", ["one's complement range", "-(2^5-1) to +(2^5-1) = -31 to 31"])
    ...
    require("Calculate check digit using modulo 11 weights 2 3 4 for digits 5 7 2.", ["5*2", "7*3", "weighted sum = 39", "check digit = (11-6) mod 11 = 5"])
    ...
    require("A video stream has bitrate 5Mbps for 45 seconds. Find file size in MB.", ["bit rate = 5000000 bit/s", "size = 5000000*45 = 225000000 bits", "= 26.82209015 MiB"])
    ...
    require("Huffman codes A=0 B=10 C=11 encode ABAC.", ["Use the given Huffman code table.", "ABAC -> 010011"])
    require("Use Vigenere cipher key LEMON to encrypt ATTACKATDAWN.", ["Vigenere uses a repeated keyword", "keyword = lemon", "attackatdawn -> LXFOPVEFRNHR"])
    require("Decrypt Vigenere LXFOPVEFRNHR with key LEMON.", ["keyword = lemon", "lxfopvefrnhr -> ATTACKATDAWN", "decrypted text = ATTACKATDAWN"])
    require("Find maxterms from truth table output 0110 variables A B.", ["variables: A,B", "0-cells/maxterms: 0,3", "simplified POS = (A+B)&(A'+B')"])
