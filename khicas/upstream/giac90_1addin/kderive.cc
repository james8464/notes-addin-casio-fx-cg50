...
namespace giac {
  ...
  static gen derive_SYMB(const gen &g_orig,const identificateur & i,GIAC_CONTEXT){
	...
	return v.front();
      if (v.empty())
	return zero;
      gen res=_plus(gen(v,_SEQ__VECT),contextptr); // symb_plus(v);
      return res;
    }
    if (s.sommet==at_prod){
      bool do_step=step_infolevel(contextptr)>1 && count_noncst(s.feuille,i)>1;
	...
	return -derive(symb_ln(s.feuille._SYMBptr->feuille),i,contextptr);
	...
	if (f.type==_VECT){
	  ...
	  for (;it!=itend;++it)
	    res=res+derive(symb_ln(*it),i,contextptr);
	  return res;
	}
      }
    }
	...
	if (!is_zero(derive(v0,i,contextptr)))
	  return gensizeerr(gettext("diff incomplete gamma"));
	...
	return a3*b3-a2*b2+_integrate(makesequence(newint,v[1],v[2],v[3]),contextptr);
      }
      return gensizeerr(contextptr);
    }
	...
	if (darg.type==_VECT){
	  ...
	  for (int i=0;i<int(darg._VECTptr->size());++i){
	    gen fprime=symb_derive(f,i);//symbolic(at_derive,makesequence(f,i));
	    res += darg[i]*symb_of(fprime,arg); //symbolic(at_of,makesequence(fprime,arg));
	  }
	  return res;
	}
	// f(arg)'=arg'*f'(arg)
	gen fprime=symb_derive(f);
	return darg*symb_of(fprime,arg); // symbolic(at_of,makesequence(fprime,arg));
      }
    }
	...
	if (*othervar._IDNTptr==i){ // _FUNCnd derivative
	  ...
	  symbolic sprime(s);
	  res.push_back(2);
	  return symb_derive(gen(res,_SEQ__VECT));
	}
	else {
	  vecteur var;
	  ...
	  nderiv.push_back(1);
	  nderiv.push_back(1);
	  return symb_derive((*s.feuille._VECTptr)[0],var,nderiv);//symbolic(at_derive,gen(makevecteur((*s.feuille._VECTptr)[0],var,nderiv),_SEQ__VECT));
	}
      }
      else { // derive(f,x,n)
	...
	if (othervar.type==_IDNT){
	  ...
	  return symb_derive((*s.feuille._VECTptr)[0],othervar,nderiv);//symbolic(at_derive,gen(makevecteur((*s.feuille._VECTptr)[0],othervar,nderiv),_SEQ__VECT));
	}
      }
    }
    ...
    return symb_derive(s,i);//symbolic(at_derive,gen(makevecteur(s,i),_SEQ__VECT));
    //i.dbgprint();
    //s.dbgprint();
  }
  ...
  gen derive(const gen & e,const gen & vars,GIAC_CONTEXT){
    ...
    switch (vars.type){
    case _INT_:
      return symb_derive(e,vars); // symbolic(at_derive,makesequence(e,vars));
    case _IDNT:
  ...
  gen derive(const gen & e,const gen & vars,const gen & nderiv,GIAC_CONTEXT){
	...
	ecopie=eprime;
      ...
      if (j==n+1)
	return eprime;
      return symb_derive(ecopie,vars,n+1-j);//symbolic(at_derive,gen(makevecteur(ecopie,vars,n+1-j),_SEQ__VECT));
    }
  ...
  gen symb_derive(const gen & a,const gen & b){
    return symbolic(at_derive,makesequence(a,b));
  }
  ...
  gen symb_derive(const gen & a,const gen & b,const gen &c){
    ...
    return symb_derive(makesequence(a,b,c));
  }
  ...
  gen _derive(const gen & args,GIAC_CONTEXT){
    ...
    res=v[0];
    for (;it!=itend;++it)
      res=ratnormal(_derive(makesequence(res,*it),contextptr),contextptr);
    return res;
  }
  ...
  static define_unary_function_eval4_quoted (__derive,&_derive,_derive_s,printasderive,texprintsommetasoperator);
  ...
  gen _function_diff(const gen & g,GIAC_CONTEXT){
    ...
    if (g.is_symb_of_sommet(at_function_diff)){
      ...
      return symb_of(gen(symbolic(at_composepow,makesequence(at_function_diff,2))),f); //symbolic(at_of,makesequence(gen(symbolic(at_composepow,makesequence(at_function_diff,2))),f));
    }
	...
	if (f1.is_symb_of_sommet(at_composepow)){
	  ...
	  if (f1f.type==_VECT && f1f._VECTptr->size()==2 && f1f._VECTptr->front()==at_function_diff){
	    return symb_of(gen(symbolic(at_composepow,makesequence(at_function_diff,f1f._VECTptr->back()+1))),f2);// symbolic(at_of,makesequence(gen(symbolic(at_composepow,makesequence(at_function_diff,f1f._VECTptr->back()+1))),f2));
	  }
	}
      }
    }
  ...
  void domain(const gen & f,const gen & x,vecteur & eqs,vecteur &excluded,int mode,GIAC_CONTEXT){
	...
	continue;
      ...
      if (u==at_inv || (mode==1 && (u==at_ln || u==at_log10 ))){
  ...
  gen domain(const gen & f,const gen & x,int mode,GIAC_CONTEXT){
    ...
    if (res.size()==1 && res.front()==x)
      return tmp.size()==1?tmp.front():symb_and(tmp);
    else {
	...
	for (int j=0;j<int(excluded.size());++j){
	  ...
	  if (!res[i].is_symb_of_sommet(at_and)){
	    res[i]=symb_and(res[i],tmp[j]);//symbolic(at_and,makesequence(res[i],tmp[j]));
	    continue;
	  }
    ...
    tmp.insert(tmp.begin(),symb_ou(res));
    return symb_and(tmp);
  }
