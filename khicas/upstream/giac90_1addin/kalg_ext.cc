...
namespace giac {
  ...
  gen common_EXT(gen & a,gen & b,const vecteur * l,GIAC_CONTEXT){
    ...
    if (!trouve){
      ...
      if (innerdim){
	gen params;
	*logptr(contextptr) << gettext("Param root branch warning") << endl;
	if (l && l->size()>=2){
  ...
  */
  ...
  gen _sturmab(const gen & g_orig,GIAC_CONTEXT){
	...
	*logptr(contextptr) << gettext("Warning: variable name ignored: ") << x << endl;
    ...
    if (!is_zero(ai) || !is_zero(bi)){
      gen p=_e2r(makesequence(P,vecteur(1,x)),contextptr),n,d,g1,g2;
  ...
  gen _sturm(const gen & g,GIAC_CONTEXT){
    ...
    if (s==3){
      if (v[2].type!=_IDNT)
	return gensizeerr(contextptr);
      gen S=_e2r(makesequence(v[0],v[2]),contextptr);
      if (is_undef(S)) return S;
      gen R=_e2r(makesequence(v[1],v[2]),contextptr);
  ...
  gen fminmax(const gen & g,int minmax,GIAC_CONTEXT){
	...
	if (u==at_abs && tmp.type==_VECT && tmp._VECTptr->size()==2 ){
	  ...
	  if (is_positive(-t2,contextptr)){
	    return makeline(-t2,-t1);//gen(makevecteur(-t2,-t1),_LINE__VECT);
	  }
	  // t1<=0  t2>=0
	  if (is_greater(-t1,t2,contextptr))
	    return makeline(0,-t1); // gen(makevecteur(0,-t1),_LINE__VECT);
	  else
	    return makeline(0,t2); // gen(makevecteur(0,t2),_LINE__VECT);
	}
    ...
    if (range==makevecteur(minus_inf,plus_inf))
      ...
      giac_assume(symb_and(symb_superieur_egal(var,range[0]),symb_inferieur_egal(var,range[1])),contextptr);// giac_assume(symbolic(at_and,makevecteur(symb_superieur_egal(var,range[0]),symb_inferieur_egal(var,range[1]))),contextptr);
      w=solve(df,var,2,contextptr);
      if (savevar==var)
	...
	return xmin;
      if (minmax %3 ==2)
	return xmax;
      return makeline(xmin,xmax); // gen(makevecteur(xmin,xmax),_LINE__VECT);      
    }
    else {
      if (minmax %3 ==1)
	return resmin;
      if (minmax %3 ==2)
	return resmax;
      return makeline(resmin,resmax); // gen(makevecteur(resmin,resmax),_LINE__VECT);
    }
  }
  ...
  int find_range(const gen & g,vecteur & a,GIAC_CONTEXT){
	...
	a=vecteur(1,makeline(minus_one,plus_one));// a=vecteur(1,gen(makevecteur(minus_one,plus_one),_LINE__VECT));
	return 1;
      }
    }
    a=vecteur(1,makeline(minus_inf,plus_inf));// a=vecteur(1,gen(makevecteur(minus_inf,plus_inf),_LINE__VECT));
    return 1;
  }
  ...
  static int insturmsign1(const gen & g0,bool strict,GIAC_CONTEXT){
	...
	if (is_zero(limit(testg,idnttmp,last,-1,contextptr))){
	  ...
	  gg=_r2e(makesequence(p,v0),contextptr)/den;
	}
  ...
  int sturmsign(const gen & g0,bool strict,GIAC_CONTEXT){
	...
	tmp=insturmsign(w.front(),strict,contextptr); return tmp==-2?-2:res*tmp;
      default:
	tmp=insturmsign(symb_prod(w),strict,contextptr); return tmp==-2?-2:res*tmp;
      }
    }
