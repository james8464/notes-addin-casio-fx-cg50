...
namespace giac {
  ...
  static gen nop_inv(const gen & e,GIAC_CONTEXT){
    return symbolic(at_nop,gen(symb_inv(e)));
  }
  static gen nop_pow(const gen & e,GIAC_CONTEXT){
    ...
    if ( (e._VECTptr->back().type!=_INT_) || (e._VECTptr->back().val>=0) || ( (e._VECTptr->front().type==_SYMB) && (e._VECTptr->front()._SYMBptr->sommet==at_exp) ) )
      return symb_pow(change_subtype(e,_SEQ__VECT));
    return nop_inv(symb_pow(e._VECTptr->front(),-e._VECTptr->back()),contextptr);
  }
  ...
  gen linear_apply(const gen & e,const gen & x,gen & remains, GIAC_CONTEXT, gen (* f)(const gen &,const gen &,gen &,const context *)){
	...
	res = f(symb_prod(non_constant),x,remains,contextptr);
  ...
  static gen substconj_(const gen & N,const gen & X,const gen & x,const gen & dephasage_,bool residue_only,GIAC_CONTEXT){
    ...
    gen cx(c*x),sx(s*x);
    if (residue_only)
      return N2*sign(sx,contextptr);
    gen res=normal(rb,contextptr)*symb_ln(pow(X,2)+ratnormal(-2*cx,contextptr)*X+x.squarenorm(contextptr)); 
    gen atanterm=pi/cst_pi*symb_atan((X-cx)/(sx));
    if (X.is_symb_of_sommet(at_tan))
      atanterm += pi*sign(sx,contextptr)*symbolic(at_floor,X._SYMBptr->feuille/pi+plus_one_half);
    res += N2*atanterm;
    return res;
  }
  ...
  static gen substconj(const gen & N,const gen & X,const gen & x,const gen & dephasage,bool residue_only,GIAC_CONTEXT){
    if (has_i(N)){
      ...
      return gen(substconj_(Nr,X,x,dephasage,residue_only,contextptr),substconj_(Ni,X,x,dephasage,residue_only,contextptr));
    }
  ...
  gen surd(const gen & c,int n,GIAC_CONTEXT){
    ...
    if (n%2 && is_positive(-c,contextptr)){
      // if (c.type==_FLOAT_) return -exp(ln(-c,contextptr)/n,contextptr);
  ...
  gen _surd(const gen & args,GIAC_CONTEXT){
    ...
    if (is_equal(a)){
      ...
      return symb_equal(_surd(makesequence(a0,b),contextptr),_surd(makesequence(a1,b),contextptr));
    }
    ...
    // if (c.type==_FLOAT_) c=get_int(c._FLOAT_val);
	...
	return pow(a,inv(c,contextptr),contextptr);	
      return symbolic(at_NTHROOT,makesequence(b,a));
    }
    if (c.type==_INT_ && c==b)
  ...
  static bool integrate_deno_length_2(const polynome & num,const vecteur & v,const vecteur & l,const vecteur & lprime,gen & res,bool residue_only,int intmode,GIAC_CONTEXT){
	...
	for (int i=0;i<d;++i){
	  gen exp2(exp((2*i*cst_ipi())/d,contextptr));
	  gen x=c1s*exp2;
	...
	*jt=normal(*jt * pow(c,-k),contextptr);
	...
	for (int i=1;i<deg;++i){
	  if (gcd(i,deg)!=1)
	    continue;
	  gen x=c*exp((2*i*cst_ipi())/deg,contextptr);
	...
	c=pow(c,inv(n,contextptr),contextptr);
	for (int i=0;i<n;++i){
	  gen x=c*exp((2*i*cst_ipi())/n,contextptr);
	  res += normal(subst(N,X,x,false,contextptr),contextptr)*ln(X-x,contextptr);
	}
	return true;
      }
  ...
  static gen integrate_linearizable(const gen & e,const gen & gen_x,gen & remains_to_integrate,int intmode,GIAC_CONTEXT){
	...
	coeff=coeff*gen(cos(reaxbi,contextptr),sin(reaxbi,contextptr));
	...
	vecteur resnum;
	gen resden,resplus;
	fxnd(gen(in_rea,in_ima),in_anum,in_aden);//fxnd(in_rea+cst_i*in_ima,in_anum,in_aden);
	...
	gen exptmp0(exp(reaxb,contextptr)),exptmp1(expi(imaxb,contextptr)),exptmp2(exp(-cst_i*imaxb,contextptr));
	if (trig_type==1)
	  res += exptmp0*rdiv(resplus*exptmp1-resmoins*exptmp2,plus_two*cst_i,contextptr);
	else
	  res += exptmp0*rdiv(resplus*exptmp1+resmoins*exptmp2,plus_two,contextptr);
      } // end for (jt)
    } // end for (it)
	...
	if (is_quadratic_wrt(argument,gen_x,a,b,c,contextptr)){
	  if (!is_positive(-a,contextptr)){
	    ...
	    */
	    ...
	    // -> asin(a*x+b/2)
	    vecteur vin(makevecteur(u,symb_atan(u))),vout(makevecteur(uasx,inv(-2,contextptr)*sD*asin(ratnormal((-2*a*gen_x-b)/D,contextptr),contextptr)));
  ...
  static gen integrate_rational_end(vector< pf<gen> >::iterator & it,vector< pf<gen> >::const_iterator & itend,const gen & x,const gen & xvar,const vecteur & l,const vecteur & lprime,const polynome & ipnum,const polynome & ipden,const gen & ratpart,gen & remains_to_integrate,int intmode,GIAC_CONTEXT){
	...
	lnpart=lnpart+rdiv(r2e(d,lprime,contextptr),r2e(lnpartden,lprime,contextptr),contextptr)*gen(uselog?lnabs2(r2e(it->fact,l,contextptr),xvar,contextptr):symb_ln(r2e(it->fact,l,contextptr)));
	...
	if (uselog){
	  ...
	  if (tmpatan.is_symb_of_sommet(at_tan))
	    ...
	    if (  //0 && 
	      ...
	      gen si=symb_sin(argtan),ci=symb_cos(argtan);
	      tmpatan=symb_atan(ratnormal(((beta-1)*si+gamma*(1+ci))/(1+beta+gamma*si+(1-beta)*ci),contextptr));
  ...
  static gen integrate_rational(const gen & e, const gen & x, gen & remains_to_integrate,gen & xvar,int intmode,GIAC_CONTEXT){
	...
	xvar=symb_cos(ratnormal(2*xvar._SYMBptr->feuille,contextptr));
	xvar=(1-xvar)/(1+xvar);
	aa/=2;
      }
      xvar=pow(xvar,aa);
	...
	if (xvar.is_symb_of_sommet(at_tan)){
	  ...
	  xvar=symb_sin(ratnormal(2*xvar._SYMBptr->feuille,contextptr));
	}
	else {
	...
	if (xvar.is_symb_of_sommet(at_tan)){
	  q=N; r=D;
	  xtoinvx(-2,q,r,N,D,true);
	  xvar=symb_tan(ratnormal(2*xvar._SYMBptr->feuille,contextptr));
	}
	else
	  xvar=xvar-inv(xvar,contextptr);
  ...
  gen add_lnabs(const gen & g,GIAC_CONTEXT){
    return symb_ln(abs(g,contextptr));
  }
  ...
  void surd2pow(const gen & e,vecteur & subst1,vecteur & subst2,GIAC_CONTEXT){
	...
	g=_pow(gen(gv,_SEQ__VECT),contextptr);//symb_pow(gen(gv,_SEQ__VECT));
      }
    }
  ...
  gen integrate_id_rem(const gen & e_orig,const gen & gen_x,gen & remains_to_integrate,GIAC_CONTEXT,int intmode){
    ...
    *logptr(contextptr) << gettext("integrate step -2 ") << e << endl;
	...
	if ( (tmp.is_symb_of_sommet(at_superieur_strict) || 
	  ...
	  repl=symbolic(at_same,makesequence(tmp,0));
  ...
  gen symb_integrate(const gen &f,const gen &x,const gen &a,const gen &b){
    return symbolic(at_integrate,makesequence(f,x,a,b));
  }
  ...
  gen integrate_id(const gen & e,const identificateur & x,GIAC_CONTEXT){
    ...
    if (is_zero(remains_to_integrate))
      ...
      return res+symbolic(at_integrate,makesequence(remains_to_integrate,x));
  }
  ...
  gen _integrate(const gen & args,GIAC_CONTEXT){
    ...
    *logptr(contextptr) << gettext("integrate begin") << endl;
	...
	return symb_prog3(a,0,_integrate(makesequence(b,a),contextptr));
	...
	return _integrate(makesequence(v[0],v[1]),contextptr)+v[2];
    ...
    if (x.type!=_IDNT){
      if (x.type<_IDNT)
	return gensizeerr(contextptr);
      // if (abs_calc_mode(contextptr)==38 && x.type!=_SYMB) return gensizeerr(contextptr);
	...
	if (ld==vecteur(1,v[1]) || ld.empty())
	  return intnum(makesequence(v[0],v[1],v[2],v[3]),false,contextptr,true);
      }
      v0orig=v[0];
      aorig=borne_inf=v[2];
      borig=borne_sup=v[3];
	...
	for (int i=0;i<vs/2;++i){
	  ...
	  if (ck_is_greater(l,borne_sup,contextptr)){
	    ...
	    res += _integrate(makesequence(tmp,x,borne_inf,borne_sup),contextptr);
	    return ck_int_numerically(v0orig,x,aorig,borig,(chsign?-res:res),contextptr);
	  }
	  ...
	  if (positif){
	    ...
	    res += _integrate(makesequence(tmp,x,l,borne_sup),contextptr);
	    borne_sup=l; // continue with integral from borne_inf to l
	    continue;
	  }
	  // compute integral between borne_inf and l
	  res += _integrate(makesequence(tmp,x,borne_inf,l),contextptr);
    ...
    if (s==2){
      if (is_zero(rem))
	return primitive;
      return primitive + symbolic(at_integrate,makesequence(rem,x));
    }
    // here s==4
    ...
    if (!lop(res,at_bounded_function).empty())
      res=undef;
    if (is_undef(res)){
      // if (res.type==_STRNG && abs_calc_mode(contextptr)==38) return res;
	...
	*logptr(contextptr) << gettext("Unhandled singular point ")+sp[i].print(contextptr);
    ...
    if (!is_zero(rem)){
      if (is_inf(res))
	return symb_integrate(v[0],x,v[2],v[3]);
      if (ordonne || !desordonne)
	res = res + symb_integrate(rem,x,v[2],v[3]);
      else
	res = res - symb_integrate(rem,x,v[3],v[2]);
      return res;
    }
    return ck_int_numerically(v0orig,x,aorig,borig,res,contextptr);
  }
  static const char _integrate_s []="integrate";
  static define_unary_function_eval4_quoted (__integrate,&_integrate,_integrate_s,0,&texprintsommetasoperator);
  ...
  gen evalf_int(const gen & f0,const gen & x0,const gen & a,const gen &b,const gen & eps,int nmax,bool romberg_method,GIAC_CONTEXT,bool exactcheck){
    ...
    gen value=undef;
    if (tegral(f,x,a,b,eps,(1 << nmax),value,exactcheck,contextptr))
      return value;
    return gensizeerr("Adaptive method failure");
  }
...
**********************************************************************/
  ...
  gen seqprod(const gen & g,int type,GIAC_CONTEXT){
	...
	return change_subtype(seqprod(makesequence(symb_interval(v[0],v[1]),v[2]),type,contextptr),0);
      if (type==0)
  ...
  bool maple_sum_product_unquote(vecteur & v,GIAC_CONTEXT){
	...
	if (tmp.size()==2){
	  ...
	  v[1]=symb_equal(tmp[0],eval(tmp[1],eval_level(contextptr),contextptr));
	}
      }
      else
  ...
  gen _sum(const gen & args,GIAC_CONTEXT) {
	...
	return -_sum(makesequence(v[0],v[1],v[3]+(numeval?gen(1.0):plus_one),v[2]-1),contextptr);
  ...
  */ 
  ...
  gen odesolve(const gen & t0orig,const gen & t1orig,const gen & f,const gen & y0orig,double tstep,bool return_curve,double * ymin,double * ymax,int maxstep,GIAC_CONTEXT){
	...
	yv.push_back(identificateur(" y"+print_INT_(i)));
      tmp=f(makesequence(t_id,yv),contextptr);
    }
     ...
     */
    gen tolerance=epsilon(contextptr)>1e-10?epsilon(contextptr):1e-10;
  ...
  static gen odesolve(const vecteur & w,GIAC_CONTEXT){
	...
	return gendimerr(contextptr);
      ...
      t1=v[3];
      gen newv1=symb_equal(v[1]._VECTptr->front(),symb_interval(v[2]._VECTptr->front(),v[3]));
  ...
  bool assume_t_in_ab(const gen & t,const gen & a,const gen & b,bool exclude_a,bool exclude_b,GIAC_CONTEXT){
    vecteur v_interval(1,makeline(a,b));//gen(makevecteur(a,b),_LINE__VECT));
    vecteur v_excluded;
    if (exclude_a)
