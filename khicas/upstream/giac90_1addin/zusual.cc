...
namespace giac {
  ...
  */
  ...
  gen alog10(const gen & e,GIAC_CONTEXT){
    ...
    gen res=pow(gen(10),e,contextptr);
    return res;
  }
  ...
  */
  ...
  gen _evalf(const gen & a,int ndigits,GIAC_CONTEXT){
    int save_decimal_digits=bf_global_prec; // decimal_digits(contextptr);
#ifndef HAVE_LIBMPFR
    if (ndigits>14)
      return gensizeerr(gettext("Longfloat library not available"));
#endif
    set_decimal_digits(ndigits,contextptr);
    //bf_global_prec=(int(std::ceil(3.3219280948873623*giacmax(absint(ndigits),1)))+31)/32*32;
    ...
    bf_global_prec=save_decimal_digits;
    return res;
  }
