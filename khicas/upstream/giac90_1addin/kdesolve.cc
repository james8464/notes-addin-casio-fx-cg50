...
namespace giac {
  ...
  gen laplace(const gen & f0,const gen & x,const gen & s,GIAC_CONTEXT){
    ...
    if (!assume_t_in_ab(t,plus_inf,plus_inf,true,true,contextptr))
      return gensizeerr(contextptr);
    gen res=_integrate(makesequence(f*exp(-t*x,contextptr),x,0,plus_inf),contextptr);
    for (int i=1;i<=n;++i){
      if (is_undef(res))
	return res;
      res = _integrate(makesequence(res,t,0,t),contextptr);
      res += _integrate(makesequence(f/pow(-x,i),x,0,plus_inf),contextptr);
    }
  ...
  static gen pf_ilaplace(const gen & e0,const gen & x, gen & remains,GIAC_CONTEXT){
	...
	*logptr(contextptr) << gettext("Warning, exponential x coeff is positive ") << expa << endl;
	...
	tmp = tmp*symbolic(at_Dirac,(i==nums-1)?laplace_var:makesequence(laplace_var,nums-1-i));
	res += tmp;
      }
  ...
  static gen in_desolve_with_conditions(const vecteur & v_,const gen & x,const gen & y,const gen & solution_generale,const vecteur & parameters,const gen & f,int step_info,GIAC_CONTEXT){
	...
	if (ndiff)
	  *jt=symb_of(symb_derive(y,x,int(ndiff)),jt->_VECTptr->front())-jt->_VECTptr->back();//symbolic(at_of,makesequence(symbolic(at_derive,makesequence(y,x,int(ndiff))),jt->_VECTptr->front()))-jt->_VECTptr->back();
	else
	  *jt=symb_of(y,jt->_VECTptr->front())-jt->_VECTptr->back();//symbolic(at_of,makesequence(y,jt->_VECTptr->front()))-jt->_VECTptr->back();
      }
    }
    ...
    vecteur parameters_solutions=*_solve(makesequence(conditions,parameters),contextptr)._VECTptr;
  ...
  bool desolve_linn(const gen & x,const gen & y,const gen & t,int n,vecteur & v,vecteur & parameters,gen & result,int step_info,GIAC_CONTEXT){
	...
	vecteur lopei=lop(laplace_cst,at_integrate);
	if (lopei.empty()){
	  gen arbitrary,tmp;
	  for (int i=n-1;i>=0;--i){
	    parameters.push_back(diffeq_constante(int(parameters.size()),contextptr));
  ...
  gen desolve_f(const gen & f_orig,const gen & x_orig,const gen & y_orig,int & ordre,vecteur & parameters,gen & fres,int step_info,GIAC_CONTEXT){
	...
	if (separate_variables(f,x,y,xfact,yfact,step_info,contextptr)){ // y'/yfact=xfact
	  ...
	  if (!prv.empty() && prv[0].is_symb_of_sommet(at_ln) && is_linear_wrt(pr,prv[0],pra,prb,contextptr)){
	    pr=_lncollect(pra*(symb_ln(parameters.back()*prv[0]._SYMBptr->feuille))+prb,contextptr);
	  }
	  else
	  ...
	  if (pr.is_symb_of_sommet(at_ln))
	    pr=symb_ln(parameters.back()*pr._SYMBptr->feuille);
	  else
	    pr=parameters.back()+pr;
#endif
	...
	for (unsigned i=0;i<soly2v.size();++i){
	  ...
	  if (is_quadratic_wrt(soly2c,der1,a,b,c,contextptr)
	    ...
	    gen ysol=desolve(symb_equal(symb_derive(y,x),usolj),x,y,ordre,parameters,contextptr);
	    if (is_undef(ysol))
	  ...
	  if (is_zero(derive(soly2c,x,contextptr))){ // x-incomplete
	    ...
	    parameters=paramsave;
	    gen usol=desolve(symb_equal(symb_derive(der1,y),soly2c/der1),y,der1,ordre,parameters,contextptr);
	    if (is_undef(usol))
	      return unable_to_solve_diffeq();
	    if (usol.type!=_VECT)
	      usol=vecteur(1,usol);
