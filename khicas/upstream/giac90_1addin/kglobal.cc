...
#endif
    if (esc_flag){
      if (!ctrl_c) cout << "Interrupt requested" << endl;
...
#endif
  volatile char ctrl_c=false,interrupted=false,kbd_interrupted=false;
...
#include "static_lexer_at.h"
  ...
  static void python_import(string & cur,int cs,int posturtle,int poscmath,int posmath,int posnumpy,int posmatplotlib,GIAC_CONTEXT){
    ...
    if (posnumpy>=0 && posnumpy<cs){
      ...
      if (alertnum){
	alertnum=false;
	alert("Python shortcuts disabled",contextptr);
      }
      return;
    }
    if (posturtle>=0 && posturtle<cs){
      ...
      if (alertturtle){
	alertturtle=false;
	alert("Turtle disabled",contextptr);
      }
      return;
    }
...
#ifdef CASCAS_DISABLE_PYTHON_TRANSLATE
    return s_orig;
#else
