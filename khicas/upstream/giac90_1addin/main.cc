...
#if 0
/* Source-audit denylist. Compiled implementation lives in cascas_working.cc. */
static bool cascas_ident_char(char c){
  return isalnum((unsigned char)c) || c=='_';
}

static unsigned cascas_hash_ident(const char *s,int begin,int end){
...
static bool cascas_reject_removed_feature(const char *input){
  ...
  if (strchr(input,'%') || strstr(input,"[["))
    return true;
  for (int i=0;input[i];){
    if (!cascas_ident_char(input[i])){
      ++i;
      continue;
    }
    int begin=i;
    while (input[i] && cascas_ident_char(input[i]))
      ++i;
    if (cascas_denied_hash(cascas_hash_ident(input,begin,i)))
      return true;
  }
  return false;
}
