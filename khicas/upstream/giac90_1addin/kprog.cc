...
namespace giac {
  ...
  static void replace_keywords(const gen & a,const gen & b,gen & newa,gen & newb,GIAC_CONTEXT){
	...
	*logptr(contextptr) << gettext("Invalid argument ") << *it << gettext("; use ") << it->_SYMBptr->feuille._VECTptr->front() << endl;
	*it=it->_SYMBptr->feuille._VECTptr->front();
      }
      if (it->is_symb_of_sommet(at_deuxpoints)){
    ...
    if (v1.empty())
      ...
      *logptr(contextptr) << gettext("Variable name(s) replaced, check ") << v1 << endl;
  ...
  symbolic symb_program_sto(const gen & a_,const gen & b_,const gen & c_,const gen & d,bool embedd,GIAC_CONTEXT){
	...
	*logptr(contextptr) << gettext("Use function_diff or unapply: ") << c << endl;
	 ...
	 if (!lofc_no_d.empty()){
	   *logptr(contextptr) << gettext("Algebraic function warning") << endl;
...
#pragma clang diagnostic pop
  ...
  static string printaslocal(const gen & feuille,const char * sommetstr,GIAC_CONTEXT){
    ...
    //*logptr(contextptr) << "LOC " << local_global << endl;
    if (local_global.type==_VECT && local_global._VECTptr->size()==2){ 
      gen f=local_global._VECTptr->front(),b=local_global._VECTptr->back();
      if (f.type!=_IDNT){
	locals=gen2vecteur(f);
  ...
  */
  ...
  bool is_address(const gen & g,size_t & addr,GIAC_CONTEXT){
    if (g.type==_INT_){
      addr=g.val;
      return true;
    }
  ...
  gen _write(const gen & args,GIAC_CONTEXT){
    ...
    if (args.type==_VECT){
      ...
      size_t addr;
      if (v.size()==2 && is_address(v.front(),addr,contextptr) && v.back().type==_INT_){
    ...
    *logptr(contextptr) << "Write "<<fname << endl;
    ...
    Bfile_StrToName_ncpy(pFile, (const unsigned char*)fname.c_str(), size+1);
