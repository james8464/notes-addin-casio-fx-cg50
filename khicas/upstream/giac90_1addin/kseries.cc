...
namespace giac {
  ...
  bool series__SPOL1(const gen & e_orig,const identificateur & x,const gen & lim_point,int ordre,int direction,sparse_poly1 & s,GIAC_CONTEXT){
	...
	continue;
      ...
      //*logptr(contextptr) << *lvx_it << " " << lvx_s << endl;
	...
	if (nargs==4 && temp__SYMB.sommet==at_sum){
	  ...
	  if (contains(k,x)){
	    invalidserieserr(gettext("bad summation variable"));
	    return false;
	  }
	...
	if (temp__SYMB.sommet==at_integrate){
	  ...
	  gen t=tempfv[1];
	  if (contains(t,x)){
	    invalidserieserr(gettext("bad integration variable"));
	    return false;
	  }
	...
	if (tf.type==_VECT && tf._VECTptr->size()==2){
	  ...
	  vecteur expansion(1,symb_of(tff,image_of_lim_point)); // symbolic(at_of,makesequence(tff,image_of_lim_point)));
	  expansion.reserve(ordre);
	  for (int i=1;i<=ordre;i++){
	    gen fn;
	    if (i==1)
	      fn=symb_of(symbolic(at_function_diff,tff),image_of_lim_point);//symbolic(at_of,makesequence(symbolic(at_function_diff,tff),image_of_lim_point));
	    else
	      fn=symb_of(symb_of(symbolic(at_composepow,makesequence(at_function_diff,i)),tff),image_of_lim_point);//symbolic(at_of,makesequence(symbolic(at_of,makesequence(symbolic(at_composepow,makesequence(at_function_diff,i)),tff)),image_of_lim_point));
  ...
  void rlvarx(const gen &e,const gen & xgen,vecteur & res){
	...
	for (--n;n>1;--n){
	  res.push_back(symb_derive(it->_SYMBptr->feuille._VECTptr->front(),(*it->_SYMBptr->feuille._VECTptr)[1],n));//res.push_back(symbolic(at_derive,makesequence(it->_SYMBptr->feuille._VECTptr->front(),(*it->_SYMBptr->feuille._VECTptr)[1],n)));
	}
	res.push_back(symb_derive(it->_SYMBptr->feuille._VECTptr->front(),(*it->_SYMBptr->feuille._VECTptr)[1]));//res.push_back(symbolic(at_derive,makesequence(it->_SYMBptr->feuille._VECTptr->front(),(*it->_SYMBptr->feuille._VECTptr)[1])));
      }
      if (it->type==_SYMB) {
	rlvarx(it->_SYMBptr->feuille,xgen,res);
	if ( (it->_SYMBptr->sommet==at_pow)
	     && contains((*(it->_SYMBptr->feuille._VECTptr))[1],xgen) )
	  rlvarx(symb_ln((*(it->_SYMBptr->feuille._VECTptr))[0]),xgen,res);
      }
    }
  }
  ...
  */
  ...
  static gen ln_expand0_(const gen & e,GIAC_CONTEXT){
    ...
    if (e._SYMBptr->sommet==at_prod)
      return symb_plus(apply(e._SYMBptr->feuille,ln_expand0_,contextptr));
    if (e._SYMBptr->sommet==at_inv)
  ...
  static gen in_limit(const gen & e0,const identificateur & x,const gen & lim_point,int direction,GIAC_CONTEXT){
    ...
    if (_about(x,contextptr)!=x){
      identificateur xprime("x"+print_INT_(giac_rand(contextptr)));
	...
	// gen chk=first_try; //chk=recursive_normal(first_try,contextptr);
	if (hasnum && !is_undef(chk) && abs(chk-chknum,contextptr)>1e-10 && abs(1-chk/chknum,contextptr)>1e-10){
	  chk=undef;
	  e=_simplify(e,contextptr);
	}
	...
	*logptr(contextptr) << gettext("infinity unsigned; use +infinity")<< endl;
	...
	return gensizeerr(gettext("complex erf limit unsupported"));
    }
    // Rewrite constants
  ...
  static bool mrv(const gen & e,const identificateur & x,vecteur & faster_var,vecteur & coeff_ln, vecteur & slower_var,GIAC_CONTEXT){
	...
	temp=symb_exp((*(temp._SYMBptr->feuille._VECTptr))[1]*ln((*(temp._SYMBptr->feuille._VECTptr))[0],contextptr));
  ...
  static bool mrv_lead_term(const gen & e,const identificateur & x,gen & coeff, gen & mrv_var, gen & exponent,sparse_poly1 & q,int begin_ordre,GIAC_CONTEXT,bool series){
	...
	if (!p.empty() && !is_undef(p.front().coeff) ){
	  if (!is_zero(p.front().exponent))
	    argln=argln*symb_pow(w,-p.front().exponent);
	  lf1.push_back(*it);
	  lf2.push_back(p.front().exponent*(dont_invert?g:-g)+symb_ln(argln));
	}
      }
      if (!lf1.empty())
  ...
  static define_unary_function_eval4 (__limit,&_limit,_limit_s,0,&texprintsommetasoperator);
  ...
  static gen sparse_poly12gen_expand(const sparse_poly1 & s,const identificateur & x,const gen & mrv_var,int ordre,gen & remains,bool with_order_size,GIAC_CONTEXT){
    ...
    if (mrv_fait.is_symb_of_sommet(at_inv) && mrv_fait._SYMBptr->feuille.is_symb_of_sommet(at_exp))
      mrv_fait=symb_exp(symb_neg(mrv_fait._SYMBptr->feuille._SYMBptr->feuille));
  ...
  bool convert_to_euler_mac_laurin(const gen & g,const identificateur & n,gen & res,GIAC_CONTEXT){
	...
	*logptr(contextptr) << gettext("No Euler Mac");
	...
	return false;
      res=symbolic(at_euler_mac_laurin,makesequence(f0,F0,x,f[2],f[3]));
      return true;
    }
