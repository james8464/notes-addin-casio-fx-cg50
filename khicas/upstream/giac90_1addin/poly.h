...
namespace giac {
  ...
  bool operator == (const tensor<T> & p,const tensor<T> & q){
    if (p.dim!=q.dim || p.coord.size()!=q.coord.size())
      return false;
    typename vector< monomial<T> >::const_iterator it=p.coord.begin(),jt=q.coord.begin(),itend=p.coord.end();
    for (;it!=itend;++it,++jt){
      if (*it!=*jt)
	return false;
    }
  ...
  */
  ...
  nio::ios_base<I> & operator << (nio::ios_base<I> & os, const std::vector< pf<T> > & v){
