  ...
  */
  ...
  gen r2sym(const polynome & p, const vecteur & l,GIAC_CONTEXT){
    if (p.coord.empty())
      return 0;
    if (p.dim==0){
      return p.constant_term(); // -> static initialization
    }
  ...
  */
  ...
  gen normal(const gen & e,bool distribute_div,GIAC_CONTEXT){
    ...
    if (is_equal(e)){
      ...
      return symb_equal(normal(v.front(),distribute_div,contextptr),normal(v.back(),distribute_div,contextptr)); // symbolic(at_equal,makesequence(normal(v.front(),distribute_div,contextptr),normal(v.back(),distribute_div,contextptr)));
    }
	...
	*logptr(contextptr) << gettext("Unable to build a single algebraic extension for simplifying.\nTrying rational simplification only. This might return a wrong answer if simplifying 0/0!") << endl;
	...
	gen res=r2sym(f,l,contextptr);
	purgeassumelist(L,contextptr);	
	return symb_neg(res);//symbolic(at_neg,res);
      }
    }
  ...
  gen _recursive_normal(const gen & e,GIAC_CONTEXT){
    ...
    if (is_algebraic_program(e,var,res))
      return symb_prog3(var,0,recursive_normal(res,contextptr));
    res=recursive_normal(e,true,contextptr);
    return res;
  }
  ...
  gen rationalgcd(const gen & a, const gen & b,GIAC_CONTEXT){
    ...
    if (is_algebraic_program(a,A,B) && is_algebraic_program(b,C,D)){
      if (A==C)
	return symb_prog3(A,0,gcd(B,D,contextptr));
  ...
  // symbolic symb_e2r(const gen & arg1,const gen & arg2){    return symbolic(at_e2r,makesequence(arg1,arg2));  }
  ...
  //symbolic symb_r2e(const gen & arg1,const gen & arg2){    return symbolic(at_r2e,makesequence(arg1,arg2));  }
  ...
  //symbolic symb_normal(const gen & args){    return symbolic(at_normal,args);  }
  ...
  //symbolic symb_non_recursive_normal(const gen & args){    return symbolic(at_non_recursive_normal,args);  }
  ...
  gen _factor(const gen & args,GIAC_CONTEXT){
    ...
    if (args.type!=_VECT && is_algebraic_program(args,var,res))
      return symb_prog3(var,0,_factor(res,contextptr));
  ...
  gen _collect(const gen & args,GIAC_CONTEXT){
    ...
    if (is_algebraic_program(args,var,res))
      return symb_prog3(var,0,_collect(res,contextptr));
    if (is_equal(args))
  ...
  // symbolic symb_partfrac(const gen & args){    return symbolic(at_partfrac,args);  }
  gen _partfrac(const gen & args_,GIAC_CONTEXT){
    if (args_.type==_STRNG && args_.subtype==-1) return  args_;
    gen args(args_),var,res;
    if (is_algebraic_program(args,var,res))
      return symb_prog3(var,0,_partfrac(res,contextptr));
    if (is_equal(args))
  ...
  gen _resultant(const gen & args,GIAC_CONTEXT){
	...
	if (v[s-2]==at_det){
	  gen S=_sylvester(makesequence(p,q,x),contextptr);
	  return _det(makesequence(S,at_lagrange),contextptr);
	}
