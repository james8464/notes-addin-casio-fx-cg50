  ...
  symbolic symb_program_sto(const gen & a_,const gen & b_,const gen & c_,const gen & d,bool embedd,GIAC_CONTEXT){
    ...
    if (warn){
      *logptr(contextptr) << gettext("// Parsing ") << d << '\n';
      ...
      //if (c.is_symb_of_sommet(at_derive)) *logptr(contextptr) << gettext("Warning, defining a derivative function should be done with function_diff or unapply: ") << c << '\n';
      if (contains(a,vx_var) && c.type==_SYMB && (!lop(c,at_derive).empty() || !lop(c,at_integrate).empty() || !lop(c,at_int).empty()))
...
#pragma clang diagnostic pop
  ...
  gen _sample(const gen & args,GIAC_CONTEXT){
    ...
    if (args._VECTptr->size()!=2 || !is_integral(b) || b.type==_ZINT || b.val<0)
      return gensizeerr(contextptr);
    if (is_distribution(a)>0 || a.type==_SYMB || a.type==_IDNT)
  ...
  void debug_loop(gen &res,GIAC_CONTEXT){
	...
	m=giacmax(0,M-5);
      for (int i=m;i<=M;++i){
	PrintMiniMini(&dispx,&dispy,(unsigned char*)(print_INT_(i)+":"+ws[i]).c_str(),(i==w[4].val?1:0),TEXT_COLOR_BLACK,0);
    ...
    PrintMiniMini(&dispx,&dispy,(unsigned char*)"----------------",0,TEXT_COLOR_BLACK,0);
    dispx=0;
    dispy += 8;
    // progs += "======\n";
	...
	PrintMiniMini(&dispx,&dispy,(unsigned char*)evals.c_str(),0,TEXT_COLOR_BLACK,0);
	dispy+=12;
	dispx=0;	
	evals="";
	// evals += '\n';
    ...
    dispx=0;
    dispy=3*58;//LCD_HEIGHT_PX-40;
    PrintMini(&dispx,&dispy,(unsigned char *)" next | stepin | cont. | kill |             ",4,0xFFFFFFFF,0,0,COLOR_BLACK, COLOR_WHITE, 1, 0);
    w.push_back(dw);
  ...
  void set_decimal_digits(int n,GIAC_CONTEXT){
    ...
    //bf_global_prec=32*((int(std::ceil(3.3219280948873623*giacmax(absint(n),1)))+31)/32);
#else
  ...
  gen _Digits(const gen & g,GIAC_CONTEXT){
    ...
    bf_global_prec=(int(std::ceil(3.3219280948873623*giacmax(absint(args.val),1)))+31)/32*32;
...
#define BUFFER_SIZE 16384
  ...
  */
...
#if 1 //def STATIC_BUILTIN_LEXER_FUNCTIONS
  ...
  const gen & _A_unit = * (gen *) & alias_A_unit_rom;
  ...
  const gen & _K_unit = * (gen *) & alias_K_unit_rom;
  ...
  const gen & _cd_unit = * (gen *) & alias_cd_unit_rom;
  ...
  const gen & _m_unit = * (gen *) & alias_m_unit_rom;
  ...
  const gen & _mol_unit = * (gen *) & alias_mol_unit_rom;
  ...
  const define_alias_gen(alias_s_unit_rom,_IDNT,0,&ref_s_unit_rom);
  const gen & s_unit_rom_IDNT_e = * (gen *) & alias_s_unit_rom;
  const gen & _s_unit = * (gen *) & alias_s_unit_rom;
  ...
  const gen & _E_unit = * (gen *) & alias_E_unit_rom;
  ...
  static vecteur mksa_unit2vecteur(const mksa_unit * tmp){
	...
	v.reserve(5);
	v.push_back(Coeff);
	v.push_back((double)tmp->m);
	v.push_back((double)tmp->kg);
	v.push_back((double)tmp->s);
	v.push_back((double)tmp->A);
      }
    }
    else {
      v.reserve(9);
      ...
      v.push_back((double)tmp->s);
      v.push_back((double)tmp->A);
      v.push_back((double)tmp->K);
      v.push_back((double)tmp->mol);
      v.push_back((double)tmp->cd);
      v.push_back((double)tmp->E);
    }
    return v;
  }
  ...
  string printasunit(const gen & feuille,const char * sommetstr,GIAC_CONTEXT){
    ...
    vecteur & v=*feuille._VECTptr;
    string tmp0(v[1].print(contextptr)),tmp;bool skip=true;
