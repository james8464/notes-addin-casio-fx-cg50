...
namespace giac {
  ...
  gen _time(const gen & a,GIAC_CONTEXT){
    ...
    unsigned t1= RTC_GetTicks(); // 1 tick=1/128 s
    // CERR << t1 << endl;
    for (unsigned i=1;i<=100;++i){
      eval(a,level,contextptr);
      unsigned t2= RTC_GetTicks();
      if (t2>=t1+32)
	return double(t2-t1)/double(i)/128;
    }
    return 0.0;
  }
...
#define S_IWUSR 00200
  ...
  */
  ...
  gen _seqsolve(const gen & args,GIAC_CONTEXT){
	...
	if (!is_one(q0r0))
	  return gensizeerr("Can not find particular solution, general solution: "+res.print(contextptr));
	// u_{n+1}=l*u_{n}+c
	...
	*logptr(contextptr) << gettext("Can not find particular solution for inhomogeneous part") << endl;
	return symb_seqsolve(args);
      }
  ...
  static gen crsolve(const gen & f0,const gen &u,const gen & n,vecteur & initcond0,GIAC_CONTEXT){
	...
	if (is_linear_wrt(tmp,n,a,b,contextptr) && !is_zero(a)){
	  ...
	  return gensizeerr(gettext("Unable to solve recurrence"));
      }
    }
    if (vofa.empty())
      return undef;
