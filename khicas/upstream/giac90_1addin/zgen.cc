...
#else
const size_t stackptr=0x18000,stack_mask=0x1ffff;
#endif
...
/*
 define GINT_MALLOC in Makefile and in ../libfxcg/libc/stdlib.c 
 rebuild kgen.cc and main.cc
*/
#if 1 // ndef GINT_MALLOC
  #define ALLOCSMALL
#endif
...
#define debugtypeptr _CPLXptr
  ...
  volatile ref_count_t & gen::ref_count() const { 
#ifdef SMARTPTR64
    ...
    case _REAL:
      return __REALptr->ref_count;
    case _CPLX:
      return __CPLXptr->ref_count;
    case _IDNT:
      return __IDNTptr->ref_count;
    case _STRNG:
      return __STRNGptr->ref_count;
    case _POLY:
      return __POLYptr->ref_count;
    case _SPOL1:
      return __SPOL1ptr->ref_count;
    case _MOD:
      return __MODptr->ref_count;
    case _SYMB:
      return __SYMBptr->ref_count;
    case _VECT:
      return __VECTptr->ref_count;
    case _MAP:
      return __MAPptr->ref_count;
    case _FRAC:
      return __FRACptr->ref_count;
    case _EXT:
      return __EXTptr->ref_count;
    case _USER:
      return __USERptr->ref_count;
    default:
      return __ZINTptr->ref_count;
    }
#endif
...
#if 0
  gen::gen(): type(_INT_),subtype(0),val(0) {
  ...
  gen::gen(int i): type(_INT_),subtype(0),val(i) {
#ifdef COMPILE_FOR_STABILITY
      control_c();
#endif
  }
...
#endif
    if ( (((size_t) _ZINTptr) &stack_mask) > stackptr)
...
#endif
    if ( v.size()>1 &&
	 ( (((size_t) _VECTptr) &stack_mask) > stackptr ||
	   (((size_t) _VECTptr->begin()) &stack_mask) > stackptr )
	 )
      ctrl_c=interrupted=true;
    type=_VECT;
  ...
  ref_symbolic * new_ref_symbolic(const symbolic & s){
    ...
    *(unsigned *) &ptr->s.feuille=0;
    * (((unsigned *) &ptr->s.feuille) +1)=0;
  ...
  gen::gen(const symbolic & s){
    ...
    * ((ulonglong * ) this) = ulonglong(new_ref_symbolic(s)) << 16;
    ...
    type = _SYMB;
    subtype = 0;
    if ( _SYMBptr->sommet!=at_restart && _SYMBptr->sommet!=at_purge && (((size_t) _SYMBptr) &stack_mask) > stackptr)
      ctrl_c=interrupted=true;
  }
...
#endif
	if ( (((size_t) _POLYptr) &stack_mask) > stackptr)
	  ctrl_c=interrupted=true;
	type = _POLY;
      }
    }
  ...
  bool ref_mpz_t2gen(ref_mpz_t * mptr,gen & g){
	...
	return false;
      ...
      * ((ulonglong * ) &g) = ulonglong(mptr) << 16;
      ...
      g.__ZINTptr = mptr;
#endif
      if ( (((size_t) g._ZINTptr) &stack_mask) > stackptr)
	ctrl_c=interrupted=true;
      g.type =_ZINT;
      g.subtype=0;
      return true;
...
#endif
	if ( (((size_t) _SPOL1ptr) &stack_mask) > stackptr)
	  ctrl_c=interrupted=true;
	subtype=0;
	type=_SPOL1;
      }
  ...
  gen & gen::operator = (int a){
    if (type>_DOUBLE_ && type!=_FLOAT_ && type!=_FUNC)
      delete_ptr(subtype,type,__ZINTptr);
    type=0;
    subtype=0;
    val=a;
    return *this;
  ...
  gen & gen::operator = (double a){
    return *this=gen(a);
  }

  gen & gen::operator = (longlong a){
    return *this=gen(a);
  }
  ...
  gen _FRAC2_SYMB(const fraction & f){
    ...
    if (is_minus_one(f.num))
      return -symb_inv(f.den);      
    return symb_prod2(f.num,symb_inv(f.den));
  }
  ...
  gen _FRAC2_SYMB(const gen & n,const gen & d){
    return symb_prod2(n,symb_inv(d));
  }
  ...
  static bool in_eval_func(const gen & g,gen * evaledptr,GIAC_CONTEXT){
    if (0 && rpn_mode(contextptr) && (history_out(contextptr).size()>=unsigned(g.subtype)))
  ...
  bool gen::in_eval(int level,gen & evaled,const context * contextptr) const{
    ...
    if (!level)
      return false;
    switch (type) {
    case _INT_: case _DOUBLE_: case _ZINT: case _CPLX: case _POLY: case _FRAC: case _SPOL1: case _EXT: case _STRNG: case _MAP: case _EQW: case _GROB: case _POINTER_:
      return false;
    case _IDNT:
	...
	return true;
      ...
      //case _USER: return in_eval_user(*this,evaled,level,contextptr);
    case _MOD:
  ...
  bool has_evalf(const gen & g,gen & res,int level,GIAC_CONTEXT){
	...
	return res.type==_DOUBLE_ || res.type==_CPLX ;
      }
      else
	return false;
    }
  ...
  bool gen::in_evalf(int level,gen & evaled,const context * contextptr) const{
    if (!level)
      return false;
    switch (type) {
    case _DOUBLE_: case _STRNG: case _MAP: case _EQW: case _GROB: case _POINTER_:
      return false;
    case _INT_:
      //*logptr(contextptr) << "in_evalf 1 " << *this << endl;
      if (subtype)
	return false;
      evaled=gen(double(val));
      // *logptr(contextptr) << "in_evalf 2 " << evaled << endl;
      return true;
    case _ZINT:
#if 0 // def BCD
    ...
    case _CPLX: 
      ...
      return true;
      // case _USER: evaled=_USERptr->evalf(level,contextptr); return true;
    case _IDNT:
...
#endif
	{
	  if (is_zero(_FRACptr->num.im(contextptr)) && is_zero(_FRACptr->den.im(contextptr)))
	    ...
	    evaled=gen(re(contextptr).evalf(1,contextptr),im(contextptr).evalf(1,contextptr));// re(contextptr).evalf(1,contextptr)+cst_i*im(contextptr).evalf(1,contextptr);
	}
      return true;
#ifdef HAVE_LIBMPFR
      if (decimal_digits(contextptr)<=14)
  ...
  gen evalf2double_nock(const gen & g0,int level,const context * contextptr){
    ...
    if (g0.type==_DOUBLE_)
      return g0;
    if (g0.type==_FRAC || g0.type==_ZINT)
...
//      if (storcl_38(res,0,g0._IDNTptr->id_name,undef,false,contextptr)) return evalf2double_nock(res,level-1,contextptr);
	...
	*it=evalf2double_nock(*it,level,contextptr);
	...
	return g0;
      ...
      return gen(evalf2double_nock(*g0._CPLXptr,1,contextptr),evalf2double_nock(*(g0._CPLXptr+1),1,contextptr));//evalf2double_nock(*g0._CPLXptr,1,contextptr)+cst_i*evalf2double_nock(*(g0._CPLXptr+1),1,contextptr);
    }
    gen g=evalf(g0,level,contextptr);
    if (g.type==_CPLX)
      return gen(evalf2double_nock(*g._CPLXptr,1,contextptr),evalf2double_nock(*(g._CPLXptr+1),1,contextptr));//evalf2double_nock(*g._CPLXptr,1,contextptr)+cst_i*evalf2double_nock(*(g._CPLXptr+1),1,contextptr);
    return g;
  }
  ...
  bool gen::is_constant() const {
    switch (type ) {
    case _INT_: case _DOUBLE_: case _ZINT: case _CPLX:
      return true;
    case _VECT:
  ...
  bool is_atomic(const gen & e){
    return e.type<_POLY;// || e.type==_USER;
  }
  ...
  gen gen::conj(GIAC_CONTEXT) const {
    switch (type ) {
    case _INT_: case _DOUBLE_: case _ZINT: case _STRNG:
      return *this;
    case _CPLX:
	...
	if (_SYMBptr->feuille.type==_VECT && _SYMBptr->feuille._VECTptr->size()==2){
      ...
      if ( equalposcomp(analytic_sommets,_SYMBptr->sommet) || _SYMBptr->sommet==at_surd || _SYMBptr->sommet==at_erf)
	return (symbolic(_SYMBptr->sommet,_SYMBptr->feuille.conj(contextptr)));
      else
	return symb_conj(*this);
    case _FRAC:
  ...
  static void symb_reim(const symbolic & s,gen & r,gen & i,GIAC_CONTEXT){
	...
	if (nrealposroot>0){
	  ...
	  if (r.type==_VECT){
	    tmp[0]=r;
	    r=(symbolic(u,gen(tmp,f.subtype)));
	  }
	  i=algtrim(i);
	  if (i.type==_VECT){
	    tmp[0]=i;
	    i=(symbolic(u,gen(tmp,f.subtype)));
	  }
	  return;
	}
      }
    ...
    if (u==at_erf){ // works for analytic functions
      gen conjf=symbolic(u,gen(ref,-imf));//symbolic(u,ref-cst_i*imf);
      r=(s+conjf)/2;
      i=-cst_i*(s-conjf)/2;
      return;
    }
  ...
  void reim(const gen & g,gen & r,gen & i,GIAC_CONTEXT){
	...
	r=(symbolic(g._SYMBptr->sommet,r));
	i=(symbolic(g._SYMBptr->sommet,i));
      }
      else {
	if (expand_re_im(contextptr))
      ...
      break;
      // case _USER:r=g._USERptr->re(contextptr); i=g._USERptr->im(contextptr); break;
    case _FRAC:
  ...
  gen symbolic_re(const gen & a){
    return symbolic(at_re,a);
  }

  gen symbolic_im(const gen & a){
    return symbolic(at_im,a);
  }
  ...
  gen gen::re(GIAC_CONTEXT) const {
	...
	return *this;
      return (symbolic_re(*this));
    case _SYMB:
      if (expand_re_im(contextptr))
	return symb_re(*_SYMBptr,contextptr);
      else
	return (symbolic_re(*this));
      //case _USER: return _USERptr->re(contextptr);
    case _FRAC:
  ...
  gen gen::im(GIAC_CONTEXT) const {
	...
	return zero;
      if ( (_IDNTptr->value) && (is_zero(_IDNTptr->value->im(contextptr),contextptr)) )
	return zero;
      return (symbolic_im(*this));
    case _SYMB:      
      if (expand_re_im(contextptr))
	return symb_im(*_SYMBptr,contextptr); 
      else
	return (symbolic_im(*this));
      //case _USER: return _USERptr->im(contextptr);
    case _FRAC:
  ...
  static gen symb_abs(const symbolic & s,GIAC_CONTEXT){
	...
	return pow(abs(f._VECTptr->front(),contextptr),fback,contextptr);
      return (symbolic(u,makesequence(abs(f._VECTptr->front(),contextptr),f._VECTptr->back())));
    }
    if (u==at_inv)
      return inv(abs(f,contextptr),contextptr);
    if (u==at_prod)
      return (symbolic(u,apply(f,contextptr,abs)));
  ...
  gen abs(const gen & a,GIAC_CONTEXT){ 
	...
	return apply_to_equal(a,abs,contextptr);
      ...
      //case _USER: return a._USERptr->abs(contextptr);
    case _FRAC:
  ...
  gen arg_CPLX(const gen & a,GIAC_CONTEXT){
    ...
    if ( (realpart.type==_DOUBLE_ ) || (imagpart.type==_DOUBLE_) )
  ...
  gen arg(const gen & a,GIAC_CONTEXT){ 
    ...
    switch (a.type ) {
    case _INT_: case _ZINT: case _DOUBLE_: 
	...
	//case _USER:return a._USERptr->arg(contextptr);
    case _FRAC:
  ...
  gen operator_plus(const gen & a,const gen & b,unsigned t,GIAC_CONTEXT){
    ...
    case _CPLX__INT_: 
      if (b.val==0) return a;
    case _CPLX__ZINT: case _CPLX__DOUBLE_: 
      return gen(*a._CPLXptr+b,*(a._CPLXptr+1));
    case _INT___CPLX: 
      if (a.val==0) return b;
    case _ZINT__CPLX: case _DOUBLE___CPLX: 
    ...
    case _STRNG__STRNG:
      ...
      return string2gen((*a._STRNGptr)+(*b._STRNGptr),false);
	...
	return spadd(gen2spol1(a),*b._SPOL1ptr,contextptr);
      //if (a.type==_USER)return (*a._USERptr)+b;
      //if (b.type==_USER)return (*b._USERptr)+a;
    }
    return sym_add(a,b,contextptr);
  }
  ...
  gen sym_add(const gen & a,const gen & b,GIAC_CONTEXT){
	...
	*logptr(contextptr) << "Warning constant+function with constant dependant of mute variable" << endl;
	...
	return (symbolic(b._SYMBptr->sommet,makesequence(va.front()+vb.front(),va.back()+vb.back())));
      }
      if (ia==1 || !ib) // = + 
	return (symbolic(a._SYMBptr->sommet,makesequence(va.front()+b,va.back()+b)));
      if ( (ia==5 && ib==6) || (ia==6 && ib==5)){
	vecteur & vb=*b._SYMBptr->feuille._VECTptr;
	return symb_superieur_strict(va.front()+vb.front(),va.back()+vb.back());
      }
    }
    if (ib)
      return b+a;
	...
	if (b.is_symb_of_sommet(at_interval)){
	  ...
	  if (g.type==_VECT && g._VECTptr->size()==2){
	    ...
	    return symb_interval(w[0]+v[0],w[1]+v[1]);//(symbolic(at_interval,gen(makenewvecteur(w[0]+v[0],w[1]+v[1]),_SEQ__VECT)));
	  }
	}
	return symb_interval(b+v[0],b+v[1]);//(symbolic(at_interval,gen(makenewvecteur(b+v[0],b+v[1]),_SEQ__VECT)));
      }
    }
    if (b.is_symb_of_sommet(at_interval))
      return b+a;
	...
	if (v.size()==1)
	  ...
	  return (symb_plus(gen(v,a._SYMBptr->feuille.subtype)));
      }
      if (a._SYMBptr->sommet==at_plus)
	return (symbolic(*a._SYMBptr,b));
      else
  ...
  gen sym_sub(const gen & a,const gen & b,GIAC_CONTEXT){
	...
	var2=var1;
      ...
      if (var1==var2)
	return symb_prog3(var1,0,operator_minus(res1,res2,contextptr));
    }
    if (a.type==_VECT)
      return sym_add(a,-b,contextptr);
    if (b.type==_VECT)
	...
	vecteur & vb=*b._SYMBptr->feuille._VECTptr;
	return (symbolic(at_equal,makesequence(va.front()-vb.front(),va.back()-vb.back())));
      }
      else
	return (symbolic(at_equal,makesequence(va.front()-b,va.back()-b)));
    }
    if (b.type==_SYMB && b._SYMBptr->sommet==at_equal){
      vecteur & vb=*b._SYMBptr->feuille._VECTptr;
      return (symbolic(at_equal,makesequence(a-vb.front(),a-vb.back())));
    }
    */
    ...
    if ((a.type==_SYMB) && (b.type==_SYMB)){
      if (a._SYMBptr->sommet==at_plus) {
	if (b._SYMBptr->sommet==at_plus)
	  return symb_plus(mergevecteur(*(a._SYMBptr->feuille._VECTptr),negvecteur(*(b._SYMBptr->feuille._VECTptr))));
	else
	  return (symbolic(*a._SYMBptr,-b));
      }
      else { 
  ...
  gen operator -(const gen & a){
	...
	return a._SYMBptr->feuille;
      if (a._SYMBptr->sommet==at_plus)
	return (symb_plus(negvecteur(*a._SYMBptr->feuille._VECTptr)));
      if (a._SYMBptr->sommet==at_interval && a._SYMBptr->feuille.type==_VECT && a._SYMBptr->feuille._VECTptr->size()==2){
	return symb_interval(-a._SYMBptr->feuille._VECTptr->back(),-a._SYMBptr->feuille._VECTptr->front()); // (symbolic(at_interval,gen(makenewvecteur(-a._SYMBptr->feuille._VECTptr->back(),-a._SYMBptr->feuille._VECTptr->front()),_SEQ__VECT)));
      }
      if (a.is_symb_of_sommet(at_program)){
	gen a1,b;
	if (is_algebraic_program(a,a1,b))
	  return symb_prog3(a1,0,-b);
      }
      if (a._SYMBptr->sommet==at_equal || a._SYMBptr->sommet==at_equal2 || a._SYMBptr->sommet==at_different || a._SYMBptr->sommet==at_same)
	return (symbolic(a._SYMBptr->sommet,makesequence(-a._SYMBptr->feuille._VECTptr->front(),-a._SYMBptr->feuille._VECTptr->back())));
      if (is_inequality(a))
	return (symbolic(a._SYMBptr->sommet,makesequence(-a._SYMBptr->feuille._VECTptr->back(),-a._SYMBptr->feuille._VECTptr->front())));
      return symb_neg(a);
    case _VECT:
      if (a.subtype==_VECTOR__VECT && a._VECTptr->size()==2)
  ...
  static gen operator_times(const gen & a,const gen & b,unsigned t,GIAC_CONTEXT){
    ...
    case _VECT__ZINT: case _VECT__DOUBLE_: case _VECT__CPLX: case _VECT__SYMB: case _VECT__IDNT: case _VECT__POLY: case _VECT__EXT: case _VECT__MOD: case _VECT__FRAC:  {
      gen A(a),B(b);
  ...
  static gen minus1pow(const gen & exponent,GIAC_CONTEXT,bool allow_recursion=true){
	...
	if (num.type==_INT_ && den.type==_INT_ && den.val<=MAX_ALG_EXT_ORDER_SIZE){
	  return expi(exponent*cst_pi,contextptr);
	}
      }
    }
    return symb_pow(-1,exponent); //symb_pow(gen(makenewvecteur(-1,exponent),_SEQ__VECT)));
  }
  ...
  gen pow(const gen & base,const gen & exponent,GIAC_CONTEXT){
	...
	return symb_prog3(a,0,pow(b,exponent,contextptr));
	...
	if (v.size()==1 && v.front().is_symb_of_sommet(at_pow) && v.front()._SYMBptr->feuille[1]==plus_one_half && is_integer(v.front()._SYMBptr->feuille[0])){
      ...
      return pow(base,(symb_prod2(exponent._FRACptr->num,symb_inv(exponent._FRACptr->den))),contextptr);
    }
    if (is_inf(base)){ 
	...
	return (symbolic(base._SYMBptr->sommet,makesequence(pow(vb.front(),exponent,contextptr),pow(vb.back(),exponent,contextptr))));
      }
      if (exponent.type==_INT_){
	if (exponent.val==0)
	  return 1;
    ...
    // if (abs_calc_mode(contextptr)==38 && !complex_mode(contextptr) && is_exactly_zero(base) && is_exactly_zero(exponent)) return undef; // was is_zero(,.contextptr) changed so that MINREAL^0 is not undef but 1
    switch ( (base.type<< _DECALAGE) | exponent.type ) {
    case _INT___INT_: case _ZINT__INT_: case _CPLX__INT_: case _IDNT__INT_: 
	...
	if (exponent.type>=_IDNT)
	  *logptr(contextptr) << gettext("Unsupported array power") << endl;
	  ...
	  ||  ( (base.type<_POLY || base.type==_FLOAT_) && (exponent.type==_DOUBLE_ )))
	return exp(exponent*log(base,contextptr),contextptr);
      /* 
	...
	if (subexponent_deno.type!=_INT_){
	  if (is_one(superexponent))
	    return symb_pow(base,_FRAC2_SYMB(subexponent_num,subexponent_deno));
	  return symb_pow(symb_pow(base,_FRAC2_SYMB(subexponent_num,subexponent_deno)),superexponent);
	}
	...
	if (r){
	  if (complex_mode(contextptr) && fastsign(base,contextptr)==-1){ // is_strictly_positive(-base,contextptr)){
	    gen base1=-base;
	    res=exp((cst_ipi()*subexponent_num)/subexponent_deno,contextptr);
	    res=symb_pow(pow(base1,subexponent_num,contextptr),inv(subexponent_deno,contextptr))*res;
	  }
	  else
	    res=symb_pow(pow(base,subexponent_num,contextptr),inv(subexponent_deno,contextptr));
	  if (r!=1)
	    res=symb_pow(res,r);
	}
	if (!q)
	...
	if (q==1){
	  if (is_one(subexponent_num))
	    return res*base;
	  return res*symb_pow(base,subexponent_num);
	}
	if (q==-1)
	  return res*inv(pow(base,subexponent_num,contextptr),contextptr);
	return res*symb_pow(pow(base,subexponent_num,contextptr),q);
      }
      gen var1,var2,res1,res2;
	...
	if (var1==var2)
	  return symb_prog3(var1,0,pow(res1,res2,contextptr));
      }
      if (exponent.type==_ZINT){
	//if (base.type==_USER)return pow_iterative(base,exponent,contextptr);
	return exp(exponent*log(base,contextptr),contextptr);
      }
      return symb_pow(base,exponent);
    }  
  ...
  gen sym_mult(const gen & a,const gen & b,GIAC_CONTEXT){
    ...
    if (is_inequality(a) && !is_equal(a)){
      ...
      if (bs==-1)
	return (symbolic(a._SYMBptr->sommet,makesequence(a._SYMBptr->feuille._VECTptr->back()*b,a._SYMBptr->feuille._VECTptr->front()*b)));
      if (bs==1)
    ...
    if (is_inequality(b) && !is_equal(b)){
      ...
      if (bs==-1)
	return (symbolic(b._SYMBptr->sommet,makesequence(a*b._SYMBptr->feuille._VECTptr->back(),a*b._SYMBptr->feuille._VECTptr->front())));
      if (bs==1)
	return (symbolic(b._SYMBptr->sommet,makesequence(a*b._SYMBptr->feuille._VECTptr->front(),a*b._SYMBptr->feuille._VECTptr->back())));
    }
    gen var1,var2,res1,res2;
    if (is_algebraic_program(a,var1,res1)){
      if (is_algebraic_program(b,var2,res2)){
	...
	*logptr(contextptr) << "Warning constant*function with constant dependant of mute variable" << endl;
    ...
    if ((a.type<=_REAL ) && is_strictly_positive(-a,contextptr))
      return -sym_mult(-a,b,contextptr);
    if ((b.type<=_REAL) && is_strictly_positive(-b,contextptr))
      return -sym_mult(a,-b,contextptr);
    if (a.type==_EXT){
        if (a.is_constant() && (b.type==_POLY))
            return a*(*b._POLYptr);
    ...
    if (is_equal(a)){
      ...
      if (is_equal(b)){
	vecteur & vb=*b._SYMBptr->feuille._VECTptr;
	return (symbolic(a._SYMBptr->sommet,gen(makenewvecteur(va.front()*vb.front(),va.back()*vb.back()),_SEQ__VECT)));
      }
      else
	return (symbolic(a._SYMBptr->sommet,gen(makenewvecteur(va.front()*b,va.back()*b),_SEQ__VECT)));
    }
    if (is_equal(b)){
      vecteur & vb=*b._SYMBptr->feuille._VECTptr;
      return (symbolic(b._SYMBptr->sommet,gen(makenewvecteur(a*vb.front(),a*vb.back()),_SEQ__VECT)));
    }
    if ((a.type==_SYMB)&& (b.type==_SYMB)){
      if ((a._SYMBptr->sommet==at_prod) && (b._SYMBptr->sommet==at_prod))
	return symb_prod(mergevecteur(*(a._SYMBptr->feuille._VECTptr),*(b._SYMBptr->feuille._VECTptr)));
      else {
	...
	return b.type==_INT_?symb_prod2(b,a):symb_prod2(a,b);
    }
    if ((a.type==_IDNT) || (b.type==_IDNT))
      return symb_prod2(a,b);
    if (a.type==_MOD)
  ...
  gen inv_distrib(const gen & b,GIAC_CONTEXT){
    if (b.is_symb_of_sommet(at_prod)){
      ...
      return symb_prod(inv_distrib(f,contextptr));
    }
  ...
  gen inv(const gen & a,GIAC_CONTEXT){
	...
	if (is_inf(a2)){
	  ...
	  theta=gen(cos(theta,contextptr),-sin(theta,contextptr));//cos(theta,contextptr)-cst_i*sin(theta,contextptr);
	...
	return undef;
      if (a==unsigned_inf)
	return 0;
      return symb_inv(a); //(symbolic(at_inv,a));
    case _SYMB:
	...
	if (a._SYMBptr->sommet==at_prod)
	  return (symb_prod(gen(inv__VECT(*(a._SYMBptr->feuille._VECTptr),contextptr),a._SYMBptr->feuille.subtype)));
	else
	  return symb_inv(a); // (symbolic(at_inv,a));
      }
    case _VECT:
    ...
    case _SPOL1:
      ...
      //case _USER:return a._USERptr->inv();
    case _MOD:
      return modinv(a);
    case _FRAC:
  ...
  */
  ...
  gen pow(const gen & base, unsigned long int exponent){
    ...
    case _CPLX: case _EXT: case _MOD: 
      // gauss integer power
      if (!exponent){
	if (ckmatrix(base))
	...
	return 1;
      if (exponent==1)
	return base;
      return symb_pow(base,(longlong)exponent);
    case _SYMB:
      if (!exponent)
	return 1;
      if (exponent==1)
	...
	return symb_pow(base._SYMBptr->feuille,(longlong) exponent);
      return symb_pow(base,(longlong) exponent);
    case _POLY:
...
#endif
    // cout << "pow1 " << int(base) << " " << int(exponent) << "\n";
    mpz_ui_pow_ui(e->z,base,exponent);
    // cout << "pow2 " << int(base) << " " << int(exponent) << "\n";
    return e;
  }
  ...
  gen rdiv(const gen &a,const gen &b,GIAC_CONTEXT){
    ...
    case _DOUBLE___CPLX: case _INT___CPLX: case _ZINT__CPLX: 
      if (is_one(a))
	return inv(b,contextptr);
    ...
    case _VECT__INT_: case _VECT__ZINT: case _VECT__DOUBLE_: case _VECT__CPLX: case _VECT__FRAC:
	...
	*logptr(contextptr) << gettext("Unsupported array division") << endl;
	...
	return (-b._FRACptr->den*a)/(-b._FRACptr->num);
      ...
      // if (is_one(a)) return fraction(b._FRACptr->den,b._FRACptr->num);
      return (b._FRACptr->den*a)/b._FRACptr->num;
    case _INT___VECT: case _ZINT__VECT: case _CPLX__VECT: case _DOUBLE___VECT:  case _SYMB__VECT:
	...
	return spdiv(gen2spol1(a),*b._SPOL1ptr,contextptr);      
      //if (a.type==_USER && b.type!=_USER)return (*a._USERptr)/b;
      //if (a.type==_USER || b.type==_USER) return a*inv(b,contextptr);
      if (a.type==_FRAC){
	if ( (b.type!=_SYMB) && (b.type!=_IDNT) )
	  return (*a._FRACptr)/b;
	...
	// return symb_prod(makesequence(a,b._FRACptr->den,symbolic(at_inv,b._FRACptr->num)));
	return (b._FRACptr->den*a)/b._FRACptr->num;
      }
      if (is_equal(a)){
	vecteur & va=*a._SYMBptr->feuille._VECTptr;
	if (is_equal(b)){
	  vecteur & vb=*b._SYMBptr->feuille._VECTptr;
	  return (symbolic(a._SYMBptr->sommet,makesequence(rdiv(va.front(),vb.front(),contextptr),rdiv(va.back(),vb.back(),contextptr))));
	}
	else
	  return (symbolic(a._SYMBptr->sommet,makesequence(rdiv(va.front(),b,contextptr),rdiv(va.back(),b,contextptr))));
      }
      if (is_equal(b)){
	vecteur & vb=*b._SYMBptr->feuille._VECTptr;
	return (symbolic(b._SYMBptr->sommet,makesequence(rdiv(a,vb.front(),contextptr),rdiv(a,vb.back(),contextptr))));
      }
  ...
  bool operator_equal(const gen & a,const gen & b,GIAC_CONTEXT){
    ...
    case _DOUBLE___FRAC: case _FRAC__DOUBLE_: 
	...
	return is_zero(a-b,contextptr);
      if ( (a.type==_DOUBLE_ ) && (b.type<=_REAL || b.type==_FRAC ))
	return is_zero(a-evalf(b,1,contextptr));
      if ( (b.type==_DOUBLE_ ) && (a.type<=_REAL || a.type==_FRAC ))
  ...
  gen equal(const gen & a,const gen &b,GIAC_CONTEXT){
    ...
    if (calc_mode(contextptr)==1 && a.type==_IDNT && b.type==_VECT && b.subtype!=_SEQ__VECT){
      vecteur v=*b._VECTptr;
      for (unsigned i=0;i<v.size();++i){
	v[i]=symb_equal(a,v[i]); // symbolic(at_equal,makesequence(a,v[i]));
      }
      return gen(v,b.subtype);
    }
#endif
    gen res=symb_equal(a,b); // symbolic(at_equal,makesequence(a,b));
    return res;
  }
  ...
  bool is_zero(const gen & a,GIAC_CONTEXT){
    ...
    case _MOD:
      ...
      //case _USER:return a._USERptr->is_zero();
    default: 
      return false;
    }
  }
  ...
  bool is_one(const gen & a){
    ...
    case _MOD:
      ...
      //case _USER:return a._USERptr->is_one();
    default: 
      return false;
    }
  }
  ...
  bool is_minus_one(const gen & a){
    ...
    case _SYMB:
      ...
      //case _USER:return a._USERptr->is_minus_one();
    default: 
      return false;
    }
  }
  ...
  gen gen::operator_at(int i,GIAC_CONTEXT) const{
	...
	gen g=symb_program(tmp,zero,(symbolic(at_makesuite,i,tmp)),contextptr);
	g=makesequence(at_maple_root,g);
	return symb_compose(g);
      }
    }
    if (this->type!=_VECT){
      // if (calc_mode(contextptr)==1) return *this;
  ...
  gen gen::operator_at(const gen & i,GIAC_CONTEXT) const {
    ...
    if ((type==_STRNG) && (i.type==_INT_)){
      ...
      if (I<0) I+=s;
      if ( (I<s) && (I>=0))
	return string2gen(_STRNGptr->substr(I,1),false);
    }
    if (type==_IDNT)
      return (symbolic(at_at,gen(makenewvecteur(*this,i),_SEQ__VECT)));
    //if (type==_USER)return (*_USERptr)[i];
    if (type==_MAP){
  ...
  */
  ...
  gen gen::operator () (const gen & i,const gen & progname,const context * contextptr) const{
	...
	return symb_of(*this,i); //symbolic(at_of,makesequence(*this,i)));
	...
	*logptr(contextptr) << gettext("Warning, evaluating (") << *this << ")(" << i << ") as a function not as a product" << endl;
    ...
    if (type<_IDNT )
      return *this;
    //if (type==_USER)return (*_USERptr)(i,contextptr);
    if (type==_STRNG){
      gen ii(i);
      if (!is_integral(ii))
  ...
  gen operator && (const gen & a,const gen & b){
    ...
    if (a.type<=_CPLX || a.type==_FRAC){
      if (b.type<=_CPLX || b.type==_FRAC)
	return change_subtype(!is_zero(b),_INT_BOOLEAN);
      return b;
    }
    if (b.type<=_CPLX || b.type==_FRAC){
	...
	return symb_and(mergevecteur(*a._SYMBptr->feuille._VECTptr,*b._SYMBptr->feuille._VECTptr));
      vecteur v=*a._SYMBptr->feuille._VECTptr;
      v.push_back(b);
      return symb_and(v);
    }
  ...
  gen operator || (const gen & a,const gen & b){
	...
	return symb_ou(mergevecteur(*a._SYMBptr->feuille._VECTptr,*b._SYMBptr->feuille._VECTptr));
      vecteur v=*a._SYMBptr->feuille._VECTptr;
      v.push_back(b);
      return symb_ou(v);
    }
  ...
  static gen regroup_inv(const vecteur & vtmp){
	...
	tt=(symb_prod(gen(vtmp1,_SORTED__VECT)));
    }
    if (!vtmp2.empty()){
      if (vtmp2.size()==1)
	tt=tt/vtmp2.front();
      else
	tt=tt/(symb_prod(gen(vtmp2,_SORTED__VECT)));
    }
    return tt;
  }
  ...
  static vecteur terme2unitaire(const gen & x,bool sorted,GIAC_CONTEXT){
	...
	if (vtmp[i].is_symb_of_sommet(at_inv) && vtmp[i]._SYMBptr->feuille.is_symb_of_sommet(at_pow)){
	  ...
	  if (f.type==_VECT && f._VECTptr->size()==2)
	    vtmp[i]=symb_pow(f._VECTptr->front(),-f._VECTptr->back());
	}
      }
	...
	if (!is_zero(precexpo,contextptr)){
	  if (is_strictly_positive(-precexpo,contextptr)){
	    ...
	    if (tmp.is_symb_of_sommet(at_prod))
	      tmp=symb_inv(tmp); // symbolic(at_inv,tmp);
	    else
    ...
    return makevecteur(plus_one,(symbolic(x._SYMBptr->sommet,tmp)));
  }
  ...
  */
  ...
  gen liste2symbolique(const vecteur & v){
    ...
    if (s==1)
      return res.front();
    return (symb_plus(gen(res,_SORTED__VECT)));
  }
  ...
  */
  ...
  static gen symgcd(const gen & a,const gen& b,GIAC_CONTEXT){
    ...
    if ( (a.type==_MOD) && (b.type==_MOD) && (a._MODptr->type<=_CPLX) && (b._MODptr->type<= _CPLX) )
      return chkmod(plus_one,a);
    if (a.type==_MOD || b.type==_MOD || a.type==_DOUBLE_  || b.type==_DOUBLE_ )
      return plus_one;
  ...
  gen simplify(gen & n, gen & d){
    if ( (d.type==_DOUBLE_ ) || 
	 ( (d.type==_CPLX) && 
	   ((d._CPLXptr->type==_DOUBLE_ ) || 
	    ((d._CPLXptr+1)->type==_DOUBLE_ )) )
	 ){
    ...
    if ( (n.type==_DOUBLE_ ) ||
	 ( (n.type==_CPLX) && 
	   ((n._CPLXptr->type==_DOUBLE_ ) || 
	    ((n._CPLXptr+1)->type==_DOUBLE_ )) )
	 ){
      gen nn=no_context_evalf(n);
      n=plus_one;
  ...
  gen gcd(const gen & a,const gen & b,GIAC_CONTEXT){
	...
	return fraction(gcd(b._FRACptr->num,a,contextptr),b._FRACptr->den);
      //if (a.type==_USER)return a._USERptr->gcd(b);
      //if (b.type==_USER)return b._USERptr->gcd(a);
      {
	gen aa(a),bb(b);
  ...
  gen invmod(const gen & a,const gen & modulo){
    //if (a.type==_USER)return a._USERptr->inv();
    if (a.type==_MOD){
    ...
    if (a.type==_CPLX){
      ...
      return gen(smod(r*n,modulo),-smod(i*n,modulo));//smod(r*n,modulo)-cst_i*smod(i*n,modulo);
    }
    if (a.type==_POLY)
      return fraction(1,a);
    ref_mpz_t * res;
  ...
  bool alloc_fracmod(const gen & a_orig,const gen & modulo,gen & res,mpz_t & d,mpz_t & d1,mpz_t & absd1,mpz_t &u,mpz_t & u1,mpz_t & ur,mpz_t & q,mpz_t & r,mpz_t &sqrtm,mpz_t & tmp){  
	...
	return false;
      res=gen(reres,imres);//reres+cst_i*imres;
      return true;
    }
  ...
  bool has_denominator(const gen & n){
    switch (n.type ) {
    case _INT_: case _ZINT: case _CPLX: case _DOUBLE_: case _IDNT: case _EXT: case _POLY: case _MOD:  case _VECT:
      return false;
...
#else
    string tmp(ss.begin()+(remove_ss_quotes?1:0),ss.end()-(remove_ss_quotes?1:0));
    res.__STRNGptr = new ref_string(tmp);
#endif
    res.type=_STRNG;
    return res;
  }
  ...
  gen aplatir_fois_plus(const gen & g){
	...
	*it=aplatir_fois_plus(*it);
    ...
    if (g._SYMBptr->sommet==at_pow && g._SYMBptr->feuille.type==_VECT && g._SYMBptr->feuille.subtype!=_SEQ__VECT)
      return symb_pow(change_subtype(g._SYMBptr->feuille,_SEQ__VECT));
    ...
    return (symbolic(g._SYMBptr->sommet,aplatir_fois_plus(f)));
  }
  ...
  static gen aplatir_plus_only(const gen & g){
	...
	*jt=aplatir_plus_only(*it);
	...
	it=f->_VECTptr->begin();
      ...
      reverse(v.begin(),v.end());
      return gen((symb_plus(v))).change_subtype(g.subtype);
    }
    return gen((symbolic(g._SYMBptr->sommet,aplatir_plus_only(g._SYMBptr->feuille)))).change_subtype(g.subtype);
  }
  ...
  */
  ...
  string print_DOUBLE_(double d,GIAC_CONTEXT){
    ...
    *u &= 0xe0;
	...
	ch='g';
      // if (calc_mode(contextptr)==1) ch=toupper(ch);
    ...
    // if (calc_mode(contextptr)==1) form[form.size()-1]=toupper(form[form.size()-1]);
    if (sf==2){
      // engineering format
  ...
  string begin_VECT_string(int subtype,bool tex,GIAC_CONTEXT){
	...
	s="list["; // abs_calc_mode(contextptr)==38?"{":"list[";
      break;
    case _GGB__VECT:
      if (calc_mode(contextptr)==1)
  ...
  static string print_FRAC(const gen & f,GIAC_CONTEXT){
    if (f._FRACptr->num.type==_INT_ && f._FRACptr->den.type==_INT_){
      ...
      s += "/";
      s += f._FRACptr->den.print(contextptr);
  ...
  string gen::print(GIAC_CONTEXT) const{
    ...
    case _ZINT:
      // if (abs_calc_mode(contextptr)==38 && is_strictly_positive(-*this,contextptr)) return "−"+(-*this).print(contextptr);
      switch (integer_format(contextptr)){
      case 16:
	...
	if (is_minus_one(*(_CPLXptr+1)))
	  return string("-")+printi(contextptr);
	...
	if (_SYMBptr->sommet==at_plus){
      // if (abs_calc_mode(contextptr)==38) return "∞"; else
	    return "+infinity";
	}
    ...
    case _EXT: {
      ...
      //case _USER:return _USERptr->print(contextptr);
    case _MOD:
#ifdef GIAC_HAS_STO_38
  ...
  */
  ...
  const char * caseval(const char *s){
    ...
    // cout << "caseval " <<  history_plot(contextptr).size() << "\n";
