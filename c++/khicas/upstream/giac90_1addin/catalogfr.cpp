...
const catalogFunc completeCat[] = { // list of all functions (including some not in any category)
  ...
  {" edit matrix ", "matrix ", "Assistant creation de matrice.", 0, 0, CAT_CATEGORY_MATRIX |  (CAT_CATEGORY_LINALG<<8)},
  ...
  {"=>", "=>", "Affectation vers la droite ou conversion en (touche ->). Exemple 5=>a ou x^4-1=>* ou (x+1)^2=>+ ou sin(x)^2=>cos.", "#5=>a", 0, CAT_CATEGORY_PROGCMD},
...
#endif
  {"plotparam([x,y],t)", 0, "Graphe en parametriques. Exemple plotparam([sin(3t),cos(2t)],t,0,pi) ou plotparam(exp(i*t),t,0,pi)", "[sin(3t),cos(2t)],t,0,pi", "[t^2,t^3],t=-1..1,tstep=0.1", CAT_CATEGORY_PLOT},
  ...
  {"ranm(n,m,[loi,parametres])", 0, "Matrice aleatoire a coefficients entiers ou selon une loi de probabilites (ranv pour un vecteur). Exemples ranm(2,3), ranm(3,2,binomial,20,.3)", "3,3,10", 0, CAT_CATEGORY_MATRIX},
  {"ranv(n,[loi,parametres])", 0, "Vecteur aleatoire", "10,30", 0, CAT_CATEGORY_LINALG},
