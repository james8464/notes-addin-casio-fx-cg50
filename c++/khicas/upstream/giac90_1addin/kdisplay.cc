...
namespace xcas {
  ...
  Graph2d::Graph2d(const giac::gen & g_):window_xmin(gnuplot_xmin),window_xmax(gnuplot_xmax),window_ymin(gnuplot_ymin),window_ymax(gnuplot_ymax),g(g_),display_mode(0x45),show_axes(1),show_names(1),labelsize(16) {
    update();
    autoscale();
  }
  ...
  void Graph2d::autoscale(bool fullview){
    ...
    // *logptr(contextptr) << window_xmin << " " << window_xmax << " " << window_ymin << " " << window_ymax << endl; window_xmin=gnuplot_xmin; window_xmax=gnuplot_xmax; window_ymin=gnuplot_ymin; window_ymax=gnuplot_ymax;
    bool do_ortho=ortho;
    if (!do_ortho){
  ...
  bool Graph2d::findij(const gen & e0,double x_scale,double y_scale,double & i0,double & j0,GIAC_CONTEXT) const {
    //*logptr(contextptr) << "findij " << e0 << endl;
    gen e,f0,f1;
  ...
  void Graph2d::draw(){
	...
	fl_line(delta,deltay+j_0,delta,deltay+j_0-4,COLOR_GREEN);
      }
	...
	if (delta>=taille && delta<=vertical_pixels-taille){
	  fl_line(deltax+i_0,STATUS_AREA_PX+delta,deltax+i_0+4,STATUS_AREA_PX+delta,COLOR_RED);
	}
      }
	...
	taille=strlen(ch)*9;
	fl_line(delta,vertical_pixels+STATUS_AREA_PX-6,delta,vertical_pixels+STATUS_AREA_PX-1,COLOR_GREEN);
	if (delta>=taille/2 && delta<=horizontal_pixels){
	  text_print(10,ch,delta-taille/2,vertical_pixels+STATUS_AREA_PX-7,COLOR_GREEN);
	}
      }
      // Y
      aff=ticks(window_ymin,window_ymax,true);
	...
	if (delta>=taille && delta<=vertical_pixels-taille){
	  fl_line(horizontal_pixels-5,STATUS_AREA_PX+delta,horizontal_pixels-1,STATUS_AREA_PX+delta,COLOR_RED);
	  text_print(10,ch,horizontal_pixels-strlen(ch)*9,STATUS_AREA_PX+delta+taille,COLOR_RED);
	}
      }
    }
