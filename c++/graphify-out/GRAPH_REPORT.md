...
## Corpus Check
- 105 files · ~404,276 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2815 nodes · 11771 edges · 40 communities detected
...
- `run()` --calls--> `numeric_eval()`  [EXTRACTED]
  python/src/calc_files/trig.py → c++/addin/src/modules/trig/trig.cpp
- `run()` --calls--> `split_csv()`  [EXTRACTED]
  python/src/calc_files/trig.py → c++/addin/src/modules/trig/trig.cpp
- `run()` --calls--> `split_csv()`  [EXTRACTED]
  python/src/calc_files/derive.py → c++/addin/src/modules/derive/derive.cpp
- `run()` --calls--> `collect_domain()`  [EXTRACTED]
  python/src/calc_files/algebra.py → c++/addin/src/modules/algebra/algebra.cpp
...
Cohesion: 0.03
Nodes (138): add_poly(), as_int64(), collect_domain(), is_square_i64(), is_zero(), mul_poly(), poly_of(), r_add() (+130 more)
...
Cohesion: 0.05
Nodes (70): FixedString, is_alpha(), lower_ascii(), LineBuffer, add_prompt_line(), add_shell_line(), clamp_top(), count_of() (+62 more)
...
Cohesion: 0.14
Nodes (38): abs_int(), add_input_line(), append_fraction(), append_fraction_poly(), append_fraction_value(), append_linear(), append_poly(), append_power() (+30 more)
...
Cohesion: 1.0
Nodes (3): draw_frame(), map_key_to_char(), text_input()
...
## Knowledge Gaps
- **190 isolated node(s):** `Convert the python program stdin payload to our host CLI input.     Only handles`, `Arena`, `LineBuffer`, `Quick numeric check: evaluate eq_expr at expected_x.     eq_expr should be an ex`, `Returns (script_relpath, stdin_payload) for program invocation.` (+185 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 36`** (2 nodes): `check_g3a_size.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 37`** (2 nodes): `check_regressions.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (2 nodes): `promote_failures.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 39`** (2 nodes): `summarize_failures.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `CASIOApp` connect `Community 2` to `Community 9`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `TransformRegressionTests` connect `Community 9` to `Community 2`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `LLMManager` connect `Community 2` to `Community 9`?**
...
- **What connects `Convert the python program stdin payload to our host CLI input.     Only handles`, `Arena`, `LineBuffer` to the rest of the system?**
