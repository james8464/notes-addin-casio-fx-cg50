...
static Rational qsub(Rational a, Rational b)
{
    Rational r{a.num * b.den - b.num * a.den, a.den * b.den};
    r.normalize();
    return r;
...
{
    if(n < 0) return false;
    r = static_cast<long long>(std::sqrt(static_cast<double>(n)) + 0.5);
    return r * r == n;
}

static NodeId sqrt_rational_exact(Arena &a, Rational r)
{
    r.normalize();
    if(r.num < 0) return a.fn(FnKind::Sqrt, a.num(r));
    long long rn = 0, rd = 0;
    if(square_ll(r.num, rn) && square_ll(r.den, rd) && rd != 0)
        return casio::num(a, rn, rd);
    return casio::simplify(a, casio::div(a, sqrt_int(a, r.num), casio::num(a, r.den)));
}

static std::optional<ExactSC> exact_angle_sincos(Arena &a, NodeId n);

static std::optional<ExactSC> inverse_angle_sincos(Arena &a, NodeId n)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Fn) return std::nullopt;
    auto q = as_num(a, x.a);
    ...
    q->normalize();
    NodeId s = 0, c = 0;
    if(x.fkind == FnKind::Asin || x.fkind == FnKind::Acos) {
        Rational one{1, 1};
        Rational q2 = qmul(*q, *q);
        Rational rest = qsub(one, q2);
        if(rest.num < 0) return std::nullopt;
        NodeId other = sqrt_rational_exact(a, rest);
        if(x.fkind == FnKind::Asin) {
            s = casio::num(a, q->num, q->den);
            c = other;
        }
        else {
            c = casio::num(a, q->num, q->den);
            s = other;
        }
        return ExactSC{casio::simplify(a, s), casio::simplify(a, c)};
    }
    if(x.fkind == FnKind::Atan) {
        long long hyp2 = q->num * q->num + q->den * q->den;
        long long h = 0;
        NodeId hyp = square_ll(hyp2, h) ? casio::num(a, h) : sqrt_int(a, hyp2);
        s = casio::simplify(a, casio::div(a, casio::num(a, q->num), hyp));
        c = casio::simplify(a, casio::div(a, casio::num(a, q->den), hyp));
        return ExactSC{s, c};
    }
    return std::nullopt;
}

static std::optional<NodeId> neg_angle_inner(Arena &a, NodeId n)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Mul) return std::nullopt;
    Rational coef{1, 1};
    std::vector<NodeId> rest;
    for(NodeId k : x.kids) {
        if(auto q = as_num(a, k)) coef = qmul(coef, *q);
        else rest.push_back(k);
    }
    if(!(coef.num == -coef.den && rest.size() == 1)) return std::nullopt;
    return rest[0];
}
...
{
    Node const &x = a.get(n);
    if(auto inv = inverse_angle_sincos(a, n)) return inv;
    if(x.kind == NodeKind::Num && x.num.num == 0) return ExactSC{casio::num(a, 0), casio::num(a, 1)};
    if(x.kind == NodeKind::Const && x.ckind == ConstKind::Pi) return ExactSC{casio::num(a, 0), casio::num(a, -1)};
    if(auto inner = neg_angle_inner(a, n)) {
        auto sc = exact_angle_sincos(a, *inner);
    ...
    if(x.kind == NodeKind::Add) {
        ExactSC acc{casio::num(a, 0), casio::num(a, 1)};
        for(NodeId k : x.kids) {
            auto t = exact_angle_sincos(a, k);
            if(!t) return std::nullopt;
            NodeId ns = casio::simplify(a, casio::add(a, {
                casio::mul(a, {acc.s, t->c}),
                casio::mul(a, {acc.c, t->s})
            }));
            NodeId nc = casio::simplify(a, casio::add(a, {
                casio::mul(a, {acc.c, t->c}),
                casio::neg(a, casio::mul(a, {acc.s, t->s}))
            }));
            acc = ExactSC{ns, nc};
        }
        return acc;
...
{
    Node const &x = a.get(n);
    if(x.kind == NodeKind::Fn && (x.fkind == FnKind::Asin || x.fkind == FnKind::Acos || x.fkind == FnKind::Atan))
        return true;
    if(x.kind == NodeKind::Fn) return contains_inverse_trig(a, x.a);
    if(x.kind == NodeKind::Pow || x.kind == NodeKind::Div) return contains_inverse_trig(a, x.a) || contains_inverse_trig(a, x.b);
    if(x.kind == NodeKind::Add || x.kind == NodeKind::Mul) {
        for(NodeId k : x.kids) if(contains_inverse_trig(a, k)) return true;
    }
    return false;
}

static std::optional<std::vector<std::string>> inverse_trig_compound_exact_route(Arena &a, NodeId n)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Fn || !(x.fkind == FnKind::Sin || x.fkind == FnKind::Cos || x.fkind == FnKind::Tan))
        return std::nullopt;
    if(!contains_inverse_trig(a, x.a)) return std::nullopt;
    auto sc = exact_angle_sincos(a, x.a);
    if(!sc) return std::nullopt;
    NodeId ans = 0;
    if(x.fkind == FnKind::Sin) ans = sc->s;
    else if(x.fkind == FnKind::Cos) ans = sc->c;
    else ans = casio::simplify(a, casio::div(a, sc->s, sc->c));
    ans = casio::simplify(a, ans);
    std::string U = format_expr(a, x.a);
    std::string final = format_expr(a, ans);
    return std::vector<std::string>{
        "U = " + U,
        "sin(U) = " + format_expr(a, sc->s) + ", cos(U) = " + format_expr(a, sc->c) + ".",
        trig_name(x.fkind) + "(U) = " + final,
        final,
    };
}
...
static std::optional<std::vector<std::string>> solve_sin_sum_to_cos_factor(
    Arena &a,
    NodeId residual,
    std::string const &var,
    std::string const &lo_text,
...
)
{
    ...
    std::vector<T> sins;
    std::optional<T> cos_term;
    for(NodeId k : r.kids) {
        auto t = term(k);
        if(!t) return std::nullopt;
        if(t->fk == FnKind::Sin) sins.push_back(*t);
        else {
            if(cos_term) return std::nullopt;
            cos_term = *t;
        }
    }
    if(sins.size() != 2 || !cos_term) return std::nullopt;
    if(std::fabs(sins[0].c - sins[1].c) > 1e-10 || std::fabs(sins[0].c) < 1e-12) return std::nullopt;
    auto A = linear_angle(a, sins[0].arg, var, rad);
    auto B = linear_angle(a, sins[1].arg, var, rad);
    auto C = linear_angle(a, cos_term->arg, var, rad);
    if(!A || !B || !C) return std::nullopt;
    double dm = (A->first - B->first) / 2.0, db = (A->second - B->second) / 2.0;
    bool same_d = std::fabs(C->first - dm) < 1e-12 && std::fabs(C->second - db) < 1e-9;
    bool neg_d = std::fabs(C->first + dm) < 1e-12 && std::fabs(C->second + db) < 1e-9;
    if(!same_d && !neg_d) return std::nullopt;
    NodeId Sarg = casio::simplify(a, casio::div(a, casio::add(a, {sins[0].arg, sins[1].arg}), casio::num(a, 2)));
    NodeId Darg = cos_term->arg;
    double target = -cos_term->c / (2.0 * sins[0].c);
    std::string Atext = format_expr(a, sins[0].arg);
    std::string Btext = format_expr(a, sins[1].arg);
    std::string Stext = format_expr(a, Sarg);
    std::string Dtext = format_expr(a, Darg);
    std::string ktxt = trig_root_text(2.0 * sins[0].c);
    std::string ctxt = trig_root_text(cos_term->c);
    std::string target_s = trig_root_text(target);
    std::vector<std::string> steps{
        "sin(A)+sin(B)=2*sin((A+B)/2)*cos((A-B)/2)",
        "A=" + Atext + ", B=" + Btext,
        ktxt + "*sin(" + Stext + ")*cos(" + Dtext + ")" + (cos_term->c >= 0 ? "+" : "") + ctxt + "*cos(" + Dtext + ")=0",
        "cos(" + Dtext + ")*(" + ktxt + "*sin(" + Stext + ")" + (cos_term->c >= 0 ? "+" : "") + ctxt + ")=0",
        "cos(" + Dtext + ")=0 or sin(" + Stext + ")=" + target_s + ".",
    };
    auto family = [&](NodeId arg, FnKind fk, double val, double root_period) -> std::string {
        auto lin = linear_angle(a, arg, var, rad);
        if(!lin || std::fabs(lin->first) < 1e-12) return "";
        auto bases = base_trig_degrees(fk, val);
        if(bases.empty()) return "";
        double period = root_period / std::fabs(lin->first);
        std::vector<double> xbases;
    ...
    std::string f1 = family(Darg, FnKind::Cos, 0.0, 180.0);
    std::string f2 = (target >= -1.0 - 1e-10 && target <= 1.0 + 1e-10) ? family(Sarg, FnKind::Sin, target, 360.0) : "";
    if(general) {
        if(f2.empty()) steps.push_back("Reject sin(" + Stext + ")=" + target_s + ": outside [-1,1].");
        else steps.push_back(f2 + ".");
        steps.push_back(f1 + ".");
        std::string ans = f1;
        if(!f2.empty()) ans += " or " + f2;
    ...
    std::vector<double> xs = x_values_from_angle_degrees(a, Darg, var, lo_text, hi_text, rad, base_trig_degrees(FnKind::Cos, 0.0));
    if(target >= -1.0 - 1e-10 && target <= 1.0 + 1e-10) {
        auto ys = x_values_from_angle_degrees(a, Sarg, var, lo_text, hi_text, rad, base_trig_degrees(FnKind::Sin, target));
        for(double y : ys) add_unique(xs, y);
    }
    else steps.push_back("Reject sin(" + Stext + ")=" + target_s + ": outside [-1,1].");
    std::sort(xs.begin(), xs.end());
    steps.push_back(interval_text(angle_bounds(a, lo_text, hi_text, rad), var) + " => " + format_solution_list(var, rad, xs) + ".");
    return casio::exam_block("trig solve", steps, format_solution_list(var, rad, xs));
}
...
static std::string trig_root_text(double r)
{
    ...
    double tan225 = sqrt2 - 1.0;
    double tan675 = sqrt2 + 1.0;
    double rt3p1 = sqrt3 + 1.0;
    double rt3m1 = sqrt3 - 1.0;
    ...
    if(std::fabs(r + tan15) < 1e-9) return "-2+sqrt(3)";
    if(std::fabs(r - rt3p1) < 1e-9) return "sqrt(3)+1";
    if(std::fabs(r + rt3p1) < 1e-9) return "-sqrt(3)-1";
    if(std::fabs(r - rt3m1) < 1e-9) return "sqrt(3)-1";
    if(std::fabs(r + rt3m1) < 1e-9) return "1-sqrt(3)";
    if(std::fabs(r - tan675) < 1e-9) return "1+sqrt(2)";
    if(std::fabs(r + tan675) < 1e-9) return "-1-sqrt(2)";
    if(std::fabs(r - tan225) < 1e-9) return "sqrt(2)-1";
    ...
    for(int rad : {2, 3, 5, 6, 7, 10, 19}) {
        ...
        for(int den = 1; den <= 24; ++den) {
            for(int num = 1; num <= 48; ++num) {
                double v = (double)num * root / (double)den;
                if(std::fabs(std::fabs(r) - v) > 1e-9) continue;
                int g = std::gcd(num, den);
                int n = num / g;
                int d = den / g;
                std::string s = (r < 0 ? "-" : "");
                if(n != 1) s += std::to_string(n) + "*";
                s += "sqrt(" + std::to_string(rad) + ")";
                if(d != 1) s += "/" + std::to_string(d);
                return s;
            }
        }
    }
...
static bool trig_text_needs_parens(std::string const &s)
{
    return s.find('*') != std::string::npos || s.find('/') != std::string::npos ||
           s.find('+') != std::string::npos || s.find('-', 1) != std::string::npos;
}

static std::string trig_paren_text(std::string const &s)
{
    return trig_text_needs_parens(s) ? "(" + s + ")" : s;
}
...
static std::optional<std::vector<std::string>> linear_sincos_rform_real(Arena &a, NodeId n)
{
    ...
    auto ratio = [](std::string const &num, std::string const &den) {
        return trig_paren_text(num) + "/" + trig_paren_text(den);
    };
    if(C > 1e-12) {
        double A = std::fabs(S);
        bool plus = S < 0.0;
        ...
        std::vector<std::string> steps{
            "R=sqrt(" + trig_paren_text(Ctxt) + "^2+" + trig_paren_text(absS) + "^2)=" + Rtxt + ".",
    ...
    if(S > 1e-12) {
        ...
        std::vector<std::string> steps{
            "R=sqrt(" + trig_paren_text(Stxt) + "^2+" + trig_paren_text(absC) + "^2)=" + Rtxt + ".",
...
static bool const_near(Arena &a, NodeId n, double want)
{
    if(has_any_symbol(a, n)) return false;
    auto v = numeric_eval(a, n, 0.0);
    return v && std::isfinite(*v) && std::fabs(*v - want) < 1e-10;
}

static bool match_fn_call(Arena &a, NodeId n, FnKind fk, NodeId &arg)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Fn || x.fkind != fk) return false;
    arg = x.a;
    return true;
}

static bool match_fn_square_plain(Arena &a, NodeId n, FnKind fk, NodeId &arg)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Pow || !const_near(a, x.b, 2.0)) return false;
    return match_fn_call(a, x.a, fk, arg);
}

static bool match_sin_cos_product(Arena &a, NodeId n, NodeId &arg)
{
    Node const &x = a.get(n);
    ...
    for(NodeId k : x.kids) {
        ...
        if(q.kind == NodeKind::Fn && (q.fkind == FnKind::Sin || q.fkind == FnKind::Cos)) {
            if(got && !same_sig(a, got, q.a)) return false;
            got = q.a;
            got_s = got_s || q.fkind == FnKind::Sin;
            got_c = got_c || q.fkind == FnKind::Cos;
            continue;
        }
        return false;
    }
    if(!got_s || !got_c || std::fabs(coef - 1.0) > 1e-10) return false;
    arg = got;
    return true;
}

static bool match_sincos_plus_half(Arena &a, NodeId n, NodeId &arg)
{
    Node const &x = a.get(n);
    ...
    for(NodeId k : x.kids) {
        NodeId p_arg = 0;
        if(match_sin_cos_product(a, k, p_arg)) {
            if(got && !same_sig(a, got, p_arg)) return false;
            got = p_arg;
            got_prod = true;
            continue;
        }
        if(has_any_symbol(a, k)) return false;
        auto v = numeric_eval(a, k, 0.0);
        if(!v) return false;
        c += *v;
    }
    if(!got_prod || std::fabs(c - 0.5) > 1e-10) return false;
    arg = got;
    return true;
}

static bool match_const_plus_trig(
    Arena &a, NodeId n, FnKind fk, double want_c, double want_k, NodeId &arg)
{
    std::vector<NodeId> terms;
    Node const &x = a.get(n);
    if(x.kind == NodeKind::Add) terms = x.kids;
    else terms.push_back(n);
    double c = 0.0, ksum = 0.0;
    NodeId got = 0;
    for(NodeId t : terms) {
        double k = 1.0;
        NodeId rest = t;
        ...
        if(got && !same_sig(a, got, r.a)) return false;
        got = r.a;
        ksum += k;
    }
    if(!got || std::fabs(c - want_c) > 1e-10 || std::fabs(ksum - want_k) > 1e-10) return false;
    arg = got;
    return true;
}

static bool match_sqrt_node(Arena &a, NodeId n, NodeId &inside)
{
    Node const &x = a.get(n);
...
static bool match_const_plus_cos(Arena &a, NodeId n, NodeId arg, double &c0, double &c1)
{
    std::vector<NodeId> terms;
    Node const &x = a.get(n);
    if(x.kind == NodeKind::Add) terms = x.kids;
    ...
    for(NodeId t : terms) {
        ...
        Node const &r = a.get(rest);
        if(r.kind != NodeKind::Fn || r.fkind != FnKind::Cos || !same_sig(a, r.a, arg)) return false;
        c1 += k;
    }
    return std::fabs(c1) > 1e-12;
}

static bool match_sin_over_sqrt_cos(
    Arena &a, NodeId n, double &k, NodeId &arg, double &c0, double &c1)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Div) return false;
    NodeId rest = x.a;
    bool has_rest = true;
    if(!split_coeff_term(a, x.a, k, rest, has_rest) || !has_rest) return false;
    if(!match_fn_call(a, rest, FnKind::Sin, arg)) return false;
    NodeId inside = 0;
    if(!match_sqrt_node(a, x.b, inside)) return false;
    return match_const_plus_cos(a, inside, arg, c0, c1);
}

static std::optional<std::vector<std::string>> solve_sincos_half_cos_square(
    Arena &a, NodeId lhs, NodeId rhs, std::string const &var,
    std::string const &lo_text, std::string const &hi_text, bool rad, bool general)
{
    auto route = [&](NodeId L, NodeId R) -> std::optional<std::vector<std::string>> {
        NodeId A = 0, B = 0;
        if(!match_sincos_plus_half(a, L, A) || !match_fn_square_plain(a, R, FnKind::Cos, B))
            return std::nullopt;
        NodeId twoA = casio::simplify(a, casio::mul(a, {casio::num(a, 2), A}));
        NodeId twoB = casio::simplify(a, casio::mul(a, {casio::num(a, 2), B}));
        NodeId rhs_arg = casio::simplify(a, casio::add(a, {casio::div(a, casio::parse_expr(a, "pi"), casio::num(a, 2)),
                                                           casio::neg(a, twoB)}));
        auto nested = solve_same_fn_linear(a, a.fn(FnKind::Sin, twoA), a.fn(FnKind::Sin, rhs_arg),
                                           var, lo_text, hi_text, rad, general);
        if(!nested) return std::nullopt;
        std::string At = casio::format_expr(a, A);
        std::string Bt = casio::format_expr(a, B);
        std::string twoAt = casio::format_expr(a, twoA);
        std::string twoBt = casio::format_expr(a, twoB);
        std::string Rt = casio::format_expr(a, rhs_arg);
        std::vector<std::string> out{
            casio::format_expr(a, L) + " = " + casio::format_expr(a, R),
            "2sin(" + At + ")cos(" + At + ")+1 = 2cos(" + Bt + ")^2",
            "sin(" + twoAt + ") = 2cos(" + Bt + ")^2-1",
            "sin(" + twoAt + ") = cos(" + twoBt + ")",
            "cos(" + twoBt + ") = sin(" + Rt + ")",
            "sin(" + twoAt + ") = sin(" + Rt + ")",
        };
        for(std::string const &line : *nested) {
            if(!out.empty() && compact_key(out.back()) == compact_key(line)) continue;
            out.push_back(line);
        }
        return out;
    };
...
static std::optional<std::vector<std::string>> solve_sin2_over_one_plus_sin(
    Arena &a, NodeId lhs, NodeId rhs, std::string const &var,
    std::string const &lo_text, std::string const &hi_text, bool rad, bool general)
{
    auto route = [&](NodeId L, NodeId R) -> std::optional<std::vector<std::string>> {
        Node const &d = a.get(L);
        if(d.kind != NodeKind::Div) return std::nullopt;
        NodeId A = 0, den_arg = 0, rhs_arg = 0;
        if(!match_const_plus_trig(a, d.b, FnKind::Sin, 1.0, 1.0, den_arg)) return std::nullopt;
        if(!match_const_plus_trig(a, R, FnKind::Sin, 1.0, -1.0, rhs_arg)) return std::nullopt;
        if(!same_sig(a, den_arg, rhs_arg)) return std::nullopt;
        Node const &num = a.get(d.a);
        if(num.kind != NodeKind::Fn || num.fkind != FnKind::Sin) return std::nullopt;
        NodeId twoA = casio::simplify(a, casio::mul(a, {casio::num(a, 2), den_arg}));
        if(!same_sig(a, num.a, twoA)) return std::nullopt;
        A = den_arg;
        auto lin = linear_angle(a, A, var, rad);
        if(!lin || std::fabs(lin->first) < 1e-12) return std::nullopt;
        std::string At = casio::format_expr(a, A);
        double base = (std::atan(0.5) * 180.0 / M_PI - lin->second) / lin->first;
        double period = 180.0 / std::fabs(lin->first);
        std::vector<std::string> steps{
            casio::format_expr(a, L) + " = " + casio::format_expr(a, R),
            "1+sin(" + At + ")!=0",
            "sin(2*" + At + ") = (1-sin(" + At + "))(1+sin(" + At + "))",
            "2sin(" + At + ")cos(" + At + ") = 1-sin(" + At + ")^2",
            "2sin(" + At + ")cos(" + At + ") = cos(" + At + ")^2",
            "cos(" + At + ")!=0",
            "2tan(" + At + ") = 1",
            "tan(" + At + ") = 1/2",
        };
        ...
        auto xs = x_values_from_angle_degrees(a, A, var, lo_text, hi_text, rad, base_trig_degrees(FnKind::Tan, 0.5));
        steps.push_back(interval_text(angle_bounds(a, lo_text, hi_text, rad), var) + " => " + format_solution_list(var, rad, xs));
        return casio::exam_block("trig solve", steps, format_solution_list(var, rad, xs));
    };
    if(auto out = route(lhs, rhs)) return out;
...
static std::optional<std::vector<std::string>> solve_sin_over_sqrt_cos_eq(
    Arena &a, NodeId lhs, NodeId rhs, std::string const &var,
    std::string const &lo_text, std::string const &hi_text, bool rad, bool general)
{
    auto direct = [&](NodeId L, NodeId R, double &k, NodeId &arg, double &c0, double &c1, double &target) {
        if(!match_sin_over_sqrt_cos(a, L, k, arg, c0, c1)) return false;
        if(has_any_symbol(a, R)) return false;
        auto v = numeric_eval(a, R, 0.0);
        if(!v || !std::isfinite(*v) || std::fabs(*v) < 1e-12) return false;
        target = *v;
        return true;
    };
    auto isolate = [&](NodeId L, NodeId R, double &k, NodeId &arg, double &c0, double &c1, double &target) {
        if(direct(L, R, k, arg, c0, c1, target)) return true;
        if(!const_near(a, R, 0.0)) return false;
        Node const &x = a.get(L);
        if(x.kind != NodeKind::Add) return false;
        std::optional<NodeId> ratio;
        double off = 0.0;
        for(NodeId t : x.kids) {
            double tk = 0.0, tc0 = 0.0, tc1 = 0.0;
            NodeId ta = 0;
            if(!ratio && match_sin_over_sqrt_cos(a, t, tk, ta, tc0, tc1)) {
                ratio = t;
                k = tk;
                arg = ta;
                c0 = tc0;
            ...
            if(has_any_symbol(a, t)) return false;
            auto v = numeric_eval(a, t, 0.0);
            if(!v) return false;
            off += *v;
        }
        if(!ratio || std::fabs(off) < 1e-12) return false;
        target = -off;
        return true;
    };
    double k = 0.0, c0 = 0.0, c1 = 0.0, target = 0.0;
    NodeId arg = 0;
    if(!isolate(lhs, rhs, k, arg, c0, c1, target) &&
       !isolate(rhs, lhs, k, arg, c0, c1, target))
        return std::nullopt;
    auto lin = linear_angle(a, arg, var, rad);
    if(!lin || std::fabs(lin->first) < 1e-12) return std::nullopt;
    double qa = k * k, qb = target * target * c1, qc = target * target * c0 - k * k;
    auto roots = solve_quadratic_d(qa, qb, qc);
    std::string A = casio::format_expr(a, arg);
    std::string kt = trig_root_text(k);
    std::string tt = trig_root_text(target);
    std::string c0t = trig_root_text(c0);
    std::string c1t = trig_root_text(c1);
    std::string radicand = c0t + (c1 < 0 ? "-" + trig_root_text(-c1) : "+" + c1t) + "*cos(" + A + ")";
    std::vector<std::string> steps{
        kt + "*sin(" + A + ")/sqrt(" + radicand + ") = " + tt,
        "sqrt(" + radicand + ")>0",
        kt + "^2*sin(" + A + ")^2 = " + tt + "^2*(" + radicand + ")",
        "u=cos(" + A + "), sin(" + A + ")^2=1-u^2",
        trig_quad_text(qa, qb, qc),
    };
    for(double r : roots) steps.push_back("cos(" + A + ") = " + trig_root_text(r));
    if(general) return casio::exam_block("trig solve", steps, steps.back());
    std::vector<double> xs;
    for(double r : roots) {
        if(r < -1.0 - 1e-10 || r > 1.0 + 1e-10) continue;
        for(double xdeg : x_values_from_angle_degrees(a, arg, var, lo_text, hi_text, rad, base_trig_degrees(FnKind::Cos, r))) {
            double Adeg = lin->first * xdeg + lin->second;
            double den = c0 + c1 * std::cos(Adeg * M_PI / 180.0);
            if(den <= 1e-12) continue;
            double lhs0 = k * std::sin(Adeg * M_PI / 180.0) / std::sqrt(den);
            if(std::fabs(lhs0 - target) < 1e-7) add_unique(xs, xdeg);
        }
    }
    std::sort(xs.begin(), xs.end());
    steps.push_back(interval_text(angle_bounds(a, lo_text, hi_text, rad), var) + " => " + format_solution_list(var, rad, xs));
...
static std::optional<std::vector<std::string>> solve_shifted_sincos_tan_relation(
    Arena &a,
    NodeId lhs,
    NodeId rhs,
    std::string const &var,
...
{
    auto cnode = [&](double v) { return casio::parse_expr(a, trig_root_text(v)); };
    auto finish = [&](NodeId U, NodeId target, std::vector<std::string> steps) -> std::vector<std::string> {
        std::string Ut = format_expr(a, U);
        std::string Tt = format_expr(a, target);
        auto tv = numeric_eval(a, target, 0.0);
        if(tv && std::isfinite(*tv)) {
            std::string exact = trig_root_text(*tv);
            if(exact.find('.') == std::string::npos) Tt = exact;
        }
        steps.push_back("tan(" + Ut + ") = " + Tt + ".");
        if(!tv || !std::isfinite(*tv)) return casio::exam_block("trig solve", steps, "tan(" + Ut + ") = " + Tt);
        if(general) {
            steps.push_back(trig_alpha_family_line(FnKind::Tan, Ut, *tv, rad));
            auto lin = linear_angle(a, U, var, rad);
            if(!lin || std::fabs(lin->first) < 1e-12) return casio::exam_block("trig solve", steps, "tan(" + Ut + ") = " + Tt);
            double base = (std::atan(*tv) * 180.0 / M_PI - lin->second) / lin->first;
            double period = 180.0 / std::fabs(lin->first);
            while(base < 0) base += period;
            while(base >= period) base -= period;
            std::string fam = format_general_trig_family(var, rad, {base}, period);
        ...
        steps.push_back(trig_base_angle_line(FnKind::Tan, Ut, *tv));
        steps.push_back(trig_alpha_family_line(FnKind::Tan, Ut, *tv, rad));
        steps.push_back(interval_text(angle_bounds(a, lo_text, hi_text, rad), var) + ".");
        auto xs = x_values_from_angle_degrees(a, U, var, lo_text, hi_text, rad, base_trig_degrees(FnKind::Tan, *tv));
        return casio::exam_block("trig solve", steps, format_solution_list(var, rad, xs));
    };
    auto shifted = [&](NodeId a0, NodeId b0) {
        return casio::simplify(a, casio::add(a, {a0, casio::neg(a, b0)}));
    };

    auto cos_sin = [&](NodeId L, NodeId R) -> std::optional<std::vector<std::string>> {
        double p = 0.0, q = 0.0;
        NodeId carg = 0, sarg = 0;
        if(!coeff_fn_any(a, L, FnKind::Cos, p, carg) || !coeff_fn_any(a, R, FnKind::Sin, q, sarg)) return std::nullopt;
        if(std::fabs(p) < 1e-12 || std::fabs(q) < 1e-12) return std::nullopt;
        auto c_shift = unit_var_shift(a, carg, var);
        auto s_shift = unit_var_shift(a, sarg, var);
        if(!c_shift || !s_shift) return std::nullopt;
        NodeId B = shifted(*s_shift, *c_shift);
        auto sinB = exact_trig(a, FnKind::Sin, B);
        auto cosB = exact_trig(a, FnKind::Cos, B);
        if(!sinB || !cosB || casio::same_by_sig(a, *cosB, casio::num(a, 0))) return std::nullopt;
        NodeId P = cnode(p), Q = cnode(q);
        NodeId num = casio::simplify(a, casio::add(a, {P, casio::neg(a, casio::mul(a, {Q, *sinB}))}));
        NodeId den = casio::simplify(a, casio::mul(a, {Q, *cosB}));
        NodeId target = casio::simplify(a, casio::div(a, num, den));
        std::string Ut = format_expr(a, carg);
        std::string Bt = format_expr(a, B);
        std::vector<std::string> steps{
            format_expr(a, L) + " = " + format_expr(a, R) + ".",
            "Let U=" + Ut + ", B=" + Bt + ".",
            "sin(U+B)=sin(U)cos(B)+cos(U)sin(B).",
            trig_root_text(p) + "*cos(U)=" + trig_root_text(q) + "*(sin(U)cos(B)+cos(U)sin(B)).",
            "(" + trig_root_text(p) + "-" + trig_root_text(q) + "*sin(B))*cos(U)=" + trig_root_text(q) + "*cos(B)*sin(U).",
        };
        return finish(carg, target, steps);
    };

    auto sin_cos = [&](NodeId L, NodeId R) -> std::optional<std::vector<std::string>> {
        double p = 0.0, q = 0.0;
        NodeId sarg = 0, carg = 0;
        if(!coeff_fn_any(a, L, FnKind::Sin, p, sarg) || !coeff_fn_any(a, R, FnKind::Cos, q, carg)) return std::nullopt;
        if(std::fabs(p) < 1e-12 || std::fabs(q) < 1e-12) return std::nullopt;
        auto s_shift = unit_var_shift(a, sarg, var);
        auto c_shift = unit_var_shift(a, carg, var);
        if(!s_shift || !c_shift) return std::nullopt;
        NodeId B = shifted(*c_shift, *s_shift);
        auto sinB = exact_trig(a, FnKind::Sin, B);
        auto cosB = exact_trig(a, FnKind::Cos, B);
        if(!sinB || !cosB) return std::nullopt;
        NodeId P = cnode(p), Q = cnode(q);
        NodeId num = casio::simplify(a, casio::mul(a, {Q, *cosB}));
        NodeId den = casio::simplify(a, casio::add(a, {P, casio::mul(a, {Q, *sinB})}));
        if(casio::same_by_sig(a, den, casio::num(a, 0))) return std::nullopt;
        NodeId target = casio::simplify(a, casio::div(a, num, den));
        std::string Ut = format_expr(a, sarg);
        std::string Bt = format_expr(a, B);
        std::vector<std::string> steps{
            format_expr(a, L) + " = " + format_expr(a, R) + ".",
            "Let U=" + Ut + ", B=" + Bt + ".",
            "cos(U+B)=cos(U)cos(B)-sin(U)sin(B).",
            trig_root_text(p) + "*sin(U)=" + trig_root_text(q) + "*(cos(U)cos(B)-sin(U)sin(B)).",
            "(" + trig_root_text(p) + "+" + trig_root_text(q) + "*sin(B))*sin(U)=" + trig_root_text(q) + "*cos(B)*cos(U).",
        };
        return finish(sarg, target, steps);
    };

    if(auto r = cos_sin(lhs, rhs)) return *r;
    if(auto r = cos_sin(rhs, lhs)) return *r;
    if(auto r = sin_cos(lhs, rhs)) return *r;
    if(auto r = sin_cos(rhs, lhs)) return *r;
    return std::nullopt;
}
...
static std::optional<std::pair<std::pair<double, double>, double>> tan_zero_target(Arena &a, NodeId target, std::string const &var, bool rad)
{
    ...
    std::vector<NodeId> terms = x.kind == NodeKind::Add ? x.kids : std::vector<NodeId>{t};
    double tan_coeff = 0.0;
    double constant = 0.0;
    std::optional<std::pair<double, double>> lin;
    for(NodeId term : terms) {
        if(!has_any_symbol(a, term)) {
            auto v = numeric_eval(a, term, 0.0);
            if(!v || !std::isfinite(*v)) return std::nullopt;
            constant += *v;
            continue;
        }
        ...
        NodeId rest = term;
        bool has_rest = true;
        if(!split_coeff_term(a, term, coeff, rest, has_rest) || !has_rest) return std::nullopt;
        Node const &r = a.get(rest);
        if(r.kind != NodeKind::Fn || r.fkind != FnKind::Tan) return std::nullopt;
        auto angle = linear_angle(a, r.a, var, rad);
        if(!angle || std::fabs(angle->first) < 1e-12) return std::nullopt;
        if(lin && (std::fabs(lin->first - angle->first) > 1e-12 || std::fabs(lin->second - angle->second) > 1e-9))
            return std::nullopt;
        lin = *angle;
        tan_coeff += coeff;
    }
    if(!lin || std::fabs(tan_coeff) < 1e-12) return std::nullopt;
    return std::make_pair(*lin, -constant / tan_coeff);
}

static std::optional<std::vector<std::string>> shifted_linear_tan_target_route(
    Arena &a,
    NodeId src,
    NodeId target,
    std::string const &var,
...
{
    auto p = collect_shift_lin_trig(a, src, var, rad);
    auto t = tan_zero_target(a, target, var, rad);
    if(!p || !t || std::fabs(p->k) > 1e-10 || std::fabs(p->s) < 1e-12) return std::nullopt;
    if(std::fabs(t->first.first - p->m) > 1e-12 || std::fabs(t->first.second) > 1e-9) return std::nullopt;
    double want = -p->c / p->s;
    if(std::fabs(want - t->second) > 1e-8) return std::nullopt;
    std::string M = std::fabs(p->m - 1.0) < 1e-12 ? var : trig_root_text(p->m) + "*" + var;
    auto term = [](double v, std::string const &body, bool first) {
        std::string s;
        if(v < 0) s += first ? "-" : " - ";
        else if(!first) s += " + ";
        double av = std::fabs(v);
    ...
    std::string lhs = term(p->s, "sin(" + M + ")", true) + term(p->c, "cos(" + M + ")", false);
    return casio::exam_block(
        "trig transform",
        {
            "Source = " + format_expr(a, src) + ".",
            "Use compound-angle expansions.",
            lhs + " = 0.",
            term(p->s, "sin(" + M + ")", true) + " = " + trig_root_text(-p->c) + "*cos(" + M + ").",
            "cos(" + M + ") != 0.",
            "tan(" + M + ") = " + trig_root_text(want) + ".",
        },
        "tan(" + M + ") = " + trig_root_text(want)
    );
}
...
)
{
    ...
    if(std::fabs(poly->sc) < 1e-12 && std::fabs(poly->c1) < 1e-12 && std::fabs(poly->c2) < 1e-12) {
        ...
        auto term = [](std::string const &c, std::string const &body) {
            return trig_paren_text(c) + "*" + body;
        };
        steps.push_back("a=" + c_txt + ", b=" + s_txt + " for a*cos(A)+b*sin(A).");
        steps.push_back("R=sqrt(" + trig_paren_text(c_abs_txt) + "^2+" + trig_paren_text(s_abs_txt) + "^2)=" + r_txt + ".");
        ...
        steps.push_back(cos_aux + "=" + beta_txt + unit_txt + ", so " + term(c_txt, "cos(A)") + "+" + term(s_txt, "sin(A)") + "=R*cos(A-" + cos_aux + ").");
        ...
        steps.push_back(sin_aux + "=" + alpha_txt + unit_txt + ", so " + term(s_txt, "sin(A)") + "+" + term(c_txt, "cos(A)") + "=R*sin(A+" + sin_aux + ").");
                                                     ...
                                                     std::optional<bool> rad_override = std::nullopt)
{
    ...
    if(auto sh = solve_sincos_half_cos_square(a, lhs, rhs, var, lo_text, hi_text, rad, general)) return *sh;
    if(auto sp = solve_sin2_over_one_plus_sin(a, lhs, rhs, var, lo_text, hi_text, rad, general)) return *sp;
    if(auto sqr = solve_sin_over_sqrt_cos_eq(a, lhs, rhs, var, lo_text, hi_text, rad, general)) return *sqr;
    ...
    if(auto rel2 = solve_shifted_sincos_tan_relation(a, lhs, rhs, var, lo_text, hi_text, rad, general)) return *rel2;
...
std::vector<std::string> run(Arena &arena, Request const &req)
{
    ...
    if(auto inv_compound = inverse_trig_compound_exact_route(arena, parsed)) return *inv_compound;
    ...
    if(x.kind == NodeKind::Fn) {
        if(x.fkind == FnKind::Sin || x.fkind == FnKind::Cos || x.fkind == FnKind::Tan ||
           x.fkind == FnKind::Sec || x.fkind == FnKind::Cosec || x.fkind == FnKind::Cot) {
            if(auto inv_compound = inverse_trig_compound_exact_route(arena, n)) return *inv_compound;
