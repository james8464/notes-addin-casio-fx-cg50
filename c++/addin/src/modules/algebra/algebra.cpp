...
{
    Node const &x = a.get(n);
    return (x.kind == NodeKind::Const && x.ckind == ConstKind::E) ||
...
static std::optional<NodeId> strip_whole_negative(Arena &a, NodeId n)
{
    ...
    if(x.kind == NodeKind::Mul) {
        ...
        for(NodeId k : x.kids) {
            Node const &u = a.get(k);
            if(!stripped && u.kind == NodeKind::Num && u.num.num < 0) {
                kids.push_back(casio::num(a, -u.num.num, u.num.den));
                stripped = true;
            }
            else kids.push_back(k);
        }
        if(stripped) return casio::simplify(a, casio::mul(a, kids));
    }
    if(x.kind == NodeKind::Add) {
        std::vector<NodeId> kids;
        kids.reserve(x.kids.size());
        for(NodeId k : x.kids) {
            auto p = strip_whole_negative(a, k);
...
static NodeId exact_eval_simplify(Arena &a, NodeId n)
{
    ...
    if(x.kind == NodeKind::Fn) {
        if(x.fkind == FnKind::Log) {
            Node const &raw = a.get(x.a);
            if(raw.kind == NodeKind::Pow && is_e_const_node(a, raw.a)) return exact_eval_simplify(a, raw.b);
            if(raw.kind == NodeKind::Fn && raw.fkind == FnKind::Exp) return exact_eval_simplify(a, raw.a);
        }
        ...
        if(x.fkind == FnKind::Sqrt && u.kind == NodeKind::Mul) {
            Rational c{1, 1};
            std::vector<NodeId> rest;
            for(NodeId k : u.kids) {
                Node const &m = a.get(k);
                if(m.kind == NodeKind::Num) c = r_mul(c, m.num);
                else rest.push_back(k);
            }
            std::int64_t rn = 0, rd = 0;
            if(c.num > 0 && is_square_i64(c.num, rn) && is_square_i64(c.den, rd) && rd != 0 && rn != rd && !rest.empty()) {
                NodeId inner = rest.size() == 1 ? rest[0] : casio::mul(a, rest);
                return exact_eval_simplify(a, casio::mul(a, {casio::num(a, rn, rd), casio::fn(a, "sqrt", inner)}));
            }
        }
        ...
        if(x.fkind == FnKind::Exp) {
            if(auto split = exp_split_log_sum(a, arg)) return exact_eval_simplify(a, *split);
        }
    ...
    if(x.kind == NodeKind::Pow) {
        ...
        if(is_e_const_node(a, lhs) && expo.kind == NodeKind::Fn && expo.fkind == FnKind::Log)
...
static std::optional<std::string> simplify_log_exp_text(Arena &a, NodeId n)
{
    ...
    bool changed = false;
    for(std::size_t p = s.find("ln(e^("); p != std::string::npos; p = s.find("ln(e^(", p + 1)) {
        std::size_t i = p + 6, j = i;
        int depth = 1;
        for(; j < s.size(); ++j) {
            if(s[j] == '(') ++depth;
            else if(s[j] == ')' && --depth == 0) break;
        }
        if(j >= s.size() || j + 1 >= s.size() || s[j + 1] != ')') continue;
        s.replace(p, j + 2 - p, "(" + s.substr(i, j - i) + ")");
        changed = true;
    }
    if(!changed) return std::nullopt;
    return format_expr(a, casio::simplify(a, casio::parse_expr(a, s)));
}
...
static std::optional<Rational> num_node_early(Arena &a, NodeId n)
{
...
static bool affine_exp_coeffs(Arena &a, NodeId expr, std::string const &var, NodeId &expn, bool &seen_exp, Rational &m, Rational &b)
{
    m = Rational{0, 1};
    b = Rational{0, 1};
    std::vector<NodeId> terms;
    ...
    for(NodeId t : terms) {
        NodeId c = 0, e = 0;
        if(const_times_exp(a, t, c, e)) {
            auto cr = num_node_early(a, exact_eval_simplify(a, c));
            if(!cr) return false;
            if(seen_exp && format_expr(a, expn) != format_expr(a, e)) return false;
            expn = e;
            seen_exp = true;
            m = r_add(m, *cr);
            any = true;
        }
        else if(!contains_var_simple(a, t, var)) {
            auto br = num_node_early(a, exact_eval_simplify(a, t));
            if(!br) return false;
            b = r_add(b, *br);
        }
        else return false;
...
static std::optional<NodeId> inverse_exp_mobius(Arena &a, NodeId expr)
{
    Node const &x = a.get(expr);
    ...
    if(!affine_exp_coeffs(a, x.a, "x", expn, seen, A, B)) return std::nullopt;
    if(!affine_exp_coeffs(a, x.b, "x", expn, seen, C, D)) return std::nullopt;
    ...
    Rational tm = r_neg(D), tb = B, bm = C, bb = r_neg(A);
    if(tm.num < 0 && tb.num < 0) { tm = r_neg(tm); tb = r_neg(tb); bm = r_neg(bm); bb = r_neg(bb); }
    NodeId top = linear_expr(a, tm, tb, y);
    NodeId bot = linear_expr(a, bm, bb, y);
    NodeId ratio = casio::simplify(a, casio::div(a, top, bot));
    NodeId logy = casio::fn(a, "log", ratio);
    NodeId shifted = casio::add(a, {logy, casio::neg(a, a.num(p->a0))});
...
static bool is_negative_one_num(Arena &a, NodeId n)
{
    auto r = as_num(a, n);
...
static std::optional<std::string> quadratic_rational_interval_range(
    Arena &a,
    NodeId n,
    std::string const &var,
    std::string const &lo,
...
)
{
    ...
    if(!x0 || !x1 || r_cmp(*x0, *x1) >= 0) return std::nullopt;
    auto rp = ratpoly_of_node(a, n, var);
    if(!rp.ok || !is_zero(rp.num.a2) || is_zero(rp.den.a2)) return std::nullopt;

    auto y_at = [&](Rational x) -> std::optional<Rational> {
        Rational d = poly_eval(rp.den, x);
        if(is_zero(d)) return std::nullopt;
        return r_div(poly_eval(rp.num, x), d);
    };
    auto inside = [&](double z) {
        double l = (double)x0->num / x0->den;
        double h = (double)x1->num / x1->den;
        return z > l + 1e-10 && z < h - 1e-10;
    };
    Rational A = r_neg(r_mul(rp.num.a1, rp.den.a2));
    Rational B = r_mul(Rational{-2, 1}, r_mul(rp.num.a0, rp.den.a2));
    Rational C = r_sub(r_mul(rp.num.a1, rp.den.a0), r_mul(rp.num.a0, rp.den.a1));
    double da = (double)A.num / A.den, db = (double)B.num / B.den, dc = (double)C.num / C.den;
    if(std::fabs(da) < 1e-14) {
        if(std::fabs(db) > 1e-14 && inside(-dc / db)) return std::nullopt;
    }
    else {
        double disc = db * db - 4.0 * da * dc;
        if(disc > 1e-12) {
            double s = std::sqrt(disc);
            if(inside((-db - s) / (2.0 * da)) || inside((-db + s) / (2.0 * da))) return std::nullopt;
        }
    }

    auto y0 = y_at(*x0);
    auto y1 = y_at(*x1);
    Rational mid = r_div(r_add(*x0, *x1), Rational{2, 1});
    auto ym = y_at(mid);
    if(!ym) return std::nullopt;
    auto bound = [&](Rational y, bool open, bool lower) {
        return std::string("y ") + (lower ? (open ? "> " : ">= ") : (open ? "< " : "<= ")) + rat_node_text(a, y);
    };
    if(!y1 && hi_open && y0) {
        steps.push_back("Vertical asymptote x = " + rat_node_text(a, *x1) + ".");
        steps.push_back("Endpoint gives y(" + rat_node_text(a, *x0) + ") = " + rat_node_text(a, *y0) + ".");
        return r_cmp(*ym, *y0) > 0 ? bound(*y0, lo_open, true) : bound(*y0, lo_open, false);
    }
    if(!y0 && lo_open && y1) {
        steps.push_back("Vertical asymptote x = " + rat_node_text(a, *x0) + ".");
        steps.push_back("Endpoint gives y(" + rat_node_text(a, *x1) + ") = " + rat_node_text(a, *y1) + ".");
        return r_cmp(*ym, *y1) > 0 ? bound(*y1, hi_open, true) : bound(*y1, hi_open, false);
    }
    if(!y0 || !y1) return std::nullopt;
    steps.push_back("Function is monotone on this interval.");
    steps.push_back("Evaluate endpoints: y(" + rat_node_text(a, *x0) + ")=" + rat_node_text(a, *y0) + ", y(" + rat_node_text(a, *x1) + ")=" + rat_node_text(a, *y1) + ".");
    bool asc = r_cmp(*y0, *y1) <= 0;
    Rational mn = asc ? *y0 : *y1, mx = asc ? *y1 : *y0;
    bool mn_open = asc ? lo_open : hi_open;
    bool mx_open = asc ? hi_open : lo_open;
    return rat_node_text(a, mn) + (mn_open ? " < y " : " <= y ") + (mx_open ? "< " : "<= ") + rat_node_text(a, mx);
}
...
static std::optional<NodeId> exp_residual_alt(Arena &a, NodeId residual)
{
    ...
    NodeId pa = 0, pb = 0;
    bool have_pa = false, have_pb = false;
    for(auto k : n.kids) {
        Rational coef;
        NodeId body = 0;
        bool has_body = false;
    ...
    if(!have_pa || !have_pb) return exp_residual_alt_text(a, residual);
...
static bool one_to_one_arg(Arena &a, NodeId body, FnKind &kind, NodeId &arg)
{
    ...
    if(n.fkind == FnKind::Exp || n.fkind == FnKind::Log10 ||
       n.fkind == FnKind::Sqrt || n.fkind == FnKind::Asin || n.fkind == FnKind::Acos ||
       n.fkind == FnKind::Atan) {
        kind = n.fkind;
        arg = n.a;
        return true;
...
static bool restricted_interval_bounds(Arena &a, std::string const &domain_text, double &lo, double &hi)
{
    ...
    if(parts.size() >= 3 && trim_text(parts[0]) == "x") {
        auto l = parse_const_double(a, parts[1]);
        auto h = parse_const_double(a, parts[2]);
        if(l && h) { lo = *l; hi = *h; return lo < hi; }
    }
    bool seen = false;
    for(std::string part : parts) {
        ...
        for(std::string op : {std::string(">="), std::string("<="), std::string(">"), std::string("<")}) {
            ...
            if(left == "x") {
                ...
                if(v) { if(op[0] == '>') lo = std::max(lo, *v); else hi = std::min(hi, *v); seen = true; }
            }
            else if(right == "x") {
...
static std::optional<NodeId> inverse_restricted_quadratic(Arena &a, NodeId n, std::string const &domain_text)
{
    ...
    if(restricted_interval_bounds(a, domain_text, lo_d, hi_d)) { lo = lo_d; hi = hi_d; }
    else return std::nullopt;
...
static std::optional<NodeId> inverse_restricted_quadratic_rational(Arena &a, NodeId n, std::string const &domain_text)
{
    double lo = 0.0, hi = 0.0;
    if(!restricted_interval_bounds(a, domain_text, lo, hi)) return std::nullopt;
    auto rp = ratpoly_of_node(a, n, "x");
    if(!rp.ok || !is_zero(rp.num.a2) || is_zero(rp.den.a2)) return std::nullopt;
    if(is_zero(rp.num.a1) && is_zero(rp.num.a0)) return std::nullopt;
    ...
    double mid = (lo_f && hi_f) ? 0.5 * (lo + hi) : (lo_f ? lo + 1.0 : hi - 1.0);
    auto ymid = eval_node(a, n, "x", mid);
    if(!ymid || !std::isfinite(*ymid)) return std::nullopt;

    NodeId y = casio::sym(a, "x");
    NodeId By_minus_a = casio::add(a, {
        casio::mul(a, {a.num(rp.den.a1), y}),
        a.num(r_neg(rp.num.a1)),
    });
    NodeId Cy_minus_b = casio::add(a, {
        casio::mul(a, {a.num(rp.den.a0), y}),
        a.num(r_neg(rp.num.a0)),
    });
    NodeId disc = exact_eval_simplify(a, casio::add(a, {
        casio::power(a, By_minus_a, casio::num(a, 2)),
        casio::neg(a, casio::mul(a, {casio::num(a, 4), a.num(rp.den.a2), y, Cy_minus_b})),
    }));
    if(auto dp = poly_of(a, disc, "x"); dp && dp->ok) disc = poly2_to_node(a, *dp, "x");
    NodeId root = casio::fn(a, "sqrt", disc);
    NodeId top_base = casio::add(a, {
        a.num(rp.num.a1),
        casio::neg(a, casio::mul(a, {a.num(rp.den.a1), y})),
    });
    NodeId den = casio::mul(a, {casio::num(a, 2), a.num(rp.den.a2), y});
    NodeId cands[2] = {
        exact_eval_simplify(a, casio::div(a, casio::add(a, {top_base, root}), den)),
        exact_eval_simplify(a, casio::div(a, casio::add(a, {top_base, casio::neg(a, root)}), den)),
    };
    for(NodeId c : cands) {
        auto v = eval_node(a, c, "x", *ymid);
        if(v && std::fabs(*v - mid) < 1e-6) return c;
                                                                                 ...
                                                                                 bool hi_open = false)
{
    ...
    for(auto const &s : raw) {
        auto v = solution_line_value(a, s);
        if(v && lo && *v < *lo - 1e-9) continue;
        if(v && hi && *v > *hi + 1e-9) continue;
        valid.push_back(s);
    }
...
struct CubicXY
{
    Rational x3{0, 1}, x2y{0, 1}, xy2{0, 1}, y3{0, 1}, rhs{0, 1};
};

static bool read_cubic_xy_equation(std::string const &eq, CubicXY &p)
{
    auto sides = split_top_key(eq, '=');
    if(sides.size() != 2) return false;
    auto rhs = parse_rational_key(sides[1]);
    ...
    for(auto const &t : split_signed_terms_key(sides[0])) {
        Rational c{0, 1};
        if(read_coeff_suffix(t, "x^3", c)) p.x3 = r_add(p.x3, c);
        else if(read_coeff_suffix(t, "x^2y", c) || read_coeff_suffix(t, "x^2*y", c)) p.x2y = r_add(p.x2y, c);
        else if(read_coeff_suffix(t, "xy^2", c) || read_coeff_suffix(t, "x*y^2", c)) p.xy2 = r_add(p.xy2, c);
        else if(read_coeff_suffix(t, "y^3", c)) p.y3 = r_add(p.y3, c);
        else return false;
    }
    return true;
}

static Rational cubic_xy_at(CubicXY const &p, Rational r)
{
    return r_add(r_add(p.x3, r_mul(p.x2y, r)), r_add(r_mul(p.xy2, r_mul(r, r)), r_mul(p.y3, r_pow_int(r, 3))));
}

static std::vector<Rational> rational_roots_poly_any(PolyAny p)
{
    std::vector<Rational> roots;
    trim_poly_any(p);
    while(p.c.size() > 1) {
...
static std::optional<std::vector<std::string>> homogeneous_cubic_xy_system_route(Arena &a, std::string const &expr)
{
    std::string key = compact_input_key(expr);
    if(key.rfind("solve(", 0) == 0 && key.back() == ')') key = key.substr(6, key.size() - 7);
    auto parts = split_csv(key);
    if(parts.size() < 2 || parts[1] != "[x,y]" || parts[0].size() < 3 || parts[0].front() != '[' || parts[0].back() != ']')
        return std::nullopt;
    bool integer = false;
    for(std::size_t i = 2; i < parts.size(); ++i) {
        if(parts[i] == "z" || parts[i] == "int" || parts[i] == "integer" || parts[i] == "integers") integer = true;
    }
    auto eqs = split_top_key(parts[0].substr(1, parts[0].size() - 2), ',');
    if(eqs.size() != 2) return std::nullopt;
    CubicXY p, q;
    if(!read_cubic_xy_equation(eqs[0], p) || !read_cubic_xy_equation(eqs[1], q) || is_zero(p.rhs) || is_zero(q.rhs))
        return std::nullopt;
    PolyAny rp{{r_sub(r_mul(q.rhs, p.x3), r_mul(p.rhs, q.x3)),
                r_sub(r_mul(q.rhs, p.x2y), r_mul(p.rhs, q.x2y)),
                r_sub(r_mul(q.rhs, p.xy2), r_mul(p.rhs, q.xy2)),
                r_sub(r_mul(q.rhs, p.y3), r_mul(p.rhs, q.y3))}, true};
    trim_poly_any(rp);
    auto roots = rational_roots_poly_any(rp);
    if(roots.empty()) return std::nullopt;
    std::vector<std::string> out;
    out.push_back(eqs[0]);
    out.push_back(eqs[1]);
    out.push_back("r = y/x");
    out.push_back("x^3*P(r) = " + format_rat_plain(p.rhs));
    out.push_back("x^3*Q(r) = " + format_rat_plain(q.rhs));
    out.push_back(format_rat_plain(q.rhs) + "*P(r) " + (p.rhs.num < 0 ? "+ " + format_rat_plain(r_neg(p.rhs)) : "- " + format_rat_plain(p.rhs)) + "*Q(r) = 0");
    out.push_back(format_expr(a, poly_any_to_node(a, rp, "r")) + " = 0");
    if(integer) out.push_back("x,y in Z");
    std::vector<std::string> pairs;
    for(Rational r : roots) {
        Rational pv = cubic_xy_at(p, r);
        if(is_zero(pv)) continue;
        Rational x3 = r_div(p.rhs, pv);
        std::string xt = cube_root_rational_text(a, x3);
        auto xr = parse_rational_text(xt);
        if(integer && (!xr || xr->den != 1 || r.den != 1)) continue;
        std::string yt = multiply_root_text(r, xt);
        if(integer) {
            Rational yr = r_mul(r, *xr);
            if(yr.den != 1) continue;
            yt = format_rat_plain(yr);
        }
        out.push_back("r = " + format_rat_plain(r));
        out.push_back("x^3 = " + format_rat_plain(x3));
        out.push_back("(x,y) = (" + xt + "," + yt + ")");
        pairs.push_back("(" + xt + "," + yt + ")");
    }
    if(pairs.empty()) return std::nullopt;
    std::string ans;
    for(std::size_t i = 0; i < pairs.size(); ++i) {
...
static bool read_const_minus_linear(std::string const &s, char var, Rational &c0, Rational &k)
{
    c0 = Rational{0, 1};
    Rational cv{0, 1};
    bool saw_c = false, saw_v = false;
    for(auto const &t : split_signed_terms_key(s)) {
        Rational c{0, 1};
        if(read_coeff_suffix(t, std::string(1, var), c)) {
            cv = r_add(cv, c);
            saw_v = true;
        }
        else if(auto q = parse_rational_key(t)) {
            c0 = r_add(c0, *q);
            saw_c = true;
        }
        else return false;
    }
    if(!saw_c || !saw_v || cv.num >= 0) return false;
    k = r_neg(cv);
    return true;
}

static bool exact_cube_root_rat(Rational r, Rational &out)
{
    bool neg = r.num < 0;
    ...
    if(!rn || !rd || !*rd) return false;
    out = Rational{neg ? -*rn : *rn, *rd};
    out.normalize();
    return true;
}

static bool read_shift_cube_xy_eq(std::string const &eq, char var, std::string const &rhs_suffix, Rational &c0, Rational &k, Rational &m3)
{
    auto sides = split_top_key(eq, '=');
    if(sides.size() != 2) return false;
    std::string lhs = sides[0];
    if(lhs.size() <= 4 || lhs.front() != '(' || lhs.compare(lhs.size() - 3, 3, ")^3") != 0) return false;
    if(!read_const_minus_linear(lhs.substr(1, lhs.size() - 4), var, c0, k)) return false;
    return read_coeff_suffix(sides[1], rhs_suffix, m3);
}

static std::optional<std::vector<std::string>> shifted_cube_cross_system_route(Arena &a, std::string const &expr)
{
    std::string key = compact_input_key(expr);
    if(key.rfind("solve(", 0) == 0 && key.back() == ')') key = key.substr(6, key.size() - 7);
    auto parts = split_csv(key);
    ...
    auto eqs = split_top_key(parts[0].substr(1, parts[0].size() - 2), ',');
    if(eqs.size() != 2) return std::nullopt;
    Rational A, B, Kx, Ky, Mx3, My3, M;
    bool ok = read_shift_cube_xy_eq(eqs[0], 'x', "xy^2", A, Kx, Mx3) &&
              read_shift_cube_xy_eq(eqs[1], 'y', "x^2y", B, Ky, My3);
    if(!ok) ok = read_shift_cube_xy_eq(eqs[1], 'x', "xy^2", A, Kx, Mx3) &&
                 read_shift_cube_xy_eq(eqs[0], 'y', "x^2y", B, Ky, My3);
    if(!ok || r_cmp(Kx, Ky) != 0 || r_cmp(Mx3, My3) != 0 || !exact_cube_root_rat(Mx3, M)) return std::nullopt;
    PolyAny up{{r_mul(A, Kx), r_neg(r_mul(B, M)), r_mul(A, M), r_neg(r_mul(B, Kx))}, true};
    trim_poly_any(up);
    auto roots = rational_roots_poly_any(up);
    if(roots.empty()) return std::nullopt;
    std::vector<std::string> out{eqs[0], eqs[1]};
    out.push_back("u = (x/y)^(1/3), so x = u^3*y");
    out.push_back(format_rat_plain(A) + " = y*(" + format_rat_plain(Kx) + "*u^3 + " + format_rat_plain(M) + "*u)");
    out.push_back(format_rat_plain(B) + " = y*(" + format_rat_plain(Kx) + " + " + format_rat_plain(M) + "*u^2)");
    out.push_back(format_expr(a, poly_any_to_node(a, up, "u")) + " = 0");
    std::vector<std::string> pairs;
    for(Rational u : roots) {
        Rational den = r_add(Kx, r_mul(M, r_mul(u, u)));
        if(is_zero(den)) continue;
        Rational y = r_div(B, den);
        Rational x = r_mul(r_pow_int(u, 3), y);
        out.push_back("u = " + format_rat_plain(u));
        out.push_back("y = " + format_rat_plain(B) + "/(" + format_rat_plain(Kx) + " + " + format_rat_plain(M) + "*u^2) = " + format_rat_plain(y));
        out.push_back("x = u^3*y = " + format_rat_plain(x));
        pairs.push_back("(" + format_rat_plain(x) + "," + format_rat_plain(y) + ")");
    }
    if(pairs.empty()) return std::nullopt;
    std::string ans;
    for(std::size_t i = 0; i < pairs.size(); ++i) {
...
static bool read_xyz_power_sum(std::string const &eq, int pow, Rational &rhs)
{
    auto sides = split_top_key(eq, '=');
    if(sides.size() != 2) return false;
    auto r = parse_rational_key(sides[1]);
    if(!r) return false;
    bool hx = false, hy = false, hz = false;
    std::string sx = pow == 1 ? "x" : "x^" + std::to_string(pow);
    std::string sy = pow == 1 ? "y" : "y^" + std::to_string(pow);
    std::string sz = pow == 1 ? "z" : "z^" + std::to_string(pow);
    for(auto const &t : split_signed_terms_key(sides[0])) {
        if(t == sx) hx = true;
        else if(t == sy) hy = true;
        else if(t == sz) hz = true;
        else return false;
    }
    if(!hx || !hy || !hz) return false;
    rhs = *r;
    return true;
}

static std::optional<std::vector<std::string>> xyz_power_sum_aux_cubic_route(Arena &a, std::string const &expr)
{
    std::string key = compact_input_key(expr);
    if(key.rfind("solve(", 0) == 0 && key.back() == ')') key = key.substr(6, key.size() - 7);
    auto parts = split_csv(key);
    if(parts.size() < 2 || parts[1] != "[x,y,z]" || parts[0].size() < 3 || parts[0].front() != '[' || parts[0].back() != ']')
        return std::nullopt;
    auto eqs = split_top_key(parts[0].substr(1, parts[0].size() - 2), ',');
    if(eqs.size() != 3) return std::nullopt;
    std::optional<Rational> s1, s2, s3;
    for(auto const &e : eqs) {
        Rational r{0, 1};
        if(read_xyz_power_sum(e, 1, r)) s1 = r;
        else if(read_xyz_power_sum(e, 2, r)) s2 = r;
        else if(read_xyz_power_sum(e, 3, r)) s3 = r;
    }
    if(!s1 || !s2 || !s3) return std::nullopt;
    Rational e2 = r_div(r_sub(r_mul(*s1, *s1), *s2), Rational{2, 1});
    Rational e3 = r_div(r_sub(*s3, r_mul(*s1, r_sub(*s2, e2))), Rational{3, 1});
    PolyAny cp{{r_neg(e3), e2, r_neg(*s1), Rational{1, 1}}, true};
    auto roots = rational_roots_poly_any(cp);
    if(roots.size() != 3) return std::nullopt;
    std::sort(roots.begin(), roots.end(), [](Rational a, Rational b) { return r_cmp(a, b) < 0; });
    std::vector<std::string> out;
    out.push_back("s = x+y+z = " + format_rat_plain(*s1));
    out.push_back("xy+yz+zx = (s^2 - " + format_rat_plain(*s2) + ")/2 = " + format_rat_plain(e2));
    out.push_back("x^3+y^3+z^3 = s*(" + format_rat_plain(*s2) + " - (xy+yz+zx)) + 3xyz");
    out.push_back(format_rat_plain(*s3) + " = " + format_rat_plain(*s1) + "*(" + format_rat_plain(*s2) + " - (" + format_rat_plain(e2) + ")) + 3xyz");
    out.push_back("xyz = " + format_rat_plain(e3));
    out.push_back(format_expr(a, poly_any_to_node(a, cp, "t")) + " = 0");
    std::string factored;
    std::string ans;
    for(std::size_t i = 0; i < roots.size(); ++i) {
        if(i) { factored += "*"; ans += ","; }
        factored += linear_factor_from_root(a, "t", roots[i]);
        ans += format_rat_plain(roots[i]);
    }
    out.push_back(factored + " = 0");
    out.push_back("t = " + ans);
    out.push_back("(x,y,z) = [" + ans + "] in any order");
    return out;
}
...
static bool extract_system_expr_vars(std::string expr, std::vector<std::string> &eqs, std::vector<std::string> &vars)
{
    if(auto inner = unwrap_call_text(expr, "solve"); !inner.empty()) expr = inner;
    auto parts = split_csv(expr);
    if(parts.size() < 2) return false;
...
static std::optional<Linear2Parts> linear2_parts(Arena &a, NodeId n, std::string const &v0, std::string const &v1)
{
    ...
    NodeId a0 = zero, rest = n;
    if(contains_symbol(a, n, v0)) {
        auto p0 = symbolic_linear_parts(a, n, v0);
        if(!p0 || contains_either_symbol(a, p0->m, v0, v1)) return std::nullopt;
        a0 = p0->m;
        rest = p0->c;
    }

    NodeId a1 = zero, c = rest;
    if(contains_symbol(a, rest, v1)) {
        auto p1 = symbolic_linear_parts(a, rest, v1);
        if(!p1 || contains_either_symbol(a, p1->m, v0, v1)) return std::nullopt;
        a1 = p1->m;
        c = p1->c;
    }
...
{
    auto run = [&](NodeId l, NodeId r) {
        Node const &ln = a.get(l);
        if(ln.kind != NodeKind::Fn || ln.fkind != FnKind::Log) return false;
        NodeId rhs = 0;
        Node const &rn = a.get(r);
        if(rn.kind == NodeKind::Fn && rn.fkind == FnKind::Log) rhs = rn.a;
        else rhs = casio::power(a, casio::constant_e(a), r);
        steps.push_back(format_expr(a, l) + " = " + format_expr(a, r));
        steps.push_back(format_expr(a, ln.a) + " = " + format_expr(a, rhs));
        residual = casio::simplify(a, sub_node(a, ln.a, rhs));
        return true;
    };
...
static std::optional<std::vector<std::string>> exact_linear2_system_route(Arena &a, std::string const &expr)
{
    ...
    for(auto const &txt : eq_texts) {
        ...
        NodeId residual = 0;
        if(!log_equation_residual(a, *eq, residual, out))
            residual = casio::simplify(a, sub_node(a, eq->lhs, eq->rhs));
...
static std::optional<NodeId> log_linear_arg(Arena &a, NodeId n, std::string const &var, std::string const &lnvar, std::string const &other)
{
    ...
    if(!contains_symbol(a, n, var) && !contains_symbol(a, n, other)) return casio::fn(a, "log", n);

    Node const &node = a.get(n);
    if(node.kind != NodeKind::Mul) return std::nullopt;
    std::vector<NodeId> constants;
    bool saw_var = false;
    for(NodeId k : node.kids) {
        if(is_sym_var(a, k, var)) {
            if(saw_var) return std::nullopt;
            saw_var = true;
        }
        else {
            if(contains_symbol(a, k, var) || contains_symbol(a, k, other)) return std::nullopt;
            constants.push_back(k);
        }
    }
    if(!saw_var || constants.empty()) return std::nullopt;
    NodeId c = constants.size() == 1 ? constants[0] : casio::mul(a, constants);
    return exact_eval_simplify(a, casio::add(a, {casio::fn(a, "log", c), casio::sym(a, lnvar)}));
}

static std::optional<NodeId> log_linear_node(Arena &a, NodeId n, std::vector<std::string> const &vars)
{
    Node const &node = a.get(n);
    if(node.kind == NodeKind::Fn && node.fkind == FnKind::Log) {
        if(is_sym_var(a, node.a, vars[0])) return casio::sym(a, "X");
        if(is_sym_var(a, node.a, vars[1])) return casio::sym(a, "Y");
    }
    if(!contains_either_symbol(a, n, vars[0], vars[1])) return n;
    return std::nullopt;
}

static std::optional<NodeId> log_of_positive_side(Arena &a, NodeId n, std::vector<std::string> const &vars)
{
    Node const &node = a.get(n);
    if(node.kind != NodeKind::Pow) return std::nullopt;
    auto base_log0 = log_linear_arg(a, node.a, vars[0], "X", vars[1]);
    auto base_log1 = log_linear_arg(a, node.a, vars[1], "Y", vars[0]);
    std::optional<NodeId> base_log;
    if(base_log0 && (!base_log1 || contains_symbol(a, *base_log0, "X"))) base_log = base_log0;
    else if(base_log1) base_log = base_log1;
    if(!base_log) return std::nullopt;
    auto exponent = log_linear_node(a, node.b, vars);
    if(!exponent) return std::nullopt;
    return exact_eval_simplify(a, casio::mul(a, {*base_log, *exponent}));
}

static std::optional<std::pair<NodeId, NodeId>> snap_ln_integer_exp(Arena &a, NodeId n)
{
    auto v = eval_node(a, n, "x", 0.0);
    if(!v) return std::nullopt;
    if(std::fabs(*v) < 1e-10) return std::make_pair(zero_node(a), one_node(a));
    for(int k = 2; k <= 30; ++k) {
        double lk = std::log((double)k);
        NodeId logk = casio::fn(a, "log", casio::num(a, k));
        if(std::fabs(*v - lk) < 1e-9) return std::make_pair(logk, casio::num(a, k));
        if(std::fabs(*v + lk) < 1e-9) return std::make_pair(neg_node(a, logk), casio::num(a, 1, k));
    }
    return std::nullopt;
}

static std::optional<std::vector<std::string>> log_power_linear2_system_route(Arena &a, std::string const &expr)
{
    std::vector<std::string> eq_texts, vars;
    if(!extract_system_expr_vars(expr, eq_texts, vars)) return std::nullopt;
    if(vars[0] == "X" || vars[1] == "X" || vars[0] == "Y" || vars[1] == "Y") return std::nullopt;

    std::vector<Linear2Parts> rows;
    std::vector<std::string> out{
        "X = ln(" + vars[0] + "), Y = ln(" + vars[1] + ")",
        vars[0] + ">0, " + vars[1] + ">0"
    };
    ...
    for(auto const &txt : eq_texts) {
        auto eq = casio::parse_equation(a, txt);
        if(!eq) return std::nullopt;
        auto l = log_of_positive_side(a, eq->lhs, vars);
        auto r = log_of_positive_side(a, eq->rhs, vars);
        if(!l || !r) return std::nullopt;
        out.push_back("ln(" + format_expr(a, eq->lhs) + ") = ln(" + format_expr(a, eq->rhs) + ")");
        out.push_back(format_expr(a, *l) + " = " + format_expr(a, *r));
        NodeId residual = exact_eval_simplify(a, sub_node(a, *l, *r));
        auto row = linear2_parts(a, residual, "X", "Y");
        if(!row) return std::nullopt;
        rows.push_back(*row);
    }

    NodeId det = exact_eval_simplify(a, sub_node(a, casio::mul(a, {rows[0].a0, rows[1].a1}), casio::mul(a, {rows[1].a0, rows[0].a1})));
    if(casio::same_by_sig(a, det, zero_node(a))) return std::nullopt;
    NodeId Xv = exact_eval_simplify(a, casio::div(a, sub_node(a, casio::mul(a, {rows[0].a1, rows[1].c}), casio::mul(a, {rows[1].a1, rows[0].c})), det));
    NodeId Yv = exact_eval_simplify(a, casio::div(a, sub_node(a, casio::mul(a, {rows[1].a0, rows[0].c}), casio::mul(a, {rows[0].a0, rows[1].c})), det));
    NodeId xv = exact_eval_simplify(a, casio::power(a, casio::constant_e(a), Xv));
    NodeId yv = exact_eval_simplify(a, casio::power(a, casio::constant_e(a), Yv));
    if(auto sx = snap_ln_integer_exp(a, Xv)) { Xv = sx->first; xv = sx->second; }
    if(auto sy = snap_ln_integer_exp(a, Yv)) { Yv = sy->first; yv = sy->second; }
    out.push_back("X = " + format_expr(a, Xv));
    out.push_back("Y = " + format_expr(a, Yv));
    out.push_back(vars[0] + " = e^X = " + format_expr(a, xv));
    out.push_back(vars[1] + " = e^Y = " + format_expr(a, yv));
    out.push_back("(" + vars[0] + "," + vars[1] + ") = [(" + format_expr(a, xv) + "," + format_expr(a, yv) + ")]");
    return out;
}
...
static bool read_pow_plus_linear_var(Arena &a, Equation const &eq, std::string const &avar, std::string const &bvar, PowPlusLine &line)
{
    auto run = [&](NodeId sum, NodeId val) {
        auto rhs = as_num(a, val);
        if(!rhs) return false;
        std::vector<NodeId> terms;
        add_terms_flat(a, sum, terms);
        if(terms.size() != 2) return false;
        bool saw_b = false;
        std::optional<Rational> base;
        for(NodeId t : terms) {
            if(is_sym_var(a, t, bvar)) {
                saw_b = true;
                continue;
            }
            Node const &p = a.get(t);
            if(p.kind != NodeKind::Pow || !is_sym_var(a, p.b, avar)) return false;
            auto br = as_num(a, p.a);
            if(!br || br->num <= 0) return false;
            base = *br;
        }
        if(!saw_b || !base) return false;
        line = PowPlusLine{*base, *rhs};
        return true;
    };
    return run(eq.lhs, eq.rhs) || run(eq.rhs, eq.lhs);
}

static std::optional<Rational> snap_small_rational(double v)
{
    for(long long den = 1; den <= 200; ++den) {
        long long num = llround(v * (double)den);
        if(std::fabs(v - (double)num / (double)den) < 1e-9) {
            Rational r{num, den};
            r.normalize();
            return r;
        }
...
static std::optional<std::vector<std::string>> reciprocal_power_plus_var_system_route(Arena &a, std::string const &expr)
{
    std::vector<std::string> eq_texts, vars;
    if(!extract_system_expr_vars(expr, eq_texts, vars)) return std::nullopt;
    ...
    PowPlusLine l0, l1;
    if(!read_pow_plus_linear_var(a, *e0, vars[0], vars[1], l0) ||
       !read_pow_plus_linear_var(a, *e1, vars[0], vars[1], l1)) return std::nullopt;
    if(r_cmp(r_mul(l0.base, l1.base), Rational{1, 1}) != 0) return std::nullopt;
    if(r_cmp(l0.base, Rational{1, 1}) < 0) std::swap(l0, l1);
    if(r_cmp(l0.base, Rational{1, 1}) == 0) return std::nullopt;

    Rational diff = r_sub(l0.value, l1.value);
    Poly2 q{Rational{1, 1}, r_neg(diff), Rational{-1, 1}, true};
    auto roots = solve_poly2(a, q, "u");
    std::string base_txt = format_expr(a, a.num(l0.base));
    std::string inv_txt = format_expr(a, a.num(l1.base));
    std::vector<std::string> out{
        "u = (" + base_txt + ")^" + vars[0] + ", u>0",
        "(" + inv_txt + ")^" + vars[0] + " = 1/u",
        "u + " + vars[1] + " = " + format_rat_plain(l0.value),
        "1/u + " + vars[1] + " = " + format_rat_plain(l1.value),
        "u - 1/u = " + format_rat_plain(diff),
        format_expr(a, poly2_to_node(a, q, "u")) + " = 0"
    };

    std::vector<std::string> pairs;
    for(auto const &root : roots) {
        std::string rhs = sol_rhs(root);
        NodeId un = casio::simplify(a, casio::parse_expr(a, rhs));
        auto uv = eval_node(a, un, "x", 0.0);
        if(!uv) continue;
        if(*uv <= 0) {
            out.push_back("u = " + rhs + " rejected, u>0");
            continue;
        }
        if(auto r = snap_small_rational(*uv)) un = a.num(*r);
        NodeId av = exact_eval_simplify(a, casio::div(a, casio::fn(a, "log", un), casio::fn(a, "log", a.num(l0.base))));
        if(auto ai = eval_node(a, av, "x", 0.0)) {
            double k = std::round(*ai);
            if(std::fabs(*ai - k) < 1e-9) av = a.num(Rational{(long long)k, 1});
        }
        NodeId bv = exact_eval_simplify(a, sub_node(a, a.num(l0.value), un));
        out.push_back("u = " + format_expr(a, un));
        out.push_back(vars[0] + " = " + format_expr(a, av));
        out.push_back(vars[1] + " = " + format_expr(a, bv));
        pairs.push_back("(" + format_expr(a, av) + "," + format_expr(a, bv) + ")");
    }
    if(pairs.empty()) return std::nullopt;
...
{
    auto e = exp_like_arg(a, n);
    return e && is_sym_var(a, *e, var);
}

static std::optional<Rational> x_plus_exp_y_value(Arena &a, Equation const &eq, std::string const &xvar, std::string const &yvar)
{
    auto run = [&](NodeId sum, NodeId val) -> std::optional<Rational> {
        auto c = as_num(a, val);
        if(!c) return std::nullopt;
        Node const &s = a.get(sum);
        if(s.kind != NodeKind::Add || s.kids.size() != 2) return std::nullopt;
        bool saw_x = false, saw_exp = false;
        for(NodeId k : s.kids) {
            if(is_sym_var(a, k, xvar)) saw_x = true;
            else if(exp_of_var(a, k, yvar)) saw_exp = true;
            else return std::nullopt;
        }
        if(!saw_x || !saw_exp) return std::nullopt;
        return *c;
    };
    if(auto r = run(eq.lhs, eq.rhs)) return r;
    return run(eq.rhs, eq.lhs);
}

static std::optional<Rational> ln_square_equals_2y_shift(Arena &a, Equation const &eq, std::string const &xvar, std::string const &yvar)
{
    auto run = [&](NodeId lside, NodeId rside) -> std::optional<Rational> {
        Node const &ln = a.get(lside);
        if(ln.kind != NodeKind::Fn || ln.fkind != FnKind::Log) return std::nullopt;
        Node const &p = a.get(ln.a);
        if(p.kind != NodeKind::Pow) return std::nullopt;
        auto e = as_num(a, p.b);
        if(!e || e->num != 2 || e->den != 1) return std::nullopt;
        auto xb = symbolic_linear_parts(a, p.a, xvar);
        auto yr = symbolic_linear_parts(a, rside, yvar);
        if(!xb || !yr) return std::nullopt;
        auto xm = as_num(a, xb->m), xc = as_num(a, xb->c);
        auto ym = as_num(a, yr->m), yc = as_num(a, yr->c);
        if(!xm || !xc || !ym || !yc) return std::nullopt;
        if(r_cmp(*xm, Rational{1, 1}) != 0 || r_cmp(*ym, Rational{2, 1}) != 0 || !is_zero(*yc)) return std::nullopt;
        return *xc;
    };
    if(auto r = run(eq.lhs, eq.rhs)) return r;
    return run(eq.rhs, eq.lhs);
...
struct LnSquareYLine
{
    Rational shift{0, 1};
    NodeId constant = 0;
};

static std::optional<LnSquareYLine> ln_square_equals_2y_plus_const(Arena &a, Equation const &eq, std::string const &xvar, std::string const &yvar)
{
    NodeId residual = exact_eval_simplify(a, sub_node(a, eq.lhs, eq.rhs));
    std::vector<NodeId> terms, rest;
    add_terms_flat(a, residual, terms);
    std::optional<Rational> shift;
    for(NodeId t : terms) {
        Rational coef;
        NodeId body = 0;
        bool has = false;
        split_coeff_body(a, t, coef, body, has);
        Node const &ln = a.get(has ? body : t);
        if(!shift && r_cmp(coef, Rational{1, 1}) == 0 && ln.kind == NodeKind::Fn && ln.fkind == FnKind::Log) {
            Node const &p = a.get(ln.a);
            if(p.kind != NodeKind::Pow) return std::nullopt;
            auto e = as_num(a, p.b);
            auto xb = symbolic_linear_parts(a, p.a, xvar);
            if(!e || e->num != 2 || e->den != 1 || !xb) return std::nullopt;
            auto xm = as_num(a, xb->m), xc = as_num(a, xb->c);
            if(!xm || !xc || r_cmp(*xm, Rational{1, 1}) != 0) return std::nullopt;
            shift = *xc;
        }
        else rest.push_back(t);
    }
    if(!shift) return std::nullopt;
    NodeId rhs = exact_eval_simplify(a, neg_node(a, rest.empty() ? zero_node(a) : casio::add(a, rest)));
    auto yr = symbolic_linear_parts(a, rhs, yvar);
    if(!yr) return std::nullopt;
    auto ym = as_num(a, yr->m);
    if(!ym || r_cmp(*ym, Rational{2, 1}) != 0) return std::nullopt;
    return LnSquareYLine{*shift, yr->c};
}

static std::optional<std::vector<std::string>> exp_log_square_shift_system_route(Arena &a, std::string const &expr)
{
    std::vector<std::string> eq_texts, vars;
    ...
    auto e1 = casio::parse_equation(a, eq_texts[1]);
    if(!e0 || !e1) return std::nullopt;
    auto c = x_plus_exp_y_value(a, *e0, vars[0], vars[1]);
    auto line = ln_square_equals_2y_plus_const(a, *e1, vars[0], vars[1]);
    if(!c || !line) {
        c = x_plus_exp_y_value(a, *e1, vars[0], vars[1]);
        line = ln_square_equals_2y_plus_const(a, *e0, vars[0], vars[1]);
    }
    if(!c || !line) return std::nullopt;
    if(casio::same_by_sig(a, line->constant, zero_node(a))) return std::nullopt;

    NodeId q = exact_eval_simplify(a, casio::power(a, casio::constant_e(a), casio::div(a, line->constant, casio::num(a, 2))));
    NodeId C = a.num(*c);
    NodeId S = a.num(line->shift);
    NodeId xval = exact_eval_simplify(a, casio::div(a, casio::add(a, {casio::mul(a, {q, C}), neg_node(a, S)}), casio::add(a, {one_node(a), q})));
    NodeId ey = exact_eval_simplify(a, casio::div(a, casio::add(a, {C, S}), casio::add(a, {one_node(a), q})));
    if(auto r = as_num(a, ey); r && r->num <= 0) return std::nullopt;
    NodeId yval = casio::fn(a, "log", ey);
    std::string xps = format_expr(a, casio::add(a, {casio::sym(a, vars[0]), S}));
    return std::vector<std::string>{
        "e^" + vars[1] + " = " + format_rat_plain(*c) + " - " + vars[0],
        "ln((" + xps + ")^2) = 2*" + vars[1] + " + " + format_expr(a, line->constant),
        "(" + xps + ")^2 = e^(" + format_expr(a, line->constant) + ")*(e^" + vars[1] + ")^2",
        xps + " = " + format_expr(a, q) + "*(" + format_rat_plain(*c) + " - " + vars[0] + ")",
        vars[0] + " = " + format_expr(a, xval),
        vars[1] + " = " + format_expr(a, yval),
        "(" + vars[0] + "," + vars[1] + ") = [(" + format_expr(a, xval) + "," + format_expr(a, yval) + ")]"
    };
}

static std::optional<std::vector<std::string>> exp_log_square_system_route(Arena &a, std::string const &expr)
{
    std::vector<std::string> eq_texts, vars;
    if(!extract_system_expr_vars(expr, eq_texts, vars)) return std::nullopt;
    auto e0 = casio::parse_equation(a, eq_texts[0]);
    ...
    if(!e0 || !e1) return std::nullopt;
    auto c = x_plus_exp_y_value(a, *e0, vars[0], vars[1]);
    auto shift = ln_square_equals_2y_shift(a, *e1, vars[0], vars[1]);
    if(!c || !shift) {
        c = x_plus_exp_y_value(a, *e1, vars[0], vars[1]);
        shift = ln_square_equals_2y_shift(a, *e0, vars[0], vars[1]);
    }
    if(!c || !shift) return std::nullopt;

    Rational xval = r_div(r_sub(*c, *shift), Rational{2, 1});
    Rational arg = r_add(xval, *shift);
    if(arg.num <= 0) return std::nullopt;
    NodeId xv = casio::num(a, xval.num, xval.den);
    NodeId av = casio::num(a, arg.num, arg.den);
    std::string lin = format_expr(a, casio::add(a, {casio::sym(a, vars[0]), casio::num(a, shift->num, shift->den)}));
    std::vector<std::string> out{
        "ln((" + lin + ")^2) = 2*" + vars[1],
        "e^(2*" + vars[1] + ") = (" + lin + ")^2",
        "(e^" + vars[1] + ")^2 = (" + lin + ")^2",
        "e^" + vars[1] + " = " + lin,
        vars[0] + " + " + lin + " = " + format_rat_plain(*c),
        vars[0] + " = " + format_expr(a, xv),
        vars[1] + " = ln(" + format_expr(a, av) + ")",
        "(" + vars[0] + "," + vars[1] + ") = [(" + format_expr(a, xv) + ",ln(" + format_expr(a, av) + "))]"
    };
    return out;
}
...
)
{
    ...
    if(!inf_key(lo) && !inf_key(hi)) {
        auto ylo = eval_at_text(lo), yhi = eval_at_text(hi);
        if(!ylo || !yhi) return std::nullopt;
        bool increasing = r_mul(scale, p->a1).num > 0;
        auto xlo = parse_rational_text(lo), xhi = parse_rational_text(hi);
        auto arg_at = [&](std::optional<Rational> x) -> std::optional<Rational> {
            if(!x) return std::nullopt;
            return r_add(r_mul(p->a1, *x), p->a0);
        };
        auto azlo = arg_at(xlo), azhi = arg_at(xhi);
        bool lo_zero_open = azlo && azlo->num == 0 && lo_open;
        bool hi_zero_open = azhi && azhi->num == 0 && hi_open;
        std::string low = increasing ? *ylo : *yhi;
        std::string high = increasing ? *yhi : *ylo;
        bool low_open = increasing ? lo_open : hi_open;
        bool high_open = increasing ? hi_open : lo_open;
        bool low_unbounded = increasing ? lo_zero_open : hi_zero_open;
        bool high_unbounded = increasing ? hi_zero_open : lo_zero_open;
        steps.push_back("ln(u) monotone");
        if(low_unbounded || high_unbounded) {
            steps.push_back("u -> 0+");
            if(low_unbounded) return std::string("y ") + (high_open ? "< " : "<= ") + high;
            return std::string(low_open ? "y > " : "y >= ") + low;
        }
        steps.push_back("y(" + lo + ")=" + *ylo + ", y(" + hi + ")=" + *yhi);
        return low + (low_open ? " < y" : " <= y") + (high_open ? " < " : " <= ") + high;
    }
    std::string hk = hi;
...
static std::optional<std::string> log_positive_quadratic_range(Arena &a, NodeId n, std::string const &var, std::vector<std::string> &steps)
{
    ...
    if(x.kind != NodeKind::Fn || x.fkind != FnKind::Log) return std::nullopt;
    auto p = poly_of(a, x.a, var);
    if(!p || !p->ok || is_zero(p->a2) || p->a2.num <= 0) return std::nullopt;
    Rational disc = r_sub(r_mul(p->a1, p->a1), r_mul(Rational{4, 1}, r_mul(p->a2, p->a0)));
    if(disc.num >= 0) return std::nullopt;
    Rational minv = r_sub(p->a0, r_div(r_mul(p->a1, p->a1), r_mul(Rational{4, 1}, p->a2)));
    if(minv.num <= 0) return std::nullopt;
    Rational h = r_div(p->a1, r_mul(Rational{2, 1}, p->a2));
    steps.push_back(format_expr(a, x.a) + " = " + format_expr(a, casio::mul(a, {a.num(p->a2), casio::power(a, casio::add(a, {casio::sym(a, var), a.num(h)}), a.num(Rational{2, 1}))})) + " + " + format_expr(a, a.num(minv)));
    steps.push_back(format_expr(a, x.a) + " >= " + format_expr(a, a.num(minv)) + " > 0.");
    return "y >= ln(" + format_expr(a, a.num(minv)) + ")";
}
...
static std::optional<std::string> sqrt_quadratic_interval_range(
    Arena &a,
    NodeId n,
    std::string const &var,
    std::string const &lo,
...
)
{
    ...
    if(lo.empty() && hi.empty()) {
        ...
        add(xhi, !hi_open);
        Rational xv = r_div(r_neg(q->a1), r_mul(Rational{2, 1}, q->a2));
        bool ok = true, closed = true;
        if(xlo) {
            int c = r_cmp(xv, *xlo);
            if(c < 0 || (c == 0 && lo_open)) ok = false;
        }
        if(xhi) {
            int c = r_cmp(xv, *xhi);
            if(c > 0 || (c == 0 && hi_open)) ok = false;
        }
        if(ok) add(xv, closed);
    }
    if(cand.empty()) return std::nullopt;
    auto cmp_y = [](Cand const &u, Cand const &v) { return r_cmp(u.y, v.y) < 0; };
...
)
{
    ...
    append_rejected_by_domain(out, var, xs, valid);
    out.push_back("Answer: " + solution_list_line(var, valid));
    return out;
}
...
static std::optional<std::vector<std::string>> rational_root_substitution_route(Arena &a,
                                                                               NodeId rearr,
                                                                               std::string const &var,
                                                                               std::optional<double> lo = std::nullopt,
                                                                               std::optional<double> hi = std::nullopt,
...
)
{
    ...
    if(is_sym_var(a, rad, var)) {
        if(auto p = poly_of(a, rhs_iso, var); p && p->ok && is_zero(p->a2) && !is_zero(p->a1)) {
            Rational inv = r_div(Rational{1, 1}, p->a1);
            Poly2 uq{Rational{1, 1}, r_mul(Rational{-1, 1}, inv), r_mul(p->a0, inv), true};
            Rational D = r_sub(r_mul(uq.a1, uq.a1), r_mul(Rational{4, 1}, uq.a0));
    ...
    NodeId squared = casio::simplify(a, casio::add(a, {
        casio::power(a, rhs_iso, casio::num(a, 2)),
        casio::neg(a, rad)
    }));
    ...
    if(raw.empty()) {
        ...
        NodeId A = exact_eval_simplify(a, casio::power(a, rhs_lin->m, casio::num(a, 2)));
        NodeId B = exact_eval_simplify(a, casio::add(a, {
            casio::mul(a, {casio::num(a, 2), rhs_lin->m, rhs_lin->c}),
            casio::neg(a, rad_lin->m)
        }));
        NodeId C = exact_eval_simplify(a, casio::add(a, {
            casio::power(a, rhs_lin->c, casio::num(a, 2)),
            casio::neg(a, rad_lin->c)
        }));
        NodeId xs = casio::sym(a, var);
        NodeId eq = exact_eval_simplify(a, casio::add(a, {
            casio::mul(a, {A, casio::power(a, xs, casio::num(a, 2))}),
            casio::mul(a, {B, xs}),
            C
        }));
        std::vector<std::string> out;
...
static std::string simplify_log_power_div_text(std::string const &s)
{
    ...
    if(t.size() > 3 && t.compare(t.size() - 3, 3, "/-1") == 0) return "-" + t.substr(0, t.size() - 3);
...
static std::optional<std::string> log_power_solution_text(Rational target, Rational slope)
{
    ...
    auto fallback = [&]() -> std::string {
        ...
        if(mag.num < 0) {
            ...
            if(target.num == 1 && target.den > 1) {
                arg = r_div(Rational{1, 1}, arg);
                neg = false;
            }
        }
        std::string logt = "ln(" + format_rat_plain(arg) + ")";
        std::string s;
        if(mag.num == mag.den) s = logt;
        else if(mag.den == 1) s = format_rat_plain(mag) + "*" + logt;
        else if(mag.num == 1) s = logt + "/" + std::to_string(mag.den);
        else s = format_rat_plain(mag) + "*" + logt;
        return neg ? "-" + s : s;
    };
    if(std::llabs(coeff.num) != 1) return fallback();
    ...
    else if(target.den != 1) return fallback();
...
static std::optional<std::vector<std::string>> symbolic_linear_solve_route(Arena &a, NodeId rearr, std::string const &var)
{
    ...
    bool has_e_const = contains_const(a, rearr, ConstKind::E);
    if(!has_parameter && !has_pi) {
        if(!has_e_const && !contains_fn_kind(a, rearr, FnKind::Sqrt) && !contains_fn_kind(a, rearr, FnKind::Log)) return std::nullopt;
        auto rp = ratpoly_of_node(a, rearr, var);
        if(rp.ok) return std::nullopt;
    }
    NodeId ans = 0;
    if(is_negative_one_num(a, lin->m) || format_expr(a, lin->m) == "-1") {
        ans = exact_eval_simplify(a, lin->c);
    }
    else ans = casio::simplify(a, casio::div(a, casio::neg(a, lin->c), lin->m));
    Rational cm, cc;
    NodeId bm = 0, bc = 0;
    ...
    bool e_cancel_answer = contains_const(a, ans, ConstKind::E) &&
        !contains_const(a, ans, ConstKind::Pi) &&
        !contains_fn_kind(a, ans, FnKind::Sqrt) &&
        !contains_fn_kind(a, ans, FnKind::Log) &&
        !contains_fn_kind(a, ans, FnKind::Log10);
    if(e_cancel_answer && !has_symbols(a, ans)) {
        if(auto av = eval_node_env(a, ans, {}); av && std::isfinite(*av) && std::fabs(*av) < 1e12) {
            for(long long den = 1; den <= 24; ++den) {
                long long num = std::llround(*av * den);
                if(std::fabs(*av - (double)num / (double)den) < 1e-10) {
                    ans = casio::num(a, num, den);
                    break;
                }
            }
        }
...
static bool shifted_exp_base(Arena &a, NodeId n, NodeId arg, Rational &shift)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Add || x.kids.size() != 2) return false;
    bool saw_exp = false, saw_const = false, neg_exp = false;
    for(NodeId k : x.kids) {
        if(auto e = exp_like_arg(a, k); e && casio::same_by_sig(a, *e, arg)) saw_exp = true;
            ...
            bool has_body = false;
            split_coeff_body(a, k, coef, body, has_body);
            if(has_body && r_cmp(coef, Rational{-1, 1}) == 0) {
                if(auto e = exp_like_arg(a, body); e && casio::same_by_sig(a, *e, arg)) {
                    saw_exp = true;
                    neg_exp = true;
                    continue;
                }
            }
            if(auto r = as_num(a, k); r) {
                shift = *r;
                saw_const = true;
            }
            else return false;
...
)
{
    ...
    Rational lhs_coef{1, 1};
    NodeId lhs_body = 0;
    bool has_lhs_body = false;
    split_coeff_body(a, lhs, lhs_coef, lhs_body, has_lhs_body);
    if(!has_lhs_body) return std::nullopt;
    NodeId arg = 0;
    Rational shift{0, 1};
    if(!exp_over_shift_square(a, lhs_body, arg, shift)) return std::nullopt;
    auto p = poly_of(a, arg, var);
    if(!p || !p->ok || !is_zero(p->a2) || is_zero(p->a1)) return std::nullopt;
    ...
    std::string coef_txt = r_cmp(lhs_coef, Rational{1, 1}) == 0 ? "" : format_expr(a, a.num(lhs_coef)) + "*";
    out.push_back(coef_txt + uvar + "/(" + den + ")^2 = 1");
    out.push_back(coef_txt + uvar + " = (" + den + ")^2");
    Poly2 up{Rational{1, 1}, r_sub(r_mul(Rational{2, 1}, shift), lhs_coef), r_mul(shift, shift), true};
...
{
    Node const &x = a.get(arg);
    if(x.kind == NodeKind::Const && x.ckind == ConstKind::E) {
        add_prime_exp(logs, 1, coeff);
        return true;
    }
    if(x.kind == NodeKind::Num) {
    ...
    if(x.kind == NodeKind::Mul) {
        for(NodeId k : x.kids)
            if(!add_log_arg_factors(a, k, coeff, logs)) return false;
        return true;
    }
    if(x.kind == NodeKind::Div)
        return add_log_arg_factors(a, x.a, coeff, logs) &&
               add_log_arg_factors(a, x.b, r_neg(coeff), logs);
    if(x.kind == NodeKind::Fn && x.fkind == FnKind::Sqrt)
...
static std::string log_factor_terms_text(PrimeExps logs)
{
    ...
    for(auto const &pe : logs) {
        ...
        if(neg) e = r_neg(e);
        std::string term;
        if(pe.first == 1) term = format_rat_plain(e);
        else if(r_cmp(e, Rational{1, 1}) == 0) term = "ln(" + std::to_string(pe.first) + ")";
...
static bool add_log_term_factors(Arena &a, NodeId term, Rational side, PrimeExps &xlogs, PrimeExps &clogs, std::string const &var)
{
    ...
    auto add_log = [&](NodeId id, PrimeExps &dst) {
        ...
        return add_log_arg_factors(a, n.a, c, dst);
    };
...
static bool monomial_coeff_power(Arena &a, NodeId n, std::string const &var, Rational &coef, int &power)
{
    ...
    for(NodeId f : factors) {
        ...
        if(is_sym_var(a, f, var)) {
            ++power;
            continue;
        }
        Node const &x = a.get(f);
        if(x.kind == NodeKind::Pow && is_sym_var(a, x.a, var)) {
            auto e = as_num(a, x.b);
            if(!e || e->den != 1 || e->num <= 0 || e->num > 12) return false;
            power += (int)e->num;
            continue;
        }
        return false;
...
static std::optional<std::vector<std::string>> log_linear_symbolic_route(Arena &a, NodeId rearr, std::string const &var)
{
    ...
    for(NodeId term : terms) {
        ...
        if(has_body) {
            NodeId a0 = 0, b0 = 0;
            bool has_base = false;
            if(log_piece(a, body, a0, b0, has_base)) {
                if(!saw) {
                    saw = true;
                    arg = a0;
                    natural = !has_base;
    ...
    NodeId rest = rest_terms.empty() ? zero_node(a) : casio::simplify(a, casio::add(a, rest_terms));
    NodeId rhs1 = exact_eval_simplify(a, casio::neg(a, rest));
    NodeId target = exact_eval_simplify(a, casio::div(a, rhs1, a.num(coeff)));
    NodeId value = exact_eval_simplify(a, natural ? casio::power(a, a.constant(ConstKind::E), target)
                                                  : casio::power(a, base, target));
    NodeId ans = exact_eval_simplify(a, casio::div(a, sub_node(a, value, lin->c), lin->m));

    std::string log_txt = natural ? "ln(" + format_expr(a, arg) + ")"
                                  : "log(" + format_expr(a, base) + "," + format_expr(a, arg) + ")";
    std::string left = coeff.num == coeff.den ? log_txt :
        (coeff.num == -coeff.den ? "-" + log_txt : format_rat_plain(coeff) + "*" + log_txt);
    std::vector<std::string> out;
    out.push_back("Domain: " + format_expr(a, arg) + " > 0");
    out.push_back(left + " = " + format_expr(a, rhs1));
    if(coeff.num != coeff.den) out.push_back(log_txt + " = " + format_expr(a, target));
    out.push_back(format_expr(a, arg) + " = " + format_expr(a, value));
    std::string sol = var + " = " + format_expr(a, ans);
    out.push_back(sol);
    std::vector<std::string> syms;
    collect_symbols(a, ans, syms);
    bool param = false;
    for(auto const &s : syms) if(s != "e") param = true;
    if(!param) out.push_back(solution_list_line(var, {sol}));
    return out;
}
...
static std::optional<std::string> positive_log_power_root(Arena &a, Rational k, Rational c, int p)
{
    ...
    auto rn = integer_nth_root_i64(k.num, p);
    auto rd = integer_nth_root_i64(k.den, p);
    if(!rn || !rd || *rd == 0) return std::nullopt;
    Rational den{*rn, *rd};
    den.normalize();
    Rational ec{c.num, p};
    ec.normalize();
    std::string top = exp_e_text(ec);
    if(den.num == den.den) return top;
    return top + "/" + format_rat_plain(den);
}

static std::optional<std::vector<std::string>> log_product_power_solve_route(
    Arena &a,
    NodeId lhs,
    NodeId rhs,
    std::string const &var
...
{
    auto run = [&](NodeId log_side, NodeId val_side) -> std::optional<std::vector<std::string>> {
        auto val = as_num(a, val_side);
        if(!val) return std::nullopt;
        std::vector<NodeId> terms;
        add_terms_flat(a, log_side, terms);
        if(terms.size() < 2) return std::nullopt;
        Rational k{1, 1}, pow_sum{0, 1};
        ...
        for(NodeId term : terms) {
            ...
            bool has_body = true;
            split_coeff_body(a, term, c, body, has_body);
            if(!has_body || c.den != 1 || c.num <= 0 || c.num > 12) return std::nullopt;
            Node const &ln = a.get(body);
            if(ln.kind != NodeKind::Fn || ln.fkind != FnKind::Log) return std::nullopt;
            Rational coef;
            int p = 0;
            if(!monomial_coeff_power(a, ln.a, var, coef, p)) return std::nullopt;
            k = r_mul(k, r_pow_int(coef, (int)c.num));
            pow_sum = r_add(pow_sum, Rational{p * c.num, 1});
            if(p % 2) odd_domain = true;
            args.push_back(format_expr(a, ln.a));
        }
        if(pow_sum.den != 1 || pow_sum.num <= 0 || pow_sum.num > 12) return std::nullopt;
        int p = (int)pow_sum.num;
        std::string kt = format_rat_plain(k);
        std::string xp = p == 1 ? var : var + "^" + std::to_string(p);
        std::vector<std::string> out;
        out.push_back(format_expr(a, log_side) + " = " + format_expr(a, val_side));
        out.push_back("Domain: " + var + (odd_domain ? " > 0" : " != 0"));
        out.push_back("ln(" + join_text(args, "*") + ") = " + format_expr(a, val_side));
        out.push_back("ln(" + kt + "*" + xp + ") = " + format_expr(a, val_side));
        out.push_back(kt + "*" + xp + " = " + exp_e_text(*val));
        out.push_back(xp + " = " + exp_e_text(*val) + "/" + kt);
        auto root = positive_log_power_root(a, k, *val, p);
        if(!root) return std::nullopt;
        out.push_back(var + " = " + *root);
        out.push_back(solution_list_line(var, {var + " = " + *root}));
        return out;
    };
    if(auto r = run(lhs, rhs)) return r;
    return run(rhs, lhs);
...
static std::optional<NodeId> cos_exact_alpha_node(Arena &a, NodeId rhs)
{
    auto v = eval_node_env(a, rhs, {});
    if(!v) return std::nullopt;
    NodeId pi = a.constant(ConstKind::Pi);
    auto over = [&](int d) { return casio::div(a, pi, casio::num(a, d)); };
    double r = *v;
    if(std::fabs(r - 1.0) < 1e-9) return casio::num(a, 0);
    if(std::fabs(r - std::sqrt(3.0) / 2.0) < 1e-9) return over(6);
    if(std::fabs(r - std::sqrt(2.0) / 2.0) < 1e-9) return over(4);
    if(std::fabs(r - 0.5) < 1e-9) return over(3);
    if(std::fabs(r) < 1e-9) return over(2);
    if(std::fabs(r + 0.5) < 1e-9) return casio::mul(a, {casio::num(a, 2), over(3)});
    if(std::fabs(r + std::sqrt(2.0) / 2.0) < 1e-9) return casio::mul(a, {casio::num(a, 3), over(4)});
    if(std::fabs(r + std::sqrt(3.0) / 2.0) < 1e-9) return casio::mul(a, {casio::num(a, 5), over(6)});
    if(std::fabs(r + 1.0) < 1e-9) return pi;
    return std::nullopt;
}

static std::optional<std::vector<std::string>> trig_quadratic_arg_exact_solve(
    Arena &a, NodeId lhs, NodeId rhs, std::string const &var)
{
    NodeId arg = 0, val = 0;
    Node const &L = a.get(lhs);
    Node const &R = a.get(rhs);
    if(L.kind == NodeKind::Fn && L.fkind == FnKind::Cos) { arg = L.a; val = rhs; }
    else if(R.kind == NodeKind::Fn && R.fkind == FnKind::Cos) { arg = R.a; val = lhs; }
    else {
        auto from_residual = [&](NodeId res) -> bool {
            std::vector<NodeId> terms, rest;
            add_terms_flat(a, res, terms);
            NodeId coef = 0;
            auto cos_term = [&](NodeId t) -> std::optional<std::pair<NodeId, NodeId>> {
                Node const &n = a.get(t);
                if(n.kind == NodeKind::Fn && n.fkind == FnKind::Cos)
                    return std::make_pair(n.a, one_node(a));
                if(n.kind != NodeKind::Mul) return std::nullopt;
                NodeId found = 0;
                std::vector<NodeId> coeffs;
                for(NodeId k : n.kids) {
                    Node const &kid = a.get(k);
                    if(kid.kind == NodeKind::Fn && kid.fkind == FnKind::Cos && found == 0) {
                        found = kid.a;
                        continue;
                    }
                ...
                if(!found) return std::nullopt;
                return std::make_pair(found, mul_or_one(a, coeffs));
            };
            for(NodeId t : terms) {
                if(!arg) {
                    if(auto ct = cos_term(t)) {
                        arg = ct->first;
                        coef = ct->second;
                        continue;
                    }
            ...
            if(!arg) return false;
            NodeId r = rest.empty() ? zero_node(a) : casio::simplify(a, casio::add(a, rest));
            val = exact_eval_simplify(a, casio::div(a, casio::neg(a, r), coef ? coef : one_node(a)));
            return true;
        };
        if(casio::same_by_sig(a, rhs, zero_node(a))) {
            if(!from_residual(lhs)) return std::nullopt;
        }
        else if(casio::same_by_sig(a, lhs, zero_node(a))) {
            if(!from_residual(rhs)) return std::nullopt;
        }
        else return std::nullopt;
    }
    auto alpha = cos_exact_alpha_node(a, val);
    NodeId qa2 = 0, qb = 0, qc = 0;
    if(auto q = symbolic_quadratic_parts(a, arg, var)) {
        qa2 = q->a2;
        qb = q->b;
        qc = q->c;
    }
    else if(auto p = poly_of(a, arg, var); p && p->ok && !is_zero(p->a2)) {
        qa2 = a.num(p->a2);
        qb = a.num(p->a1);
        qc = a.num(p->a0);
    }
    ...
    NodeId qb_s = exact_eval_simplify(a, qb);
    if(!alpha || !(casio::same_by_sig(a, qb_s, zero_node(a)) || is_num_node(a, qb_s, 0)))
        return std::nullopt;
    NodeId two_pi = casio::mul(a, {casio::num(a, 2), a.constant(ConstKind::Pi)});
    std::vector<NodeId> targets{*alpha};
    if(!casio::same_by_sig(a, *alpha, zero_node(a)) && !casio::same_by_sig(a, *alpha, a.constant(ConstKind::Pi)))
        targets.push_back(casio::simplify(a, sub_node(a, two_pi, *alpha)));

    std::vector<std::string> out{format_expr(a, lhs) + " = " + format_expr(a, rhs)};
    out.push_back("alpha = arccos(" + format_expr(a, val) + ") = " + format_expr(a, *alpha));
    std::vector<std::string> sols;
    for(NodeId t : targets) {
        NodeId x2 = exact_eval_simplify(a, casio::div(a, sub_node(a, t, qc), qa2));
        auto x2v = eval_node_env(a, x2, {});
        out.push_back(format_expr(a, arg) + " = " + format_expr(a, t));
        out.push_back(var + "^2 = " + format_expr(a, x2));
        if(!x2v || *x2v < -1e-10) {
            out.push_back(format_expr(a, x2) + " < 0");
            continue;
        }
        NodeId root = exact_eval_simplify(a, casio::fn(a, "sqrt", x2));
        std::string rt = format_expr(a, root);
        sols.push_back(var + " = " + rt);
        if(!casio::same_by_sig(a, root, zero_node(a))) sols.push_back(var + " = -" + rt);
    }
    if(sols.empty()) out.push_back(var + " = []");
    else out.push_back(solution_list_line(var, sols));
    return out;
}
...
static std::optional<NodeId> exp_arg_node(Arena &a, NodeId n)
{
    ...
    if(x.kind == NodeKind::Fn && x.fkind == FnKind::Exp) return x.a;
    if(x.kind == NodeKind::Pow) {
        if(is_e_const_node(a, x.a)) return x.b;
    }
    return std::nullopt;
}
...
static bool parse_recip_affine_factor_term(Arena &a, NodeId term, std::string const &var, RecipAffineFactorTerm &out)
{
    std::vector<NodeId> factors, rest;
    collect_mul_factors(a, term, factors);
    for(NodeId f : factors) {
        ...
        if(x.kind == NodeKind::Pow) {
            auto e = as_num(a, x.b);
            if(e && e->den == 1 && e->num < 0 && (e->num == -2 || e->num == -4)) {
                auto lin = symbolic_linear_parts(a, x.a, var);
                if(!lin || out.den_base) return false;
                out.den_base = x.a;
                out.den_pow = (int)-e->num;
                continue;
            }
        }
        rest.push_back(f);
    }
    if(!out.den_base) return false;
    NodeId num_part = mul_or_one(a, rest);
    bool has_body = false;
    split_coeff_body(a, num_part, out.coef, out.body, has_body);
    return has_body && contains_symbol(a, out.body, var);
}

static std::optional<std::vector<std::string>> common_affine_recip_power_factor_route(
    Arena &a,
    NodeId residual,
    std::string const &var,
    std::vector<std::string> out
)
{
    std::vector<NodeId> terms;
    add_terms_flat(a, residual, terms);
    if(terms.size() < 2 || terms.size() > 5) return std::nullopt;

    for(std::size_t i = 0; i < terms.size(); ++i) {
        RecipAffineFactorTerm t;
        if(!parse_recip_affine_factor_term(a, terms[i], var, t)) continue;
        std::vector<NodeId> rest_terms;
        ...
        auto p_body = poly_of(a, t.body, var);
        auto p_rest = poly_of(a, rest, var);
        if(!p_body || !p_rest || !p_body->ok || !p_rest->ok) continue;
        auto scale = proportional_scale_poly2(*p_rest, *p_body);
        if(!scale || is_zero(*scale)) continue;
        Rational target = r_div(r_neg(t.coef), *scale);
        if(target.num <= 0) continue;
        auto den_lin = symbolic_linear_parts(a, t.den_base, var);
        if(!den_lin) continue;

        std::string den_txt = format_expr(a, t.den_base);
        std::string body_txt = format_expr(a, t.body);
        std::string coef_txt = format_expr(a, a.num(t.coef));
        Rational abs_scale = *scale;
        bool scale_neg = abs_scale.num < 0;
        if(scale_neg) abs_scale = r_neg(abs_scale);
        std::string inner = coef_txt + "*(" + den_txt + ")^-" + std::to_string(t.den_pow) +
            (scale_neg ? " - " : " + ") + format_expr(a, a.num(abs_scale));

        out.push_back("Domain: " + den_txt + " != 0");
        out.push_back(format_expr(a, residual) + " = 0");
        out.push_back("(" + body_txt + ")*(" + inner + ") = 0");
        out.push_back(body_txt + " = 0");

        std::vector<std::string> raw = solve_poly2(a, *p_body, var);
        out.push_back("(" + den_txt + ")^" + std::to_string(t.den_pow) + " = " + format_expr(a, a.num(target)));
        NodeId root = 0;
        if(t.den_pow == 2) root = exact_eval_simplify(a, casio::fn(a, "sqrt", a.num(target)));
        else if(t.den_pow == 4) root = exact_eval_simplify(a, casio::fn(a, "sqrt", casio::fn(a, "sqrt", a.num(target))));
        else return std::nullopt;
        out.push_back(den_txt + " = +/-" + format_expr(a, root));
        NodeId plus = exact_eval_simplify(a, casio::div(a, sub_node(a, root, den_lin->c), den_lin->m));
        NodeId minus = exact_eval_simplify(a, casio::div(a, sub_node(a, casio::neg(a, root), den_lin->c), den_lin->m));
        raw.push_back(var + " = " + format_expr(a, plus));
        raw.push_back(var + " = " + format_expr(a, minus));
        raw = filter_real_solutions(a, residual, var, raw, std::nullopt, std::nullopt);
...
static bool exp_term_with_general_arg(Arena &a, NodeId term, std::string const &var, NodeId &coef, NodeId &arg)
{
    std::vector<NodeId> factors;
    collect_mul_factors(a, term, factors);
    std::vector<NodeId> coeffs;
    bool seen = false;
    for(NodeId f : factors) {
        if(auto e = exp_arg_or_recip_arg(a, f)) {
            if(!contains_symbol(a, *e, var)) {
                coeffs.push_back(f);
                continue;
            }
            if(seen) return false;
...
static std::optional<int> integer_arg_multiple(Arena &a, NodeId arg, NodeId base)
{
    for(int k = -6; k <= 6; ++k) {
        if(k == 0) continue;
        NodeId diff = exact_eval_simplify(a, expand_square_powers(a, sub_node(a, arg, casio::mul(a, {casio::num(a, k), base}))));
        if(casio::same_by_sig(a, diff, zero_node(a))) return k;
    }
    return std::nullopt;
}

static std::optional<std::vector<std::string>> exp_general_arg_substitution_route(
    Arena &a,
    NodeId residual,
    std::string const &var,
    std::vector<std::string> out
)
{
    std::vector<NodeId> terms;
    add_terms_flat(a, residual, terms);
    if(terms.size() < 3 || terms.size() > 6) return std::nullopt;

    struct ET { NodeId coef = 0; NodeId arg = 0; };
    std::vector<ET> es;
    std::vector<NodeId> consts;
    for(NodeId t : terms) {
        NodeId coef = 0, arg = 0;
        if(exp_term_with_general_arg(a, t, var, coef, arg)) {
            es.push_back(ET{coef, arg});
            continue;
        }
        if(contains_symbol(a, t, var)) return std::nullopt;
        consts.push_back(t);
    }
    if(es.size() < 2) return std::nullopt;
    ...
    for(auto const &cand : es) {
        ...
        bool ok = true;
        for(auto const &t : es) {
            auto k = integer_arg_multiple(a, t.arg, cand.arg);
            if(!k) {
                ok = false;
                break;
            }
        ...
        if(!ok || mn < 0 || mx > 2) continue;
        NodeId cand_poly_arg = exact_eval_simplify(a, expand_square_powers(a, cand.arg));
        auto pbase = poly_of(a, cand_poly_arg, var);
        if(!pbase || !pbase->ok || is_zero(pbase->a2)) continue;

        std::string uvar = var == "u" ? "v" : "u";
        std::vector<NodeId> coeffs(3, zero_node(a));
        for(std::size_t i = 0; i < es.size(); ++i)
            coeffs[(std::size_t)powers[i]] = exact_eval_simplify(a, casio::add(a, {coeffs[(std::size_t)powers[i]], es[i].coef}));
        if(!consts.empty()) coeffs[0] = exact_eval_simplify(a, casio::add(a, {coeffs[0], exact_eval_simplify(a, casio::add(a, consts))}));
        if(casio::same_by_sig(a, coeffs[2], zero_node(a))) continue;

        NodeId u = casio::sym(a, uvar);
        NodeId upoly = exact_eval_simplify(a, casio::add(a, {
            casio::mul(a, {coeffs[2], casio::power(a, u, casio::num(a, 2))}),
            casio::mul(a, {coeffs[1], u}),
            coeffs[0]
        }));
        NodeId at_one = exact_eval_simplify(a, clone_with_substitution(a, upoly, uvar, one_node(a)));
        if(!casio::same_by_sig(a, at_one, zero_node(a))) continue;

        NodeId root2 = exact_eval_simplify(a, casio::div(a, coeffs[0], coeffs[2]));
        auto r2v = eval_node_env(a, root2, {});
        if(r2v && *r2v <= 0) continue;

        std::vector<std::string> raw;
        std::vector<NodeId> roots{one_node(a), root2};
        out.push_back(uvar + " = e^(" + format_expr(a, cand.arg) + "), " + uvar + " > 0");
        out.push_back(format_expr(a, upoly) + " = 0");
        out.push_back("(" + uvar + " - 1)(" + uvar + " - " + format_expr(a, root2) + ") = 0");
        for(NodeId r : roots) {
            NodeId target = exact_eval_simplify(a, casio::fn(a, "ln", r));
            out.push_back(uvar + " = " + format_expr(a, r));
            out.push_back(format_expr(a, cand.arg) + " = " + format_expr(a, target));
            NodeId eq = exact_eval_simplify(a, expand_square_powers(a, sub_node(a, cand.arg, target)));
            auto p = poly_of(a, eq, var);
            if(!p || !p->ok || is_zero(p->a2)) return std::nullopt;
            auto sols = solve_poly2(a, *p, var);
            for(auto const &s : sols) raw.push_back(s);
        }
        raw = filter_real_solutions(a, residual, var, raw, std::nullopt, std::nullopt);
        if(raw.empty()) continue;
        sort_solution_lines(a, raw);
...
)
{
    ...
    for(NodeId term : terms) {
        ...
        auto p = poly_of(a, *arg, var);
        if(!p || !p->ok || !is_zero(p->a2) || is_zero(p->a1)) return std::nullopt;
        es.push_back(ExpPowerTerm{coef, p->a1, p->a0});
    }
    if(es.empty()) return std::nullopt;

    Rational base = es.front().arg_coef;
    Rational base_offset = es.front().arg_offset;
    ...
    for(auto const &cand : es) {
        ...
        for(auto const &t : es) {
            ...
            if(!q || std::llabs(*q) > 4 ||
               r_cmp(t.arg_offset, r_mul(cand.arg_offset, Rational{*q, 1})) != 0) {
                ok = false;
                break;
            }
            ps.push_back(*q);
    ...
    if(!found) {
        ...
        bool ok = true;
        for(auto const &t : es) {
            auto q = rational_int(t.arg_coef);
            if(!q || !is_zero(t.arg_offset) || *q == 0) {
                ok = false;
                break;
            }
            g = g ? gcd_abs_ll(g, *q) : std::llabs(*q);
        }
        if(ok && g > 0) {
            base = Rational{g, 1};
            base_offset = Rational{0, 1};
            powers.clear();
            for(auto const &t : es) powers.push_back(*rational_int(r_div(t.arg_coef, base)));
            found = true;
        }
    }
    if(!found) return std::nullopt;
    ...
    NodeId base_arg = casio::simplify(a, casio::add(a, {casio::mul(a, {a.num(base), a.sym(var)}), a.num(base_offset)}));
    ...
    for(auto const &s : us) {
        ...
        NodeId log_arg = root;
        if(is_zero(base_offset) && pos_base.den == 1 && pos_base.num > 1) {
        ...
        NodeId log_root = is_num_node(a, log_arg, 1) ? zero_node(a) : casio::fn(a, "log", log_arg);
        if(neg_slope) {
...
)
{
    ...
    add_terms_flat(a, residual, terms);
    if(terms.size() != 2) return std::nullopt;
    Rational cst{0, 1}, coef{0, 1};
    NodeId body = 0;
    for(NodeId t : terms) {
        Rational c;
        NodeId b = 0;
        bool has = false;
        split_coeff_body(a, t, c, b, has);
        if(has) {
            if(body) return std::nullopt;
            coef = c;
            body = b;
        }
        else cst = r_add(cst, c);
    }
    if(!body || is_zero(coef) || is_zero(cst)) return std::nullopt;
    Rational target = r_div(r_neg(cst), coef);
    if(target.num <= 0) return std::nullopt;

    std::vector<NodeId> factors;
    collect_mul_factors(a, body, factors);
    NodeId slope = zero_node(a), offset = zero_node(a);
    int exp_factors = 0, mixed = 0;
    for(NodeId f : factors) {
        NodeId logbase = 0;
        std::optional<NodeId> arg = exp_arg_node(a, f);
        if(arg) {
            logbase = one_node(a);
        }
        else {
            ...
            auto ib = positive_integer_node(a, fn.a);
            if(!ib) return std::nullopt;
            arg = fn.b;
            logbase = casio::fn(a, "log", a.num(Rational{*ib, 1}));
            ++mixed;
        }
        auto p = poly_of(a, *arg, var);
        if(!p || !p->ok || !is_zero(p->a2)) return std::nullopt;
        if(is_zero(p->a1) && is_zero(p->a0)) return std::nullopt;
        NodeId m = logbase == one_node(a) ? one_node(a) : logbase;
        if(!is_zero(p->a1)) slope = casio::add(a, {slope, casio::mul(a, {a.num(p->a1), m})});
        if(!is_zero(p->a0)) offset = casio::add(a, {offset, casio::mul(a, {a.num(p->a0), m})});
        ++exp_factors;
    }
    if(exp_factors < 2 || mixed == 0) return std::nullopt;
    slope = casio::simplify(a, slope);
    offset = casio::simplify(a, offset);
    if(casio::same_by_sig(a, slope, zero_node(a))) return std::nullopt;

    std::string lhs = format_expr(a, body);
    std::string rhs = format_expr(a, a.num(target));
    out.push_back(lhs + " = " + rhs);
    out.push_back("ln(" + lhs + ") = ln(" + rhs + ")");
    NodeId log_rhs = casio::fn(a, "log", a.num(target));
    NodeId num = casio::simplify(a, casio::add(a, {log_rhs, neg_node(a, offset)}));
    NodeId ans = casio::simplify(a, casio::div(a, num, slope));
    out.push_back(var + "*(" + format_expr(a, slope) + ") + " + format_expr(a, offset) + " = " + format_expr(a, log_rhs));
    out.push_back(var + " = " + format_expr(a, ans));
...
static std::optional<Rational> log_x_multiplier(Arena &a, NodeId n, std::string const &var)
{
    Node const &x = a.get(n);
    if(x.kind != NodeKind::Fn || x.fkind != FnKind::Log) return std::nullopt;
    if(is_sym_var(a, x.a, var)) return Rational{1, 1};
    Node const &arg = a.get(x.a);
    if(arg.kind == NodeKind::Pow && is_sym_var(a, arg.a, var)) {
        auto e = as_num(a, arg.b);
        if(e && e->den == 1 && e->num > 0 && e->num <= 8) return *e;
    }
    return std::nullopt;
}

static bool add_log_x_arg(Arena &a, NodeId n, Rational c, std::string const &var, Rational &ucoef, Rational &cst)
{
    Node const &x = a.get(n);
    if(x.kind == NodeKind::Sym && x.text == var) {
        ucoef = r_add(ucoef, c);
    ...
    if(x.kind == NodeKind::Num) return x.num.num == x.num.den;
    if(x.kind == NodeKind::Mul) {
        for(NodeId k : x.kids)
            if(!add_log_x_arg(a, k, c, var, ucoef, cst)) return false;
        return true;
    }
    if(x.kind == NodeKind::Div)
        return add_log_x_arg(a, x.a, c, var, ucoef, cst) &&
               add_log_x_arg(a, x.b, r_neg(c), var, ucoef, cst);
    if(x.kind == NodeKind::Fn && x.fkind == FnKind::Sqrt)
        return add_log_x_arg(a, x.a, r_div(c, Rational{2, 1}), var, ucoef, cst);
    if(x.kind == NodeKind::Pow) {
        auto e = as_num(a, x.b);
        if(!e) return false;
        return add_log_x_arg(a, x.a, r_mul(c, *e), var, ucoef, cst);
    }
    return false;
}

static std::optional<PolyAny> log_x_poly(Arena &a, NodeId n, std::string const &var)
{
    Node const &x = a.get(n);
    if(x.kind == NodeKind::Fn && x.fkind == FnKind::Log) {
        Rational u{0, 1}, c{0, 1};
        if(!add_log_x_arg(a, x.a, Rational{1, 1}, var, u, c)) return std::nullopt;
        return PolyAny{{c, u}, true};
    }
    if(x.kind == NodeKind::Pow) {
        auto e = as_num(a, x.b);
        auto p = log_x_poly(a, x.a, var);
        if(!e || e->den != 1 || e->num < 0 || e->num > 6 || !p) return std::nullopt;
        PolyAny out{{Rational{1, 1}}, true};
        for(long long i = 0; i < e->num; ++i) out = poly_any_mul(out, *p);
        return out;
    }
    if(x.kind == NodeKind::Mul) {
        PolyAny out{{Rational{1, 1}}, true};
        for(NodeId k : x.kids) {
            auto p = log_x_poly(a, k, var);
            if(!p) return std::nullopt;
            out = poly_any_mul(out, *p);
        }
        return out;
...
static std::optional<std::vector<std::string>> log_poly_substitution_route(
    Arena &a, NodeId residual, std::string const &var, std::vector<std::string> out)
{
    ...
    for(NodeId t : terms) {
        ...
        split_coeff_body(a, t, coef, body, has);
        if(!has) {
            poly.c[0] = r_add(poly.c[0], coef);
            continue;
        }
        auto p = log_x_poly(a, body, var);
        if(!p || p->c.size() > 7) return std::nullopt;
        for(auto &q : p->c) q = r_mul(q, coef);
        poly = poly_any_add(poly, *p);
    }
    trim_poly_any(poly);
    if(poly.c.size() < 2 || poly.c.size() > 7) return std::nullopt;
    std::string uvar = var == "u" ? "v" : "u";
    out.push_back("Domain: " + var + " > 0");
    out.push_back(uvar + " = ln(" + var + ")");
    out.push_back(format_expr(a, poly_any_to_node(a, poly, uvar)) + " = 0");
    std::vector<std::string> factor_lines;
    auto us = solve_poly_any_raw(a, poly, uvar, factor_lines);
    for(auto const &line : factor_lines) out.push_back(line);
    if(us.empty()) return std::nullopt;
    std::vector<std::string> xs;
    for(auto const &s : us) {
        out.push_back(s);
        xs.push_back(var + " = " + exp_of_text(sol_rhs(s)));
    }
    for(auto const &s : xs) out.push_back(s);
    out.push_back(solution_list_line(var, xs));
    return out;
...
static std::optional<std::vector<std::string>> log_substitution_route(
    Arena &a,
    NodeId residual,
    std::string const &var,
    std::vector<std::string> out
)
{
    std::vector<NodeId> terms;
    add_terms_flat(a, residual, terms);
    std::vector<std::pair<int, Rational>> parts;
    Rational cst{0, 1};
    for(NodeId t : terms) {
        Rational coef;
        NodeId body = 0;
        ...
        int pow = 0;
        Rational scale{1, 1};
        if(auto m = log_x_multiplier(a, body, var)) {
            pow = 1;
            scale = *m;
        }
        else {
            Node const &bn = a.get(body);
            if(bn.kind == NodeKind::Pow) {
                auto e = as_num(a, bn.b);
                auto m = log_x_multiplier(a, bn.a, var);
                if(!e || e->den != 1 || !m || std::llabs(e->num) > 6) return std::nullopt;
                pow = (int)e->num;
                scale = *m;
            }
            else if(bn.kind == NodeKind::Div) {
                auto one = as_num(a, bn.a);
                auto m = log_x_multiplier(a, bn.b, var);
                if(!one || one->num != one->den || !m) return std::nullopt;
                pow = -1;
                scale = r_div(Rational{1, 1}, *m);
            }
            else return std::nullopt;
        }
        parts.push_back({pow, r_mul(coef, scale)});
    }
    if(parts.empty()) return std::nullopt;
    int mn = 0, mx = 0;
    for(auto const &p : parts) {
    ...
    int shift = mn < 0 ? -mn : 0;
    if(mx + shift > 6) return std::nullopt;
    PolyAny poly{std::vector<Rational>((std::size_t)(mx + shift + 1), Rational{0, 1}), true};
    for(auto const &p : parts) poly.c[(std::size_t)(p.first + shift)] = r_add(poly.c[(std::size_t)(p.first + shift)], p.second);
    poly.c[(std::size_t)shift] = r_add(poly.c[(std::size_t)shift], cst);
    trim_poly_any(poly);
    if(poly.c.size() < 2) return std::nullopt;
...
static std::optional<std::vector<std::string>> exp_rational_substitution_route(
    Arena &a,
    NodeId residual,
    std::string const &var,
    std::vector<std::string> out
)
{
    ...
    add_terms_flat(a, residual, terms);
    Rational A{0, 1}, B{0, 1}, C{0, 1}, D{0, 1};
    bool saw_u = false, saw_frac = false;
    for(NodeId t : terms) {
        Rational coef;
        NodeId body = 0;
        bool has = false;
        ...
        if(auto arg = exp_arg_node(a, body)) {
            auto p = poly_of(a, *arg, var);
            if(!p || !p->ok || !is_zero(p->a2) || !is_zero(p->a0) || r_cmp(p->a1, Rational{1, 1}) != 0) return std::nullopt;
            A = r_add(A, coef);
            saw_u = true;
            continue;
        }
        Node const &bn = a.get(body);
        if(bn.kind != NodeKind::Div) return std::nullopt;
        auto one = as_num(a, bn.a);
        if(!one || one->num != one->den) return std::nullopt;
        std::vector<NodeId> ds;
        add_terms_flat(a, bn.b, ds);
        Rational d{0, 1};
        bool got_exp = false;
        for(NodeId q : ds) {
            Rational qc;
            NodeId qb = 0;
            bool qhas = false;
            split_coeff_body(a, q, qc, qb, qhas);
            if(!qhas) d = r_add(d, qc);
            else if(auto arg = exp_arg_node(a, qb)) {
                auto p = poly_of(a, *arg, var);
                if(!p || !p->ok || !is_zero(p->a2) || !is_zero(p->a0) || r_cmp(p->a1, Rational{1, 1}) != 0 || r_cmp(qc, Rational{1, 1}) != 0) return std::nullopt;
                got_exp = true;
            }
            else return std::nullopt;
        }
        if(!got_exp || saw_frac) return std::nullopt;
        D = d;
        C = coef;
        saw_frac = true;
    }
    if(!saw_u || !saw_frac || is_zero(A)) return std::nullopt;
    Poly2 p{A, r_add(r_mul(A, D), B), r_add(r_mul(B, D), C), true};
    std::string uvar = var == "u" ? "v" : "u";
    out.push_back("Domain: e^(" + var + ") + " + format_expr(a, a.num(D)) + " != 0");
    out.push_back(uvar + " = e^(" + var + "), " + uvar + " > 0");
    out.push_back(format_expr(a, poly2_to_node(a, p, uvar)) + " = 0");
    auto us = solve_poly2(a, p, uvar);
    std::vector<std::string> xs;
    for(auto const &s : us) {
        auto uv = solution_line_value(a, s);
        if(!uv || *uv <= 0) {
...
static std::optional<std::vector<std::string>> affine_exp_common_slope_route(
    Arena &a,
    NodeId residual,
    std::string const &var,
    std::vector<std::string> out)
{
    ...
    bool saw = false;
    Rational slope{0, 1};
    std::vector<NodeId> coeffs, constants;
    for(NodeId term : terms) {
        Rational c{1, 1};
        NodeId body = term;
        bool has_body = true;
        ...
        if(has_body) {
            ...
            if(arg) {
                ...
                NodeId coef = a.num(c);
                if(!is_zero(p->a0))
                    coef = exact_eval_simplify(a, casio::mul(a, {coef, casio::power(a, casio::constant_e(a), a.num(p->a0))}));
    ...
    if(!saw || is_zero(slope) || constants.empty()) return std::nullopt;
    NodeId coef_sum = exact_eval_simplify(a, casio::add(a, coeffs));
    NodeId const_sum = exact_eval_simplify(a, casio::add(a, constants));
    auto cv = eval_node_env(a, coef_sum, {});
    auto kv = eval_node_env(a, const_sum, {});
    if(!cv || !kv || std::fabs(*cv) < 1e-12) return std::nullopt;
    if(*cv < 0) {
        coef_sum = exact_eval_simplify(a, casio::neg(a, coef_sum));
        const_sum = exact_eval_simplify(a, casio::neg(a, const_sum));
    }
    NodeId target = exact_eval_simplify(a, casio::div(a, casio::neg(a, const_sum), coef_sum));
    auto tv = eval_node_env(a, target, {});
    if(!tv || *tv <= 0) return std::nullopt;
    NodeId lin_arg = casio::simplify(a, casio::mul(a, {a.num(slope), a.sym(var)}));
    NodeId exact = exact_eval_simplify(a, casio::div(a, casio::fn(a, "ln", target), a.num(slope)));
    std::string exp_txt = "e^(" + format_expr(a, lin_arg) + ")";
    auto par = [&](NodeId id) {
        std::string s = format_expr(a, id);
        Node const &n = a.get(id);
        return (n.kind == NodeKind::Add || (!s.empty() && s[0] == '-')) ? "(" + s + ")" : s;
    };
    out.push_back(signed_sum_text(par(coef_sum) + "*" + exp_txt, format_expr(a, const_sum)) + " = 0");
    out.push_back(exp_txt + " = " + format_expr(a, target));
    out.push_back(format_expr(a, lin_arg) + " = ln(" + format_expr(a, target) + ")");
    out.push_back(var + " = " + format_expr(a, exact));
    return out;
}
...
static std::optional<NodeId> log_base_value(Arena &a, NodeId base)
{
    ...
    if(b.kind == NodeKind::Const && b.ckind == ConstKind::E) return one_node(a);
    if(b.kind == NodeKind::Num && b.num.num > 0) {
        if(b.num.num == b.num.den) return zero_node(a);
        return casio::fn(a, "log", base);
    }
    if(!has_symbols(a, base)) {
        auto v = eval_node_env(a, base, {});
        if(v && *v > 0 && std::fabs(*v - 1.0) > 1e-12) return casio::fn(a, "log", base);
    }
    return std::nullopt;
}
...
static std::optional<std::vector<std::string>> power_log_linear_solve_route(
    Arena &a, NodeId lhs, NodeId rhs, std::string const &var, std::vector<std::string> out)
{
    auto L = single_power_log(a, lhs, var);
    auto R = single_power_log(a, rhs, var);
    if(!L || !R) return std::nullopt;
    NodeId xcoef = exact_eval_simplify(a, sub_node(a, L->xcoef, R->xcoef));
    if(casio::same_by_sig(a, xcoef, zero_node(a))) return std::nullopt;
    NodeId constant = exact_eval_simplify(a, sub_node(a, R->constant, L->constant));
    NodeId ans = 0;
...
static std::string pi_mul_text(int n, bool half)
{
    int num = half ? 2 * n + 1 : n;
    int den = half ? 2 : 1;
    if(num == 0) return "0";
    bool neg = num < 0;
    if(neg) num = -num;
    std::string s;
    if(den == 1) s = num == 1 ? "pi" : std::to_string(num) + "*pi";
    else s = num == 1 ? "pi/2" : std::to_string(num) + "*pi/2";
    return neg ? "-" + s : s;
}

static std::optional<std::vector<std::string>> exp_trig_log_one_route(
    Arena &a,
    NodeId lhs,
    NodeId rhs,
    NodeId residual,
    ...
    std::optional<double> hi)
{
    auto run = [&](NodeId e_side, NodeId one_side) -> std::optional<std::vector<std::string>> {
        if(!is_num_node(a, one_side, 1)) return std::nullopt;
        auto arg = exp_arg_node(a, e_side);
        if(!arg) return std::nullopt;
        Node const &tr = a.get(*arg);
        if(tr.kind != NodeKind::Fn || (tr.fkind != FnKind::Sin && tr.fkind != FnKind::Cos && tr.fkind != FnKind::Tan)) return std::nullopt;
        Node const &lg = a.get(tr.a);
        if(lg.kind != NodeKind::Fn || lg.fkind != FnKind::Log || !is_sym_var(a, lg.a, var)) return std::nullopt;

        bool half = tr.fkind == FnKind::Cos;
        out.push_back(format_expr(a, e_side) + " = 1");
        out.push_back(format_expr(a, *arg) + " = 0");
        out.push_back("Domain: " + var + " > 0");
        out.push_back("ln(" + var + ") = " + (half ? "pi/2 + n*pi" : "n*pi"));
        out.push_back(var + " = e^(" + (half ? "pi/2 + n*pi" : "n*pi") + ")");
        if(!lo || !hi) return out;

        out.push_back(format_double_compact(*lo) + " <= " + var + " <= " + format_double_compact(*hi));
        std::vector<std::string> sols;
        for(int n = -12; n <= 12; ++n) {
            double theta = half ? (M_PI / 2.0 + n * M_PI) : (n * M_PI);
            double x = std::exp(theta);
            if(x < *lo - 1e-9 || x > *hi + 1e-9) continue;
            auto rv = eval_node(a, residual, var, x);
            if(!rv || std::fabs(*rv) > 1e-6 * std::max(1.0, std::fabs(x))) continue;
            std::string ang = pi_mul_text(n, half);
            sols.push_back(var + " = " + (ang == "0" ? "1" : "e^(" + ang + ")"));
        }
        if(sols.empty()) out.push_back(var + " = []");
        else out.push_back(solution_list_line(var, sols));
        return out;
...
static std::optional<NodeId> positive_log_value(Arena &a, NodeId n, std::string const &var)
{
    if(contains_symbol(a, n, var)) return std::nullopt;
    Node const &x = a.get(n);
    if(x.kind == NodeKind::Num) {
        if(x.num.num <= 0) return std::nullopt;
        if(x.num.num == x.num.den) return zero_node(a);
        return casio::fn(a, "log", n);
    }
    ...
    if(x.kind == NodeKind::Div) {
        auto top = positive_log_value(a, x.a, var);
        auto bot = positive_log_value(a, x.b, var);
        if(!top || !bot) return std::nullopt;
        return exact_eval_simplify(a, sub_node(a, *top, *bot));
    }
    return std::nullopt;
}
    ...
    Arena &a, NodeId lhs, NodeId rhs, std::string const &var, std::vector<std::string> out)
{
    auto run = [&](NodeId e_side, NodeId c_side) -> std::optional<std::vector<std::string>> {
        ...
        if(!ce || ce->first.num <= 0) return std::nullopt;
        if(!c) {
            auto logc = positive_log_value(a, c_side, var);
            if(!logc) return std::nullopt;
            if(ce->first.num != ce->first.den)
                logc = exact_eval_simplify(a, sub_node(a, *logc, casio::fn(a, "log", casio::num(a, ce->first.num, ce->first.den))));
            NodeId eq = casio::simplify(a, sub_node(a, ce->second, *logc));
            auto lin = symbolic_linear_parts(a, eq, var);
            if(!lin) return std::nullopt;
            NodeId exact = exact_eval_simplify(a, casio::div(a, casio::neg(a, lin->c), lin->m));
            out.push_back(format_expr(a, e_side) + " = " + format_expr(a, c_side));
            out.push_back(format_expr(a, ce->second) + " = " + format_expr(a, *logc));
            ...
            std::string rtxt = format_expr(a, rhs_no_c);
            if(rtxt.rfind("ln(", 0) == 0 && rtxt.back() == ')' &&
               (is_negative_one_num(a, lin->m) || format_expr(a, lin->m) == "-1")) {
                exact_text = "-" + rtxt;
            }
            else if(auto m = as_num(a, lin->m); m && m->num < 0) {
                if(rtxt.rfind("ln(", 0) == 0 && rtxt.back() == ')') {
                    auto r = parse_rational_key(rtxt.substr(3, rtxt.size() - 4));
                    if(r && r->num > 0 && r->den > 0) {
                        Rational inv{r->den, r->num};
                        inv.normalize();
                        Rational scale{m->den, -m->num};
                        scale.normalize();
        ...
        if(auto lin = symbolic_linear_parts(a, eq, var)) {
            ...
            if(auto m = as_num(a, lin->m)) {
                bool used_lp = false;
                if(auto lp = log_power_solution_text(target, *m)) {
                    exact_text = *lp;
                    used_lp = true;
                }
                if(!used_lp && m->num < 0 && target.den != 1) {
    ...
    Arena &a, NodeId residual, std::string const &var, std::vector<std::string> out)
{
    ...
    if(auto lin = symbolic_linear_parts(a, ce->second, var)) {
        NodeId exact = exact_eval_simplify(a, casio::div(a, sub_node(a, *logt, lin->c), lin->m));
        Node const &q = a.get(exact);
        if(q.kind == NodeKind::Div) {
            if(auto lr = natural_log_power_ratio(a, q.a, q.b)) exact = casio::num(a, lr->num, lr->den);
        }
        out.push_back(var + " = " + ln_text(exact));
        return out;
    }
...
std::vector<std::string> run(Arena &arena, Request const &req)
{
    ...
    try {
        ...
        if(req.mode == 10) {
            ...
            if(auto le = simplify_log_exp_text(arena, n)) n = exact_eval_simplify(arena, casio::parse_expr(arena, *le));
            ...
            if(!contains_symbol(arena, n, var)) {
                ...
                else if(auto qr = quadratic_rational_interval_range(arena, n, var, lo, hi, lo_open, hi_open, steps)) {
                    range_answer = *qr;
                    steps.push_back("Range: " + range_answer + ".");
                }
                ...
                else if(auto sq = sqrt_quadratic_interval_range(arena, n, var, lo, hi, lo_open, hi_open, steps)) {
                    range_answer = *sq;
                    steps.push_back("Range: " + range_answer + ".");
                }
            ...
            if(pn.kind == NodeKind::Fn && (pn.fkind == FnKind::Asin || pn.fkind == FnKind::Acos || pn.fkind == FnKind::Atan)) {
                ...
                if((pn.fkind == FnKind::Asin && inner.kind == NodeKind::Fn && inner.fkind == FnKind::Sin) ||
                   ...
                   (pn.fkind == FnKind::Atan && inner.kind == NodeKind::Fn && inner.fkind == FnKind::Tan)) {
                    ...
                    return casio::exam_block("inverse trig branch", steps, n_text ? *n_text : format_expr(arena, n));
                }
            }
        ...
        if(auto log_sym = log_linear_symbolic_route(arena, rearr, solve_var)) {
            out.insert(out.end(), log_sym->begin(), log_sym->end());
            return out;
        }
        ...
        if(auto pl = power_log_linear_solve_route(arena, lhs, rhs, solve_var, out)) return *pl;
        ...
        if(auto arcf = common_affine_recip_power_factor_route(arena, rearr, solve_var, out)) return *arcf;
        ...
        if(auto pqs = power_quadratic_substitution_route(arena, rearr, solve_var, interval_lo, interval_hi, interval_lo_open, interval_hi_open)) return *pqs;
        ...
        if(!rp.ok) {
            ...
            if(auto rr = rational_root_substitution_route(arena, rearr, solve_var, interval_lo, interval_hi, interval_lo_open, interval_hi_open)) return *rr;
