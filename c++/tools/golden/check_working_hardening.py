...
CASES: list[tuple[str, str, list[str], list[str]]] = [
    ...
    (
        ...
        ["cos(2*x)=0 => sin(2*x)+cos(2*x)=+/-1 != 0", "sin(2*x)/cos(2*x)+1=0", "tan(2*x)=-1", "x=-22.5+90n", "n = -1,0,1,2", "Answer: x = [-112.5, -22.5, 67.5, 157.5]"],
        ["Divide by cos(2x) where valid", "Solve the tan equation", "ERR:"],
    ),
    (
        "trig",
        "sin(2*x)+cos(2*x)=0,x,0,pi",
        ["x=-pi/8+pi*n/2", "0 <= x <= pi", "n = 1,2", "Answer: x = [3*pi/8, 7*pi/8]"],
        ["NEEDS_REVIEW", "ERR:"],
    ),
    (
        "trig",
