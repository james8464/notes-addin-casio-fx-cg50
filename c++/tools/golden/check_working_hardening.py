...
CASES: list[tuple[str, str, list[str], list[str]]] = [
    ...
    (
        "alg",
        "simplify(1/a+2/b+3/c)",
        ["(b*c + 2*a*c + 3*a*b)/(a*b*c)"],
        ["ERR:", "Done"],
    ),
    (
        "alg",
    ...
    (
        "alg",
        "partfrac((1-4*x)/((x^2+3)*(x-2)^2))",
        ["(A*x + B)/(x^2 + 3) + (C)/(x - 2) + (D)/((x - 2)^2)", "A = 1", "D = -1", "1/(x^2 + 3) - 1/((x - 2)^2)"],
        ["ERR:", "Done"],
    ),
    (
        "alg",
    ...
    (
        "alg",
        "solve([8^y=4^(2*x+1),27^(2*y)=9^(x-3)],[x,y])",
        ["2^(3*y) = 2^(4*x + 2)", "6*y = 2*x - 6", "(x,y) = [(-5/3,-14/9)]"],
        ["ERR:", "Done", "solve(["],
    ),
    (
        "alg",
        "solve([4*x-3*y=11,9^(y+3)=3*sqrt(3)/27^x],[x,y])",
        ["3^(2*y + 6) = 3^(- 3*x + 3/2)", "x = 1/2", "y = -3", "(x,y) = [(1/2,-3)]"],
        ["ERR:", "Done", "solve(["],
    ),
    (
        "alg",
        "solve(8^x=4*2^(-x),x)",
        ["2^(3*x) = 2^(- x + 2)", "3*x = - x + 2", "x = [1/2]"],
        ["ERR:", "Done", "ln(2)/(4*ln(2))"],
    ),
    (
        "alg",
        "solve([14=a*2^n,126=a*6^n],[a,n])",
        ["(6/2)^n = 9", "n = 2", "a = 14/4 = 7/2", "(a,n) = [(7/2,2)]"],
        ["ERR:", "Done", "solve(["],
    ),
    (
        "alg",
        "solve([1=a*b^(1/2),8=a*b^2],[a,b])",
        ["b^(3/2) = 8", "b = 4", "a = 1/2", "(a,b) = [(1/2,4)]"],
        ["ERR:", "Done", "solve(["],
    ),
    (
        "alg",
        "simplify((x^6-y*z^4)/(x^4+x*z^2*sqrt(y)))",
        ["(x^3 - sqrt(y)*z^2)*(x^3 + sqrt(y)*z^2)", "x^2 - sqrt(y)*z^2/x"],
        ["ERR:", "Done"],
    ),
    (
        "alg",
        "simplify(sqrt(9*x^6*y^4)/(3*x^2*y^3)^2)",
        ["sqrt(9*x^6*y^4)/(9*x^4*y^6)", "1/(3*x*y^4)"],
        ["ERR:", "Done", "abs("],
    ),
    (
        "alg",
        "simplify(sqrt((3*a^3*b*c*6*a^2*b^2*c^3)/(2*a*b*c^4)))",
        ["sqrt(9*a^4*b^2)", "3*a^2*b"],
        ["ERR:", "Done", "abs("],
    ),
    (
        "alg",
        "simplify(((2*a^(1/2)*b^(1/3))^4)*(4*a^6*b^2)^(-1/2))",
        ["16*a^2*b^(4/3)*(4*a^6*b^2)^(-1/2)", "8*b^(1/3)/a"],
        ["ERR:", "Done", "abs("],
    ),
    (
        "alg",
    ...
    (
        "alg",
        "solve(log(10,2^x-1)-log(10,2)=log(10,2^x+3)-log(10,2^x-1),x)",
        ["2*log(10,2^x - 1) = log(10,2) + log(10,2^x + 3)", "(2^x - 1)^2 = 2*(2^x + 3)", "u = 2^x, u > 0", "x = [ln(5)/ln(2)]"],
        ["x = [2.321", "ERR:"],
    ),
    (
        "alg",
        "solve((a^4-2*a^2*b^2+b^4)^(x-1)=(a-b)^(2*x)*(a+b)^(-2),x)",
        ["a^4 - 2*a^2*b^2 + b^4 = (a-b)^2*(a+b)^2", "(a+b)^x = b-a", "x = [ln(b-a)/ln(a+b)]"],
        ["ERR:"],
    ),
    (
        "alg",
