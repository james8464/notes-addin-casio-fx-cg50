...
DIRECT: dict[int, tuple[list[str], list[str]]] = {
    19: ([str(HOST), "--derive", "x^y*y^x=1,method=implicit"], ["Differentiate both sides", "dy/dx"]),
    21: ([str(HOST), "--int", "1/(x^4+1)"], ["Sophie Germain", "Factor:", "Answer:"]),
    22: ([str(HOST), "--int", "sqrt(tan(x))"], ["u=sqrt(tan(x))", "Factor u^4+1", "Answer:"]),
    23: ([str(HOST), "--int", "x^2/((x*sin(x)+cos(x))^2)"], ["hidden derivative", "u=x*sec(x)", "Answer:"]),
    ...
    78: ([str(HOST), "--int", "1/(1+sin(x)+cos(x))"], ["Weierstrass", "tan(x/2)", "Answer:"]),
    82: ([str(HOST), "--int", "1/(a^2-x^2)"], ["partial fractions", "A=B=1/(2a)", "Answer:"]),
    ...
    98: ([str(HOST), "--int", "(2*x+3)/(x^2+3*x+5)"], ["Split numerator", "log(abs(x^2 + 3*x + 5))", "Answer:"]),
}


STATIC: dict[str, tuple[list[int], list[str], str]] = {
    "systems": (list(range(1, 11)), ["Let s=x+y,p=xy.", "Set u=log/base exp."], "main"),
