...
CASES: tuple[MatrixCase, ...] = (
    ...
    MatrixCase("pure_functions_graphs", "restricted log range", ("--alg", "range(ln(3*x-2)+3,x>=1)"), ("Endpoint gives y = 3", "Range: y >= 3")),
    MatrixCase("pure_functions_graphs", "restricted odd power range", ("--alg", "range(x^3,x>e)"), ("x^3 is increasing", "Range: y > e^(3)")),
    MatrixCase("pure_functions_graphs", "split rational range across asymptote", ("--alg", "range((2*x+3)/(x-2),x>=0)"), ("Vertical asymptote x = 2", "Range: y <= -3/2 or y > 2")),
    MatrixCase("pure_functions_graphs", "restricted cosine inverse", ("--alg", "inverse(2*cos(2*x),x>=0)"), ("f^-1(x) = acos(x/2)/2",)),
    MatrixCase("pure_functions_graphs", "log-base composition simplification", ("--alg", "compose(3*2^(-x)-1,log(2,x))"), ("f(g(x)) = (- x + 3)/x",)),
    MatrixCase("pure_functions_graphs", "domain-aware log-base compare", ("--alg", "compare(3*2^(-log(2,x))-1,3/x-1)"), ("E1-E2 = 0", "not equivalent")),
    ...
    MatrixCase("pure_coordinate_geometry", "line through two points", ("--alg", "line2p((7,3),(-5,0))"), ("m = (0 - 3)/(-5 - 7) = 1/4", "x - 4*y + 5 = 0")),
    MatrixCase("pure_coordinate_geometry", "midpoint helper", ("--alg", "midpoint((-1,5),(7,11))"), ("M = (3,8)",)),
    MatrixCase("pure_coordinate_geometry", "distance helper", ("--alg", "dist2p((0,3),(2,-1))"), ("2*sqrt(5)",)),
    ...
    MatrixCase("pure_sequences", "sqrt quotient binomial product", ("--alg", "series(sqrt((1+a*x)/(4-x)),x,0,3)"), ("(a*x + 1)^1/2", "(- x + 4)^-1/2", "1/32*a - 1/16*a^2 + 3/256")),
    ...
    MatrixCase("pure_trigonometry", "solve wrapper keeps radical trig working", ("--alg", "solve(sqrt(2*cos(2*x))=1,x,0,pi/2)"), ("2*cos(2*x) = (1)^2", "0 <= x <= pi/2", "x = [pi/6]")),
    ...
    MatrixCase("pure_integration", "area between curves integral", ("--int", "defint(x-x^2,x,0,1)"), ("F(1) - F(0)", "1/6")),
    MatrixCase("pure_integration", "volume of revolution integral", ("--int", "defint(pi*(4-x^2)^2,x,-2,2)"), ("F(2) - F(-2)", "512*pi/15")),
