...
graph TD
  ...
  Q --> QSampling["latest complete: probability sampling distributions"]
```
...
- `check_madasmaths_standard_question_corpus.py`: `4554` rows, `6103` manual cases, no failures.
- `check_madasmaths_download_coverage.py`: `462` downloaded MadAsMaths question PDFs, `257` covered, `205` still gap-listed for future manual audit.
