...
graph TD
  ...
  Q --> Qt["latest complete: M1 dynamics"]
```
...
- `check_madasmaths_standard_question_corpus.py`: `4554` rows, `6130` manual cases, no failures.
- `check_madasmaths_download_coverage.py`: `462` downloaded MadAsMaths question PDFs, `264` covered, `198` still gap-listed for future manual audit.
