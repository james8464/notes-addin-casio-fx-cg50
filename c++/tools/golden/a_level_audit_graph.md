...
graph TD
  ...
  Q --> Qt["latest complete: M1 collisions"]
```
...
- `check_madasmaths_standard_question_corpus.py`: `4554` rows, `6120` manual cases, no failures.
- `check_madasmaths_download_coverage.py`: `462` downloaded MadAsMaths question PDFs, `262` covered, `200` still gap-listed for future manual audit.
