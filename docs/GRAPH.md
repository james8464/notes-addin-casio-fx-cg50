...
Last updated: 2026-06-01 05:29 BST
...
graph TD
  ...
  G3A --> Size["2,097,100 bytes; 52 byte headroom"]
```
...
```mermaid
graph TD
  Work["working engine"] --> Diff["diff: guarded affine chain powers, optimisation/quotient derivatives, ln^2 chain, exp product routes incl 4*(x^2-2)*exp(-2*x), implicit, trig basics"]
