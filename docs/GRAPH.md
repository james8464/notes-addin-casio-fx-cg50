...
Last updated: 2026-05-31 12:13 Europe/London
...
graph TD
  ...
  Direct --> RationalDomain["rational denominator exclusions as inequalities"]
...
```mermaid
graph LR
  Build["./compile exit 0"] --> Size["1,302,831 bytes"]
  Build --> Rom["rom 1,274,155 bytes"]
  ...
  Help["help pack"] --> HelpQ["40 function sheets ok, PAK 41 records / 9,485 bytes"]
  Help --> HelpEx["85/85 help examples run through same-source working engine"]
  ...
  Shared["shared working"] --> SharedRun["246/246 thin host+calculator adapter ok"]
  Shared --> WorkQ["34/34 working-quality depth cases ok"]
  Shared --> CoreRun["246/246 core routes without golden fallback ok"]
  ...
  Graphify["graphify update . --no-cluster"] --> GraphStats["10,310 nodes / 588,578 edges"]
```
