...
Last updated: 2026-06-01 05:41 BST
...
graph TD
  ...
  G3A --> Size["2,097,108 bytes; 44 byte headroom"]
```
...
```mermaid
graph TD
  Work["working engine"] --> Diff["diff: guarded affine chain powers, optimisation/quotient derivatives, ln^2 chain, arctan inverse trig, exp product routes incl 4*(x^2-2)*exp(-2*x), implicit, trig basics"]
...
graph TD
  ...
  Runner --> Strict["13,012/14,256 strict marker pass"]
  Strict --> Remaining["remaining strict clusters: integrate 639, algebra 560, derive 40, trig 5; 44 B hard headroom"]
```
