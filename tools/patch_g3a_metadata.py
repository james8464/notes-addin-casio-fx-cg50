...
def put_ascii(buf: bytearray, offset: int, size: int, text: str) -> None:
    ...
    if len(raw) >= size:
        raise ValueError(f"{text!r} too long for {size}-byte field")
    buf[offset : offset + size] = b"\0" * size
    buf[offset : offset + len(raw)] = raw


def put_fixed(buf: bytearray, old: bytes, new: str) -> None:
...
def patch(path: Path, app_name: str, internal_name: str, filename: str) -> None:
    ...
    if len(buf) < 0x7004:
        raise ValueError(f"{path} too small for .g3a")
    ...
    put_fixed(buf, b"Khicas50", app_name)
    put_fixed(buf, b"Khicas90", app_name)
    put_fixed(buf, b"Khicasen", app_name)
    put_fixed(buf, b"@KHICAS50", internal)
    put_fixed(buf, b"@KHICAS90", internal)
    put_fixed(buf, b"@KHICASEN", internal)
    put_fixed(buf, b"khicas50.g3a", "CasioCAS.g3a")
    put_fixed(buf, b"khicas90.g3a", "CasioCAS.g3a")
    cksum = checksum(buf).to_bytes(4, "big")
    buf[CHECKSUM : CHECKSUM + 4] = cksum
    buf[-4:] = cksum
