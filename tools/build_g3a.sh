...
docker run --rm \
  ...
  "${IMAGE_TAG}" \
  bash -lc 'set -euo pipefail; mkdir -p /shared/tmp ~/.wine/drive_c; make clean; make -j"${CASIO_MAKE_JOBS}" "${CASIO_KHICAS_TARGET}"'
...
ls -lh "${TRANSFER_DIR}/CasioCAS.g3a" "${TRANSFER_DIR}/CASIOCAS.PAK"
